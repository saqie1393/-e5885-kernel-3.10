...
{
	const struct ipt_tos_info *tosinfo = par->targinfo;
	struct iphdr *iph = ip_hdr(skb);
...
{
	const struct ipt_tos_info *info = par->targinfo;
...
{
	const struct xt_tos_match_info *info = par->targinfo;
	struct iphdr *iph = ip_hdr(skb);
	u_int8_t orig, nv;
...
static struct xt_target dscp_tg_reg[] __read_mostly = {
	...
	},
	{
		...
		.table		= "mangle",
		.target		= tos_tg_v0,
		.targetsize	= sizeof(struct ipt_tos_info),
	...
	},
	{
		...
		.table		= "mangle",
		.target		= tos_tg,
		.targetsize	= sizeof(struct xt_tos_match_info),
		.me		= THIS_MODULE,
	},
	{
		.name		= "TOS",
