...
{    
    ...
    /*8 REGION_Germany */
    {
         ...
         {
             /* 铃声 Ring profile & Ring Cadence profile*/
             RING_VE960_ABS100V_DEF, CR_DE,CR_DTMF_CN,
             /* 拨号音 Dial Tone profile & Dial Tone Cadence profile*/
             T_DL_DE, CT_DL_DE,
             /* 回铃音 Ringback Tone profile & Ringback Tone Cadence profile*/
             T_RB_DE, CT_RB_DE,
     ...
     /*22 REGION_Australia*/
    {
        ...
        /* Tone profile*/
         {
             /* 铃声 Ring profile & Ring Cadence profile*/
             RING_VE960_ABS100V_DEF, CR_AU,CR_DTMF_CN,
             /* 拨号音 Dial Tone profile & Dial Tone Cadence profile*/
             T_DL_AU, CT_DL_AU,
             /* 回铃音 Ringback Tone profile & Ringback Tone Cadence profile*/
             T_RB_AU, CT_RB_AU,
...
VpStatusType slic_config_line(void)
{
    ...
    /*有定制产品需要打开此宏，所有这里先默认配置成flashMax*/
#ifdef EXTENDED_FLASH_HOOK
    pulseType.onHookMin  = pulseType.flashMax;
#endif
    ...
    switch(eventid)
    {
        ...
        case VP_LINE_EVID_DTMF_DIG:    /*DTMF按键事件*/
        {
            ...
            {                                 
                ...
                //printk(KERN_ERR"key level,data:%ld\n",dBm);
                if(dBm < KEY_VAULE)
                {
                    no_send_flag = 1;
                    break;
        ...
        case SLIC_VOIP_BAND_SWITCH:
            {
                ...
                printk(KERN_ERR"VOIPsetband_%d\n",slic_ctl->band_switch);
#endif
                break;
            }
...
void slic_softtimer_handle(void* para)
{
    struct softtimer_list *softtimer = NULL;
    
    if(NULL == para)
    {
        printk(KERN_ERR "slic_softtimer_handle:para is null,stop.\n");
        return;
    }
    softtimer = (struct softtimer_list *)para;
...
int slic_band_switch(int type)
{
    ...
    if(BAND_SWITCH_TO_WIDE == type)
    {
        clkrate = SND_SAMPLE_RATE_16K;
        printk(KERN_ERR "slic band switch to wideband.\n");
    }
    else
    {
        clkrate = SND_SAMPLE_RATE_8K;
        printk(KERN_ERR "slic band switch to norrowband.\n");
    }
...
/*****************************************************************************
 ...
 Description  : 装备执行这个函数启动回环测试
 Input        : channel:通道，目前只支持通道0
                type   :振铃或者语音
                onoff  :开始、停止
 Output       : None
...
{    
    ...
    switch(type)
    {
        ...
        case TYPE_LOOPBACK_CMD:
        {
            if(0 == loopback_flag)
            {
                (void)drv_start_user_process("slic_pcm_loopback", NULL, NO_NEED_RESULT, 0);
                loopback_flag = 1;
            }
            return 0;
        }
...
/*****************************************************************************
 Prototype    : is_slic_init_ok
 Description  : 装备工位查询slic是否初始化成功
 Input        : enable
 Output       : None
 Return Value :
 Calls        :
