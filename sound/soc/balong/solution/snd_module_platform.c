...
extern void cpufreq_set_boost_hifi(int boost_en);
#endif
...
#if ( FEATURE_ON == MBB_MLOG )
            mlog_print(MLOG_PCM, mlog_lv_factory, "%s: play data error. prt->next_period:%d i:%d, val: %x, val+1: %x\n",
                                __func__, prt->next_period, i, addr[i], addr[i + 1]);
#endif
...
#if ( FEATURE_ON == MBB_MLOG )
        mlog_print(MLOG_PCM, mlog_lv_factory, "%s: send data req to hifi err.\n", __func__);
#endif
...
#if (FEATURE_ON == MBB_TELEMATIC_AUDIO)
/*CPU自动调频命令不处理pcm声卡，因此不需要判断声卡设备是否为NULL。*/
    if (ID_DRV_CPU_AUTO_CFG == cmd)
    {
        u32  *pCpuAutoEnable = NULL;
        if (NULL == data)
        {
            printk(KERN_ERR"[%s] ID_DRV_CPU_AUTO_CFG err param.\n",__func__);
            return -1;
        }
        else
        {
            pCpuAutoEnable = (u32 *)data;
            cpufreq_set_boost_hifi((int)*pCpuAutoEnable);
            return 0;
        }
    }
#endif
    ...
    switch(cmd)
    {
        ...
        case ID_HIFI_DRV_BUFFER_DATA_READY:
        {
...
#if ( FEATURE_ON == MBB_MLOG )
                mlog_print(MLOG_PCM, mlog_lv_factory, "%s: pass the err params.\n", __func__);
#endif
            ...
            if (SNDRV_PCM_STREAM_PLAYBACK == stream)
            {
                ...
                /*如果有效数据小于period_size，则发送上一次数据*/
                if ( prtd->period_size > availnum)
                {
                    printk(KERN_ERR"[%s]availnum = %d.\n",__func__,availnum);
                    pcm_miss_info.play_miss_num++;

                    //当播放到最后一帧数据时候，驱动把这帧数据当成异常帧经行处理了，但是需要把这帧数据继续播放出去
                    if (0 != prtd->period_num)
                    {
                        prtd->next_period = (prtd->next_period + 1) % prtd->period_num;
                    }

                    //当availnum为0时候，说明播放到结尾数据了，需要立刻停止本地声卡驱动，否者会产生噪音
                    if (0 == availnum)
                    {
                        (void)snd_pcm_stop(substream, SNDRV_PCM_STATE_DISCONNECTED);
                        break;
                    }
                }
                else
...
#if ( FEATURE_ON == MBB_MLOG )
                    mlog_print(MLOG_PCM, mlog_lv_factory, "%s: buf is full,empty num = %d.\n", __func__, availnum);
#endif
                }
...
void snd_sw_params_set(struct snd_pcm_substream *substream)
{
    ...
    runtime->format = dma_hardware.formats;
    runtime->rate = (g_sample_rate == SYNC_CLK_8K) ? \
    ...
    pcm_prtd->period_num  = (g_sample_rate == SYNC_CLK_16K) ? \
                            runtime->periods : (runtime->periods * 2); //8k时缓存个数是16k时的两倍
    pcm_prtd->period_size = (g_sample_rate == SYNC_CLK_16K) ? \
                            SND_LLI_BLOCK_SIZE : (SND_LLI_BLOCK_SIZE / 2); //16k时每个缓存大小是8k的两倍
...
static int snd_hifi_trigger(struct snd_pcm_substream *substream, int cmd)
{
...
#if ( FEATURE_ON == MBB_MLOG )
        mlog_print(MLOG_PCM, mlog_lv_factory, "%s: prt is null.\n", __func__);
#endif
    ...
    case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
        ...
        start_quest.ulSampeRate = g_sample_rate;
...
#if ( FEATURE_ON == MBB_MLOG )
            mlog_print(MLOG_PCM, mlog_lv_factory, "%s: send start cmd to hifi err.\n", __func__);
#endif
...
#if ( FEATURE_ON == MBB_MLOG )
        mlog_print(MLOG_PCM, mlog_lv_factory, "%s: this device has been opened.\n", __func__);
#endif
...
#if ( FEATURE_ON == MBB_MLOG )
        mlog_print(MLOG_PCM, mlog_lv_factory, "%s: kzalloc runtime_data fail.\n", __func__);
#endif
...
#if ( FEATURE_ON == MBB_MLOG )
        mlog_print(MLOG_PCM, mlog_lv_factory, "%s: wrong device num.\n", __func__);
#endif
...
#if ( FEATURE_ON == MBB_MLOG )
        mlog_print(MLOG_PCM, mlog_lv_factory, "%s: allocate pages failed!\n", __func__);
#endif
    }
    return ret;
...
static int snd_hifi_hw_params(struct snd_pcm_substream* substream,
                              struct snd_pcm_hw_params* params)
{
    if (NULL == params || NULL == substream)
    {
        printk(KERN_ERR"get params fail\n");
        return -1;
    }

    g_sample_rate = params_rate(params);
...
#if ( FEATURE_ON == MBB_MLOG )
        mlog_print(MLOG_PCM, mlog_lv_factory, "%s: snd_module_platform_probe register failed: %d\n", __func__, ret);
#endif
        return ret;
    }
