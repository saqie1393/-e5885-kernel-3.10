...
/*Just for coverity*/
unsigned int host_release_stub_data = 0;
        ...
        if (udev->config
            ...
            if (__usb_get_extra_descriptor (udev->rawdescriptors[0],
                ...
                int err = usb_control_msg(udev,
                              ...
                              USB_NTF_HOST_REL,
                              0, &host_release_stub_data, 0, USB_CTRL_SET_TIMEOUT);
                if (err < 0) {
                    dev_info(&udev->dev,
...
static int xhci_plat_probe(struct platform_device *pdev)
{
	...
	if(DWC3_MODE_DRD == bsp_usb_mode_support()){
		unsigned int hc_flags = (unsigned int )driver->flags;

		/* The XHCI controller is configured as a Synopsys USB 3.0 OTG XHCI */
		if (hc_flags & HCD_USB3) {
...
static int xhci_plat_remove(struct platform_device *dev)
{
	...
	if(DWC3_MODE_DRD == bsp_usb_mode_support()){
		...
		if (hc_flags & HCD_USB3) {
			...
			printk("OTG: Removing host on otg=%pK\n", otg);
