...
static USB_VOID fsg_send_data_to_host(struct fsg_common* common , USB_INT len)
{
    ...
    {
        /* Nothing to send */
        printk(KERN_INFO "Nothing to send \n");
    }
...
static USB_INT do_write(struct fsg_common* common)
{
    ...
    while (amount_left_to_write > 0)
    {
        ...
        {
            DBG_I(MBB_MASS, "Maybe sleep_thread\n");
            common->rx_semaphore_flag = 1;
            spin_unlock_irqrestore(&common->rx_lock , flags);
            rc = down_timeout(&common->rx_semaphore,msecs_to_jiffies(RX_SEMAPHONE_TIMEOUT));
            if (-ETIME == rc)
            {
                DBG_E(MBB_MASS, "rx_semaphore timeout!\n");
                curlun->sense_data = SS_COMMUNICATION_FAILURE;
                curlun->sense_data_info = file_offset >> curlun->blkbits;
                curlun->info_valid = 1;
                return 0;
