...
static USB_INT rndis_set_alt(struct usb_function *f, USB_UINT intf, USB_UINT alt)
{
    ...
    DBG_I(MBB_RNDIS, "rndis_set_alt: intf=%u, alt=%u, ctrl_id=%u, data_id=%u\n",
    ...
    if (intf == rndis->ctrl_id) {
        if (rndis->notify->driver_data) {
            DBG_I(MBB_NCM, "reset rndis control %u\n", intf);
            usb_ep_disable(rndis->notify);
        }
        if (!rndis->notify->desc) {
            DBG_I(MBB_NCM, "init rndis ctrl %u\n", intf);
