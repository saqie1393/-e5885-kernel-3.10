...
/*regulator 3.2v  for typec chip*/
#define TYPEC_REGULATOR_VOLT 3200000
...
#define NXP_MAX_REG 0x1A
#define TYPEC_MAX_REGS 0x20
...
typedef struct _pistat
{
    ...
    unsigned char attached_port_stat: BITS_3;
...
typedef struct _tustat
{
    ...
    unsigned char attach_ss: BITS_2;
} tustat;
...
int usbc_i2c_hotplug(int action)
{
    ...
    DBG_T(MBB_TYPECREG, "action:%d before wait!", action);
    /*等待初typec探测完成*/
    ...
    switch (action)
    {
        case TYPEC_USB_LINE_INSERT:
        {
            ...
            if (0 == ret)
            {
                ...
                {
                    DBG_T(MBB_TYPECREG, "hotplug read 0%dH = 0x%02X \n", i, buffer[i]);
                }
            }
            else
            {
                        ...
                        const char* buff, size_t size)
{
    ...
    usbc_i2c_write(usbc_driver, reg_value, reg_num);
                        ...
                        const struct i2c_device_id* id)
{
    ...
    DBG_T(MBB_TYPECREG, "%s probe: name =%20s!\n", __FUNCTION__, id->name);
    ...
    DBG_T(MBB_TYPECREG, "s_index=%d,client=%x,addr=%x,name=%20s\n", s_index, client, client->addr, client->name);
    ...
    DBG_T(MBB_TYPECREG, "device tree information gpio %d, default value =%d\n", gpio_num, gpio_value);
    ...
    DBG_T(MBB_TYPECREG, "gpio %d push %d \n", gpio_num, gpio_value);
    ...
    if (0 == usbc_i2c_read(usbc_drv, regs, usbc_drv->reg_max))
    {
        ...
        DBG_T(MBB_TYPECREG, "start set init mode \n");
        usbc_set_init_mode(usbc_drv);
        wake_up(&typec_wait);
    }
    else
    {
        DBG_E(MBB_TYPECREG, "device not present\n");
    }
    s_index++;
    return 0;
}
