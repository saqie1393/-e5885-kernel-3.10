...
#define ETHER_CONNECT_NOTIFY_STRING ("1")
#define ETHER_DISCONNECT_NOTIFY_STRING ("0")
...
#ifdef FEATURE_HUAWEI_ECM_CONNECTION_MANUAL
#define ECM_SINGLE_INDEX               0
static USB_BOOL g_ecm_connection_manual = false;
#endif
...
USB_BOOL ecm_get_bypass_state(USB_VOID)
{
    ...
    DBG_I(MBB_PNP, "ecm bypass state = %u, is force=%u\n", bRet, g_ecmforce_bypass_mode);
    return bRet;
}
...
static struct usb_interface_descriptor ecm_data_nop_intf = {
    ...
    .bInterfaceSubClass = USB_CDC_PROTO_NONE,
#else
    .bInterfaceSubClass =    USB_CDC_SUBCLASS_ETHERNET,
#endif
...
#ifdef FEATURE_HUAWEI_ECM_CONNECTION_MANUAL
    if(!g_ecm_connection_manual)
    {
        ecm->notify_state = ECM_NOTIFY_NONE;
        DBG_I(MBB_ECM, "g_ecm_connection_manual is false");
        return;
    }
#endif
...
static USB_INT ecm_set_alt(struct usb_function *f, USB_UINT intf, USB_UINT alt)
{
    ...
    DBG_I(MBB_ECM, "ecm_set_alt: intf=%u, alt=%u, ctrl_id=%u, data_id=%u\n",
...
#ifdef FEATURE_HUAWEI_ECM_CONNECTION_MANUAL
static ssize_t ecm_network_connection_show(struct device * pdev, struct device_attribute * attr, char * buf)
{
    if (NULL == buf)
    {
        return -EPERM;
...
static ssize_t ecm_network_connection_store(struct device *pdev, struct device_attribute *attr, const char *buf, size_t count)  
{
    struct f_ecm        *ecm = NULL;

    g_ecm_connection_manual = true;
    if (!the_ecm[ECM_SINGLE_INDEX])
    {
        DBG_T(MBB_ECM, "the_ecm is null\n" );
        return -EPERM;
    }
    ecm = the_ecm[ECM_SINGLE_INDEX];

    DBG_I(MBB_ECM, "enter, count=%d\n", count);
    if (!buf)
    {
        DBG_T(MBB_ECM, "buf is null\n" );
        return -EPERM;
    }

    if (!strncmp(buf, ETHER_CONNECT_NOTIFY_STRING, strlen(ETHER_CONNECT_NOTIFY_STRING)))
    {
       DBG_I(MBB_ECM, "wValue 1\n");
       ecm->is_open = true;
    }
    else if (!strncmp(buf, ETHER_DISCONNECT_NOTIFY_STRING, strlen(ETHER_DISCONNECT_NOTIFY_STRING)))
    {
        DBG_I(MBB_ECM, "wValue 0\n");
        ecm->is_open = false;
    }
    else
    {
        DBG_T(MBB_ECM, "cannot compare: buf=%s\n", buf);
        return -EPERM;
    }
...
static DEVICE_ATTR(ecm_network_connection, S_IRUGO | S_IWUSR, ecm_network_connection_show, ecm_network_connection_store);
#endif /* FEATURE_HUAWEI_ECM_CONNECTION_MANUAL */
...
#ifdef FEATURE_HUAWEI_ECM_CONNECTION_MANUAL
    &dev_attr_ecm_network_connection,
#endif
    NULL
};
