...
#define GPIO_6_2	                  (50)  //OTG检测管脚定义 ,B525 MMI优盘检测适配
...
enum mbb_binary_gpio
{
    GPIO_DET,    /*gpio detect */
    GPIO_SET,   /*gpio set phy*/
    GPIO_DMDP,    /*gpio dmdp*/
};

int mbb_binary_get_gpio( int gpio);

#define GPIO_OTG_ID_DET      mbb_binary_get_gpio(GPIO_DET)//g_otg_id_det_gpio
#define GPIO_OTG_ID_SET      mbb_binary_get_gpio(GPIO_SET)//g_otg_id_set_gpio
#define GPIO_DMDP_CONNECT    mbb_binary_get_gpio(GPIO_DMDP)//g_otg_dmdp_con_gpio
#else
...
#ifdef MBB_USB_BINARY_B
void  product_set_otg_feature(void);
void  product_set_otg_gpio(USB_UINT32* gpios,USB_UINT32 feature);
#endif
void  product_set_otg_gpio(unsigned long int* gpios,unsigned long int feature);
void  usb_otg_set_support_feature(unsigned long int feature);
#endif
