...
static netdev_tx_t eth_wrap_and_send(struct eth_dev *dev, struct sk_buff *skb)
{
    ...
    if (skb && skb->spe_own) 
    {
        ...
        if (!skb_queue_empty(&ctx->pkt_list)) 
        {
            ...
            {
                if((dev->ctx.cdc_type != GNET_CDC_TYPE_ECM) 
                    ...
                    struct net_device *net)
{
    ...
    if(unlikely(dev->is_suspend && !(dev->gadget->hs_rwakeup))) {
        dev_kfree_skb_any(skb);
                    ...
                    const USB_CHAR *buf, size_t size)
{
    ...
    if (size >= sizeof(cmd_str))
    {
        printk("gnet store cmd is oversize(more than %d)!\n", sizeof(cmd_str));
        return -ENOMEM;
    }
...
int eth_adp_for_spe_init(struct eth_dev* dev, USB_UINT32 cdc_type)
{
    ...
    if(NULL == spe_hook.port_alloc)
    {
        DBG_E(MBB_NCM, "spe_hook.port_alloc not registered\n");
        return ( - EINVAL);
    }
    ...
    else
    {
        ...
        if(NULL == spe_hook.port_enable)
        {
            DBG_E(MBB_NCM, "spe_hook.port_enable not registered\n");
            return ( - EINVAL);
        }
...
int gnet_spe_usb_plug(struct eth_dev* dev, unsigned int portno)
{
    ...
    {
        format = GNET_RNDIS_FORMAT;
    }else if(GNET_CDC_TYPE_ECM == dev->ctx.cdc_type)
    {
