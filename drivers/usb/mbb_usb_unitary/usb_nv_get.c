...
#if (defined BSP_CONFIG_BOARD_TELEMATIC) && (FEATURE_ON == MBB_FACTORY)
/* 读NV 50071失败后的默认值，烧片PID使用1C05 */
static nv_cust_pid_type gstDevPidDefault = { 0x00, CDROM_SCENE_PID, DOWNLOAD_SCENE_PID_E5 };
/* 读NV 50091失败后的默认值，烧片端口使用protocol 0x1b,0x13,0x12 */
static uint8_t gBurnPortDefault[DYNAMIC_PID_MAX_PORT_NUM] =
{
...
static USB_UINT8 g_FirstConfigPortDefault[DYNAMIC_PID_MAX_PORT_NUM] =     //setmode=0，first_config
{
...
static USB_UINT8 g_SecondConfigPortDefault[DYNAMIC_PID_MAX_PORT_NUM] =    //setmode=0，second_config
{
    USB_IF_PROTOCOL_NDISDATA,
    USB_IF_PROTOCOL_DIAG,    //DIAG
    USB_IF_PROTOCOL_PCUI,    //PCUI
    USB_IF_PROTOCOL_COMM_B,
    USB_IF_PROTOCOL_COMM_C,
...
#if(FEATURE_ON == MBB_TELE_WWAN)
static USB_UINT8 g_FirstConfigPortDebug[DYNAMIC_PID_MAX_PORT_NUM] =    //setmode=1，first_config
{
    USB_IF_PROTOCOL_NCM,
    USB_IF_PROTOCOL_DIAG,    //DIAG
    USB_IF_PROTOCOL_PCUI,    //PCUI
...
static USB_UINT8 g_SecondConfigPortDebug[DYNAMIC_PID_MAX_PORT_NUM] =    //setmode=1，second_config
{
    USB_IF_PROTOCOL_DIAG,    //DIAG
    USB_IF_PROTOCOL_PCUI,    //PCUI
    USB_IF_PROTOCOL_COMM_B,
...
static USB_UINT8 g2_FirstConfigPortDebug[DYNAMIC_PID_MAX_PORT_NUM] =    //setmode=2，first_config
{
    USB_IF_PROTOCOL_DIAG,    //DIAG
    USB_IF_PROTOCOL_PCUI,    //PCUI
    USB_IF_PROTOCOL_COMM_B,
...
static USB_UINT8 g2_SecondConfigPortDebug[DYNAMIC_PID_MAX_PORT_NUM] =    //setmode=2，second_config
{
    USB_IF_PROTOCOL_NDISDATA,
    USB_IF_PROTOCOL_DIAG,    //DIAG
    USB_IF_PROTOCOL_PCUI,    //PCUI
...
static USB_UINT8 g3_FirstConfigPortDebug[DYNAMIC_PID_MAX_PORT_NUM] =    //setmode=3，first_config
{
    USB_IF_PROTOCOL_RNDIS,
    USB_IF_PROTOCOL_DIAG,    //DIAG
    USB_IF_PROTOCOL_PCUI,    //PCUI
...
static USB_UINT8 g3_SecondConfigPortDebug[DYNAMIC_PID_MAX_PORT_NUM] =    //setmode=3，second_config
{
    USB_IF_PROTOCOL_DIAG,    //DIAG
    USB_IF_PROTOCOL_PCUI,    //PCUI
    USB_IF_PROTOCOL_COMM_B,
...
static USB_UINT8 g4_FirstConfigPortDebug[DYNAMIC_PID_MAX_PORT_NUM] =    //setmode=4，first_config
{
    USB_IF_PROTOCOL_NDISDATA,
    USB_IF_PROTOCOL_PCUI,    //PCUI
};
static USB_UINT8 g5_FirstConfigPortDebug[DYNAMIC_PID_MAX_PORT_NUM] =    //setmode=5，first_config
{
    USB_IF_PROTOCOL_NDISDATA,
    USB_IF_PROTOCOL_DIAG,    //DIAG
    USB_IF_PROTOCOL_PCUI,    //PCUI
...
static USB_UINT8 g_NullConfigPortDebug[DYNAMIC_PID_MAX_PORT_NUM] =
{
    USB_IF_PROTOCOL_VOID
};
static USB_UINT8 g_FirstConfigPortBurnDefault[DYNAMIC_PID_MAX_PORT_NUM] =    //烧片模式下的默认端口配置
{
    USB_IF_PROTOCOL_DIAG,    //DIAG
    USB_IF_PROTOCOL_PCUI,    //PCUI
    USB_IF_PROTOCOL_COMM_B,
...
{
    {
        PRODUCT_ID_P711SOLUTION,
        {
            ...
            "Huawei Mobile Connect - Serial C",
...
USB_UINT get_shell_nv_value(USB_VOID)
{
    return g_usb_nv_info.ports_lock;
}

USB_UINT get_shell_oeminfo_value(USB_VOID)
{
    return g_usb_nv_info.oeminfo_ports_lock;
}

USB_VOID set_shell_nv_value(USB_UINT lock)
{
    if (lock > 0)
    {
        g_usb_nv_info.ports_lock = true;
    }
    else
    {
        g_usb_nv_info.ports_lock = false;
    }

    return;
}

USB_VOID set_shell_oeminfo_value(USB_UINT lock)
{
    if (lock > 0)
    {
        g_usb_nv_info.oeminfo_ports_lock = true;
    }
    else
    {
        g_usb_nv_info.oeminfo_ports_lock = false;
    }
    return;
}
#endif
...
Parameters    :  无
Return        :
Description   :  wwan拨号动态适配,有NCM口时设置为bypass模式，否则设置为非bypass模式
*****************************************************************/
#define WWAN_BYPASS   0xFD
#define WWAN_NOBYPASS 0x02
#define WWAN_FLAG     0x01
#define WWAN_NOFLAG   0xFE

USB_INT usb_dynamic_wwan(USB_UINT8 * port_name)
{
    USB_INT ret = MBB_USB_ERROR;
    char flags;
    flags = wwan_read_nv();

    if (USB_IF_PROTOCOL_NCM == port_name[0])
    {
        if (flags & WWAN_NOBYPASS)
        {
            flags = flags & WWAN_BYPASS;
            flags = flags | WWAN_FLAG;
            ret = wwan_write_nv(flags);
            if (MBB_USB_OK != ret)
            {
    ...
    else
    {
        if (!(flags & WWAN_NOBYPASS))
        {
            flags = flags | WWAN_NOBYPASS;
            flags = flags & WWAN_NOFLAG;
            ret = wwan_write_nv(flags);
            if (MBB_USB_OK != ret)
            {
                return MBB_USB_ERROR;
...
USB_INT  usb_nv_get(USB_VOID)
{
    ...
    {
        DBG_W(MBB_USB_NV , "NV %d use default, ret=%d nv_status=%d \r\n", USB_NV_SERIAL_NUM_ID, ret,
    ...
    if ( MBB_USB_OK != ret )
    {
        DBG_W(MBB_USB_NV , "NV %d use default, ret=%d \r\n",
    ...
    if (MBB_USB_OK != ret)
    {
        DBG_W(MBB_USB_NV, "NV %d use default!\r\n", USB_NV_ITEM_AT_SHELL_OPEN);
        g_usb_nv_info.shell_lock = 0;
    }

     /*USB_NV_HUAWEI_SHELL_CMD NV 50523   无nv_status */
    memset(&(g_usb_nv_info.ports_lock), 0, sizeof(g_usb_nv_info.ports_lock));
    ret = usb_read_nv(USB_NV_HUAWEI_SHELL_CMD, (USB_PVOID) & (g_usb_nv_info.ports_lock),
                      sizeof(USB_UINT32));
    if (MBB_USB_OK != ret)
    {
        DBG_W(MBB_USB_NV, "NV %d use default!\r\n", USB_NV_HUAWEI_SHELL_CMD);/*lint !e506*/
        g_usb_nv_info.ports_lock = 0;
    }
    else
    {
        if (g_usb_nv_info.ports_lock > 0)
        {
            g_usb_nv_info.ports_lock = true;
        }
    }
    memset(&(g_usb_nv_info.oeminfo_ports_lock), 0, sizeof(g_usb_nv_info.oeminfo_ports_lock));
#ifdef BSP_CONFIG_BOARD_TELEMATIC
    ret = kernel_get_shlock_flag(&(g_usb_nv_info.oeminfo_ports_lock));
    if (MBB_USB_OK != ret)
    {
        DBG_W(MBB_USB_NV, "NV %d oeminfo use default!\r\n", USB_NV_HUAWEI_SHELL_CMD);
        g_usb_nv_info.oeminfo_ports_lock = 0;
    }
#endif
        ...
        || (MBB_NV_DISABLED == g_usb_nv_info.dev_profile_info.nv_status))
    {
        ...
        memcpy(g_usb_nv_info.dev_profile_info.rewind_port_style, gBurnPortDefault,
    ...
    else
    {
        ...
        {
            DBG_W(MBB_USB_NV , "please enter the NV value! use default\n");
            memcpy(g_usb_nv_info.dev_profile_info.rewind_port_style, gRewindPortDefault,
                   DYNAMIC_PID_MAX_PORT_NUM);
        }
    }
...
#if(FEATURE_ON == MBB_TELE_HARMANUSB)
    if ((MBB_USB_OK != ret)
        || (MBB_NV_DISABLED == g_usb_nv_info.port_dynamic_name.nv_status))
    {
        DBG_E(MBB_USB_NV ,"NV = %d, ret=%d\n", USB_NV_DYNAMIC_INFO_NAME,ret);
        strncpy(g_usb_nv_info.port_dynamic_name.huawei_manufacturer_dynamic_name, DAG_MANUFACTURER_STRING,
            (DYNAMIC_INFO_NAME_CHAR_NUM - 1));
        strncpy(g_usb_nv_info.port_dynamic_name.huawei_product_dynamic_name, DAG_PRODUCT_STRING_KOM1,
            (DYNAMIC_INFO_NAME_CHAR_NUM - 1));
        strncpy(g_usb_nv_info.port_dynamic_name.huawei_configuration_dynamic_name, "0123456789ABCDEF",
            (DYNAMIC_INFO_NAME_CHAR_NUM - 1));
    }
#else
    if ((MBB_USB_OK != ret)
    ...
    {
        DBG_W(MBB_USB_NV , "Now not support  NV %d! for  DTS2014021309015 \r\n",
            USB_NV_PROT_OFFSET_ID);
        DBG_W(MBB_USB_NV , "NV_%d invalid !protocol_base = %d!!\r\n",
...
#ifdef BSP_CONFIG_BOARD_TELEMATIC
        g_usb_nv_info.boot_pid_info.dynamic_boot_pid = MODULE_BALONG_PID;
#else
...
#ifdef BSP_CONFIG_BOARD_TELEMATIC
        g_usb_nv_info.dynamic_pid_info.dynamic_normal_pid = TELEMATIC_BALONG_PID;
#else
    ...
    {
        DBG_W(MBB_USB_NV, "Winblue profile nv val: %s, %u, %u, %u, %u, %u, %u\n",
    ...
    {
        DBG_W(MBB_USB_NV, "NV %d use default, ret=%d nv_status=%ld\r\n",
        ...
        || (MBB_NV_DISABLED == g_usb_nv_info.multi_config_port_info.nv_status))
    {
        ...
        memcpy(g_usb_nv_info.multi_config_port_info.first_config_port,
            g_FirstConfigPortDefault,DYNAMIC_PID_MAX_PORT_NUM);
        memcpy(g_usb_nv_info.multi_config_port_info.second_config_port,
            g_SecondConfigPortDefault,DYNAMIC_PID_MAX_PORT_NUM);
    }
    else/*NV get success*/
    {
    ...
    if ((MBB_USB_OK != ret)
        ...
        || ( SETMODE_FLAG_MAX <= (g_usb_nv_info.debug_mode.setmode_flag)))
    {
...
#ifdef BSP_CONFIG_BOARD_TELEMATIC
#if (FEATURE_ON == MBB_TELE_HARMANUSB)
        g_usb_nv_info.debug_mode.setmode_flag = ECM_DIAG_PCUI_DEBUG;
#else
        g_usb_nv_info.debug_mode.setmode_flag = USB_NOPORT_MODE;
#endif
#else
...
#if(FEATURE_ON == MBB_FACTORY)     //烧片默认端口配置
    else if((ECM_LINUX_MODE_DEBUG == g_usb_nv_info.debug_mode.setmode_flag) || (ACM_PCUI_MODE_DEBUG == g_usb_nv_info.debug_mode.setmode_flag))
    {
        memcpy(g_usb_nv_info.multi_config_port_info.first_config_port, g_FirstConfigPortBurnDefault,
               DYNAMIC_PID_MAX_PORT_NUM);
        memcpy(g_usb_nv_info.multi_config_port_info.second_config_port, g_FirstConfigPortBurnDefault,
               DYNAMIC_PID_MAX_PORT_NUM);
    }
#else
    else if (ECM_LINUX_MODE_DEBUG == g_usb_nv_info.debug_mode.setmode_flag)
    {
        memcpy(g_usb_nv_info.multi_config_port_info.first_config_port, g2_FirstConfigPortDebug,
               DYNAMIC_PID_MAX_PORT_NUM);
        memcpy(g_usb_nv_info.multi_config_port_info.second_config_port, g2_SecondConfigPortDebug,
               DYNAMIC_PID_MAX_PORT_NUM);
    }
    else if (ACM_PCUI_MODE_DEBUG == g_usb_nv_info.debug_mode.setmode_flag)
    {
        memcpy(g_usb_nv_info.multi_config_port_info.first_config_port, g3_FirstConfigPortDebug,
               DYNAMIC_PID_MAX_PORT_NUM);
        memcpy(g_usb_nv_info.multi_config_port_info.second_config_port, g3_SecondConfigPortDebug,
               DYNAMIC_PID_MAX_PORT_NUM);
    }
#endif    /*MBB_FACTORY*/
    else if (ECM_DIAG_PCUI_DEBUG == g_usb_nv_info.debug_mode.setmode_flag)
    {
        memcpy(g_usb_nv_info.multi_config_port_info.first_config_port, g4_FirstConfigPortDebug,
               DYNAMIC_PID_MAX_PORT_NUM);
        memcpy(g_usb_nv_info.multi_config_port_info.second_config_port, g_NullConfigPortDebug,
               DYNAMIC_PID_MAX_PORT_NUM);
    }
    else if (ECM_DIAG_ADB_DEBUG == g_usb_nv_info.debug_mode.setmode_flag)
    {
        memcpy(g_usb_nv_info.multi_config_port_info.first_config_port, g5_FirstConfigPortDebug,
               DYNAMIC_PID_MAX_PORT_NUM);
        memcpy(g_usb_nv_info.multi_config_port_info.second_config_port, g_NullConfigPortDebug,
               DYNAMIC_PID_MAX_PORT_NUM);
    }
    else if (USB_NOPORT_MODE == g_usb_nv_info.debug_mode.setmode_flag)
    {
        memcpy(g_usb_nv_info.multi_config_port_info.first_config_port, g_NullConfigPortDebug,
               DYNAMIC_PID_MAX_PORT_NUM);
        memcpy(g_usb_nv_info.multi_config_port_info.second_config_port, g_NullConfigPortDebug,
               DYNAMIC_PID_MAX_PORT_NUM);
    }
#if(FEATURE_ON == MBB_TELE_WWAN)
    ret = usb_dynamic_wwan(g_usb_nv_info.multi_config_port_info.first_config_port);
    if (MBB_USB_OK != ret)
    {
...
#if(FEATURE_OFF == MBB_TELE_HARMANUSB)
    if ( index >= 0 )
    {
        strncpy(g_usb_nv_info.port_dynamic_name.huawei_manufacturer_dynamic_name,
...
USB_VOID usb_log_level_set(USB_INT module, USB_INT level)
{
    ...
    usb_nv_info->usb_log_ctl.nv_status = 1;
    if (!usb_write_nv(USB_NV_USB_PRIVATE_INFO, &(usb_nv_info->usb_log_ctl), sizeof(usb_private_nv_info)))
    {
