...
#define PRINT_DBG(type, level, fmt, args...) \
    ...
    do { \
        ...
        if( U_INFO == (level))\
        {\
            mlog_print("USB", USB_MLOG_INFO, "%s:" fmt,__FUNCTION__,## args);\
        }\
        else if (U_WARN== (level))\
        {\
            mlog_print("USB", USB_MLOG_WARN, "%s:" fmt,__FUNCTION__,## args);\
        }\
        else if(U_ERROR== (level))\
        {\
            mlog_print("USB", USB_MLOG_ERROR, "%s:" fmt,__FUNCTION__,## args);\
        }\
        else if(U_TRACE== (level))\
        {\
            mlog_print("USB", USB_MLOG_NONE, "%s:" fmt,__FUNCTION__,## args);\
        }\
    } while (0)
/*lint +e40 +e58 */
#endif
