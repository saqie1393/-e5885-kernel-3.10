...
#include <mdrv_chg.h>
#define DELTA_TIME_FOR_COUL_UPDATE        1000
#endif
...
static USB_VOID mbb_usb_insert_process(USB_VOID)
{
    USB_ULONG flags = 0;
#if(FEATURE_ON == MBB_CHG_BQ27510)
    USB_ULONG coul_update_wait_timeout = 0;
#endif

#if(FEATURE_ON == MBB_CHG_BQ27510)
    coul_update_wait_timeout = jiffies + msecs_to_jiffies(DELTA_TIME_FOR_COUL_UPDATE);

    /*check whether the coul firmware update is completed*/
    bq27510_sync_coulupdate_port_control();

    if(time_after(jiffies, coul_update_wait_timeout))
    {
        mlog_print("USB", USB_MLOG_INFO, "the time wait for coul update is %d ms\n", jiffies_to_msecs(jiffies - coul_update_wait_timeout) + DELTA_TIME_FOR_COUL_UPDATE);
    }
#endif
