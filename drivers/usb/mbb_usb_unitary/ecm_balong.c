...
/*****************************************************************************
* 函 数 名  : ecm_get_ctx_ex
*
* 功能描述  : 根据设备ID获取上下文
*
* 输入参数  : u32 config     : 对应config 值
              u32 dev_id    :设备ID
* 输出参数  : 无
*
* 返 回 值  : 设备ID上下文
...
*
*****************************************************************************/
static inline ecm_app_ctx_t *ecm_get_ctx_ex(u32 config, u32 dev_id)
{
    //DBG_I(MBB_ECM, "[%d]usb_get_curr_cfg_idx= %d\n",__LINE__,usb_get_curr_cfg_idx());
...
s32 ecm_vendor_at_resp_process(ecm_app_ctx_t *ctx, void *response, u32 size)
{
    ...
    if (ECM_DBG_MASK_CHECK(ECM_DBG_MASK_AT))
    {
        ...
        DBG_I(MBB_ECM, "L[%d]: size=%u, data=\"%s\"\n", __LINE__, size, gecmAtRspBuf);
    }
/*删除实际发送动作仅给协议返回处理完成    */
...
* 其它      : NA
*****************************************************************************/
void ecm_vendor_close_ex(ECM_DEV_TYPE_E dev_type, u32 dev_id)
{
    ecm_get_ctx_ex(USB_NUM_1, dev_id)->chnl_open[dev_type] = false;
    ecm_get_ctx_ex(USB_NUM_2, dev_id)->chnl_open[dev_type] = false;
    ecm_get_ctx_ex(USB_NUM_3, dev_id)->chnl_open[dev_type] = false;
}
extern void ncm_vendor_close_ex(NCM_DEV_TYPE_E dev_type, u32 dev_id);
...
{    
    ...
    s32 ret = 0; 
    DBG_T(MBB_ECM, "line %d:enter.dev_id(%u),cmd(%x)\n", __LINE__,dev_id,cmd);
    if(!ecm_vendor_inited)
    {
        DBG_E(MBB_ECM, "%s line %d:module not init.\n", __FUNCTION__,__LINE__);
        return -ENODEV;
