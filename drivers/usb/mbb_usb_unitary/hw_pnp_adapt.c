...
huawei_interface_info g_huawei_interface_vector[MAX_INTERFACE_VECTOR_NUM] =
{
    ...
    {USB_IF_PROTOCOL_3G_PCUI,          "pcui"},     //3G_PCUI
    {USB_IF_PROTOCOL_3G_DIAG,          "3g_diag"},  //3g_diag
#endif
    /*M2M protocol*/
    ...
    {USB_IF_PROTOCOL_NDISDATA,    "ecm"},     //balong 用于模块形态标准ECM 端口
...
/*****************************************************************
Parameters    :  pnp_usb_port_authentication
...
USB_INT pnp_usb_port_authentication (struct android_usb_function* usb_func)
{
    ...
    for (index = 0; index < DYNAMIC_PID_MAX_PORT_NUM; index++)
    {
        ...
        if (0 == strncmp(usb_func->name,g_pnp_port_usable[index], usable_name_len))
        {
            ret = MBB_USB_YES;
            break;
        }
...
USB_INT pnp_get_dload_flag(USB_VOID)
{
    ...
    {
        DBG_I(MBB_PNP, "PORT_NV_RES MODE\n");
#ifdef MBB_USB_UNITARY_Q
...
#endif
    {
        DBG_I(MBB_PNP, "The power off status\n");
...
Parameters    :  void
Return        :
Description   :  查询是否解密，外部直接调用
*****************************************************************/
#ifdef USB_SECURITY
int usb_port_security(void)
{
#ifdef MBB_USB_UNITARY_Q
    int ret = -1;
    huawei_smem_info* smem_data = NULL;
...
#else
    int lock_flag;
    lock_flag = get_shell_lock();
    DBG_I(MBB_PNP, "shell lock is %d.\n",lock_flag);
    if (SHELL_OPEN_FLAG == lock_flag)
    {
        return 0;
    }
    return -1;
...
USB_INT usb_speed_work_mode(USB_VOID)
{
    ...
    {
        DBG_I(MBB_CHARGER, "CHG_CURRENT_SS\n");
        return CHG_CURRENT_SS;
    }
    else if (USB_SPEED_UNKNOWN == usb_speed )
    {
        DBG_I(MBB_CHARGER, "CHG_CURRENT_NO\n");
        return CHG_CURRENT_NO;
    }
    else
    {
        DBG_I(MBB_CHARGER, "CHG_CURRENT_HS\n");
...
USB_VOID gatway_data_disconnect (struct work_struct* w)
{
    ...
    DBG_I(MBB_PNP, "gatway_data_disconnect %d \n", UsbCtx->gatway_work_mode);
}
#endif
...
USB_VOID pnp_adp_dump(USB_VOID)
{
    ...
    DBG_T(MBB_DEBUG, "|hs_rwakeup--------- = %d\n", gadget->hs_rwakeup);
    DBG_T(MBB_DEBUG, "|hs_rwakeup_cap----- = %d\n", gadget->hs_rwakeup_cap);
