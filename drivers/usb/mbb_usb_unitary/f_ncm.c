...
#define MBB_SETMODE_NULL_POINTER      6
#endif
...
USB_BOOL ncm_get_bypass_state(USB_VOID)
{
    ...
    DBG_I(MBB_PNP, "ncm bypass state = %u, is force=%u\n", bRet, g_force_bypass_mode);
    return bRet;
}
...
USB_VOID ncm_switch_mbim_mode(struct work_struct* w)
{
    ...
    DBG_I(MBB_NCM, "ncm_switch_mbim_mode: mode=%u, gIsMbimMode=%u",
...
USB_VOID ncm_set_mbim_mode(USB_INT switch_idex)
{
    ...
    USB_INT ret = MBB_USB_ERROR;
    mbb_usb_nv_info_st* usb_nv_info = usb_nv_get_ctx();
    if((usb_nv_info->debug_mode.setmode_flag != switch_idex)
       || (MBB_NV_ENABLE != usb_nv_info->debug_mode.nv_status))
    {
        usb_nv_info->debug_mode.nv_status = MBB_NV_ENABLE;
        usb_nv_info->debug_mode.setmode_flag = switch_idex;
        ret = usb_write_nv(USB_NV_USB_DEBUG_MODE_FLAGE, &usb_nv_info->debug_mode, 
                               sizeof(nv_huawei_debug_mode_flag));
        if(MBB_USB_OK != ret)
...
USB_UINT ncm_get_mbim_mode(USB_VOID)
{
    ...
    {
        pr_err("%s mbb_usb_nv_info_st *nv_st is NULL",__func__);
        return  MBB_SETMODE_NULL_POINTER;
    }
...
USB_VOID ncm_notify_work (struct work_struct* w)
{
    ...
    {
        DBG_I(MBB_NCM, "send notify %u in work\n", ncm->queued_notify_state);
    }
...
static USB_INT ncm_setup(struct usb_function *f, const struct usb_ctrlrequest *ctrl)
{
    ...
    */
        ...
        | USB_CDC_GET_CRC_MODE):
    {
        ...
        value = 2;
        DBG_I(MBB_NCM, "Host asked CRC MODE, sending %d\n", is_crc);
        break;
    }
      ...
      | USB_CDC_NTFY_HUAWEI_GET_NCM_RESERVE_BYTES):
    {
        ...
        DBG_I(MBB_NCM, "Host asked reserved header room len, sending %ld\n",
    ...
    default:
invalid:
        DBG_E(MBB_NCM, "control req%02x.%02x v%04x i%04x l%d\n",
...
static USB_INT ncm_set_alt(struct usb_function *f, USB_UINT intf, USB_UINT alt)
{
    ...
    DBG_I(MBB_NCM, "ncm_set_alt: intf=%u, alt=%u, ctrl_id=%u, data_id=%u\n",
        ...
        || ((intf == ncm->ctrl_id) && (false == ncm->is_single_interface)))
    {
        ...
        if( (0 != ncm->is_single_interface) && (0 == alt))
        {
                    ...
                    struct sk_buff *skb)
{
    ...
    tx_ctx_t *ctx;
    USB_ULONG flags;
    USB_UINT32 pkt_len_waterline = 0;
    ...
    pkt_len_waterline =  dev->ctx.pkt_len_waterline;

    if ( port->fixed_in_len >= GNET_NCM_ROOM_FOR_NTB_INMAXSIZE)
    {
...
#ifdef BSP_CONFIG_BOARD_TELEMATIC
    mbb_usb_nv_info_st* usb_nv_info = usb_nv_get_ctx();
    USB_UINT8 first_config_port = USB_IF_PROTOCOL_NCM;
    USB_UINT8 second_config_port = USB_IF_PROTOCOL_NCM;
    if(NULL != usb_nv_info)
    {
        first_config_port = usb_nv_info->multi_config_port_info.first_config_port[0];
        second_config_port = usb_nv_info->multi_config_port_info.second_config_port[0];
    }
#endif
...
#endif
#ifdef BSP_CONFIG_BOARD_TELEMATIC
    if(USB_IF_PROTOCOL_NCM == first_config_port || USB_IF_PROTOCOL_NCM == second_config_port)
    {
        DBG_T(MBB_NCM, "network card is NCM\n");
        cdc_type = GNET_CDC_TYPE_NCM ;
    }
    else if(USB_IF_PROTOCOL_RNDIS == first_config_port || USB_IF_PROTOCOL_RNDIS == second_config_port)
    {
        DBG_T(MBB_NCM, "network card is RNDIS\n");
        cdc_type = GNET_CDC_TYPE_RNDIS;
    }
    else if(USB_IF_PROTOCOL_NDISDATA == first_config_port || USB_IF_PROTOCOL_NDISDATA == second_config_port)
    {
        DBG_T(MBB_NCM, "network card is ECM\n");
        cdc_type= GNET_CDC_TYPE_ECM;
    }else
    {
         DBG_T(MBB_NCM, "no network card in port config USE NCM\n");
    }
#endif
