...
static USB_VOID otg_cradle_insert(struct otg_dev_det* otg)
{
    ...
    USB_INT cradle_type = 0;

    DBG_I(MBB_OTG_CHARGER,"cradle insert!\n");
    cradle_type = chg_get_cradletype_from_usbid_voltage();
...
#if defined(CONFIG_USB_USBNET)
    mbb_usbnet_set_cradle_type(cradle_type);
    mbb_usbnet_set_fty_cradle_state(USB_CRADLE_PLUGIN_EVENT);
#endif
...
#if defined(CONFIG_USB_USBNET)
    mbb_usbnet_set_cradle_type(CRADLE_TYPE_INVALID);
    mbb_usbnet_set_fty_cradle_state(USB_CRADLE_UNPLUG_EVENT);
#endif
...
static irqreturn_t otg_dev_det_irq(USB_INT irq, USB_PVOID dev_id)
{
    ...
    if ((NULL == otg_device ) || (otg != otg_device))
    {
        DBG_E(MBB_OTG_CHARGER , "id %p may be not our otg %p\n", otg, otg_device);
        return IRQ_NONE;
    }
...
USB_VOID  usb_otg_device_detect_init(USB_VOID)
{
    ...
    if (USB_OTG_FEATURE_NONE == otg_device->otg_feature)
    {
        DBG_E(MBB_OTG_CHARGER, "product not support OTG \n");
        kfree(otg);
        otg = NULL;
        return ;
    }
