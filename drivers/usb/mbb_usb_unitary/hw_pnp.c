...
static ssize_t iManufacturer_show(struct device *pdev, struct device_attribute *attr, char *buf)
{
    int ret = MBB_USB_ERROR;
    mbb_usb_nv_info_st* usb_nv_info = usb_nv_get_ctx();
    ret = usb_read_nv(USB_NV_DYNAMIC_INFO_NAME,(USB_PVOID) &(usb_nv_info->port_dynamic_name), sizeof(nv_huawei_dynamic_info_name));

    if (0 != ret)
    {
        DBG_I(MBB_PNP, "read nv fail,iManufacturer use default value!\n");
    }

    memcpy(manufacturer_string, usb_nv_info->port_dynamic_name.huawei_manufacturer_dynamic_name, DYNAMIC_INFO_NAME_CHAR_NUM);
    pr_err("%s\n", __func__);
    return snprintf(buf, PAGE_SIZE, "%s\n", manufacturer_string);
}

static ssize_t iManufacturer_store(struct device *pdev, struct device_attribute *attr, const char *buff, size_t size)
{
    int i = 0;
    int ret = MBB_USB_ERROR;
    char tmp_manufacturer_string[MAX_STRING_DESC_NUM] = {0};
    mbb_usb_nv_info_st* usb_nv_info = usb_nv_get_ctx();

    if (sizeof(tmp_manufacturer_string) - 1 < size)
    {
        return MBB_USB_ERROR;
    }

    for(i = 0; (*(buff + i) != 0x0a) && (i < size); i++)
    {
         tmp_manufacturer_string[i] = *(buff + i);
    }
    pr_err("tmp_manufacturer_string=%s\n", tmp_manufacturer_string);

    if((0 == strcmp(tmp_manufacturer_string, HUAWEI_MANUFACTURER_STRING)) || ((0 == strcmp(tmp_manufacturer_string, DAG_MANUFACTURER_STRING))))
    {
        memset(manufacturer_string, 0, sizeof(manufacturer_string));
        memcpy(manufacturer_string, tmp_manufacturer_string, sizeof(tmp_manufacturer_string));
    }
    else
    {
        pr_err("string error\n");
    ...
    if ((0 != strcmp(tmp_manufacturer_string, (char *)(usb_nv_info->port_dynamic_name.huawei_manufacturer_dynamic_name)))
        || (MBB_NV_ENABLE != usb_nv_info->port_dynamic_name.nv_status))
    {
        memset(usb_nv_info->port_dynamic_name.huawei_manufacturer_dynamic_name, 0, DYNAMIC_INFO_NAME_CHAR_NUM);

        usb_nv_info->port_dynamic_name.nv_status = MBB_NV_ENABLE;

        memcpy(usb_nv_info->port_dynamic_name.huawei_manufacturer_dynamic_name, manufacturer_string, DYNAMIC_INFO_NAME_CHAR_NUM);
        ret = usb_write_nv(USB_NV_DYNAMIC_INFO_NAME, &(usb_nv_info->port_dynamic_name), sizeof(nv_huawei_dynamic_info_name));
        if (0 != ret)
        {
            return MBB_USB_ERROR;
        }
...
static ssize_t iProduct_show(struct device *pdev, struct device_attribute *attr, char *buf)
{
    int ret = MBB_USB_ERROR;
    mbb_usb_nv_info_st* usb_nv_info = usb_nv_get_ctx();
    ret = usb_read_nv(USB_NV_DYNAMIC_INFO_NAME,(USB_PVOID) &(usb_nv_info->port_dynamic_name), sizeof(nv_huawei_dynamic_info_name));
    if (0 != ret)
    {
        DBG_I(MBB_PNP, "read nv fail,iProduct use default value!\n");
    }
    memcpy(product_string, usb_nv_info->port_dynamic_name.huawei_product_dynamic_name, DYNAMIC_INFO_NAME_CHAR_NUM);
    pr_err("%s: product_string=%s\n", __func__, product_string);
    return snprintf(buf, PAGE_SIZE, "%s\n", product_string);
}

static ssize_t iProduct_store(struct device *pdev, struct device_attribute *attr, const char *buff, size_t size)
{
    int i = 0, ret = MBB_USB_ERROR;
    char tmp_product_string[MAX_STRING_DESC_NUM] = {0};
    mbb_usb_nv_info_st* usb_nv_info = usb_nv_get_ctx();

    if (sizeof(tmp_product_string) - 1 < size)
    {
        return MBB_USB_ERROR;
    }
    ...
    for(i = 0; (*(buff + i) != 0x0a) && (i < size); i++)
    {
        tmp_product_string[i] = *(buff + i);
    }
    pr_err("tmp_product_string=%s\n", tmp_product_string);
    if((0 == strcmp(tmp_product_string, HUAWEI_PRODUCT_STRING)) || ((0 == strcmp(tmp_product_string, DAG_PRODUCT_STRING_KOM1_DEACT))) || ((0 == strcmp(tmp_product_string, DAG_PRODUCT_STRING_KOM1))))
    {
         memset(product_string, 0, sizeof(product_string));
         memcpy(product_string, tmp_product_string, sizeof(tmp_product_string));
    }
    else
    {
    ...
    if ((0 != strcmp(tmp_product_string, (char *)(usb_nv_info->port_dynamic_name.huawei_product_dynamic_name)))
        || (MBB_NV_ENABLE != usb_nv_info->port_dynamic_name.nv_status))
    {
        memset(usb_nv_info->port_dynamic_name.huawei_product_dynamic_name, 0, DYNAMIC_INFO_NAME_CHAR_NUM);
        usb_nv_info->port_dynamic_name.nv_status = MBB_NV_ENABLE;
        memcpy(usb_nv_info->port_dynamic_name.huawei_product_dynamic_name, product_string, DYNAMIC_INFO_NAME_CHAR_NUM);
        ret = usb_write_nv(USB_NV_DYNAMIC_INFO_NAME, &usb_nv_info->port_dynamic_name, sizeof(nv_huawei_dynamic_info_name));
    }

    return size;
}
#endif/*MBB_TELE_HARMANUSB*/
                                ...
                                char* buf)
{
    ...
    memset(mode_type , 0 , sizeof(mode_type));
...
#if(FEATURE_ON == MBB_TELE_HARMANUSB)
static DEVICE_ATTR(iManufacturer, S_IRUGO | S_IWUSR, iManufacturer_show, iManufacturer_store);
static DEVICE_ATTR(iProduct, S_IRUGO | S_IWUSR, iProduct_show, iProduct_store);
#endif
...
#ifdef BSP_CONFIG_BOARD_TELEMATIC
    USB_UINT8 aucBuf[TAF_PH_IMEI_LEN] = {0};
    int n = 0;
#endif
...
#ifdef BSP_CONFIG_BOARD_TELEMATIC
    ret = usb_read_nv(en_NV_Item_IMEI,(USB_PVOID)aucBuf, TAF_PH_IMEI_LEN);
    if (MBB_USB_OK != ret)
    {
        memset(aucBuf, 0, TAF_PH_IMEI_LEN);
        pr_err("read en_NV_Item_IMEI error\n");
    }
    /*将BCD码转换为ASCII吗*/
    for (n = 0; n < TAF_PH_IMEI_LEN; n++)
    {
        if(( aucBuf[n] >= 0) && (aucBuf[n] <= 9)) /* the number is 0-9 */
        {
            aucBuf[n] = aucBuf[n] + 0x30;   /* 低位 */
        }
    }
    strncpy(serial_string, aucBuf, TAF_PH_IMEI_LEN - 1);
#else
...
USB_INT pnp_hilink_mode_dload( USB_VOID )
{
        ...
        && (product_port_mode == g_pnp_info.rewind_param.bGreenMode))
    {
        g_pnp_info.current_port_style = download_port_style;
        g_pnp_info.portModeIndex = PCUI_DIAG_MODE;
        DBG_I(MBB_PNP, "rndis dload mode\n");
...
USB_INT pnp_huawei_vendor_setup(struct usb_gadget* gadget, const struct usb_ctrlrequest* ctrl)
{
    ...
    switch (ctrl->bRequest)
    {
        ...
        case USB_REQ_GET_MSOS_CUSTOM_MAC://mac os service installed
            ...
            {
                DBG_I(MBB_PNP, "40 A1 recive(MAC)\n");
...
{
#ifdef BSP_CONFIG_BOARD_TELEMATIC
    if (factory_port_style == g_pnp_info.current_port_style
