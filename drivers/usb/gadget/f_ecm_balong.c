...
static void ecm_do_notify(struct f_ecm *ecm)
{
	...
	if (unlikely(cdev->gadget->is_suspend)) {
		if(unlikely((USB_SPEED_SUPER == cdev->gadget->speed) 
			&& !ecm->port.func.func_rwakeup)){
			DBG(cdev, "Superspeed remote wakeup notify disabled\n");
			ecm->notify_state = ECM_NOTIFY_NONE;
			return;
		}
		...
		if (usb_gadget_wakeup(cdev->gadget)) {
			...
			DBG(cdev, "Remote wakeup notify fail\n");

			ecm->notify_state = ECM_NOTIFY_NONE;
			return;
		}
		ecm->port.func.func_is_suspend = 0;
	}
...
static int ecm_set_alt(struct usb_function *f, unsigned intf, unsigned alt)
{
    ...
    if (intf == ecm->ctrl_id && alt == 0) {
        ...
        bsp_usb_set_enum_stat(ecm->data_id, 1, 0);
...
static void ecm_disable(struct usb_function *f)
{
    ...
    bsp_usb_set_enum_stat(ecm->data_id, 0, 0);
}
...
ecm_unbind(struct usb_configuration *c, struct usb_function *f)
{
	...
	bsp_usb_del_setup_dev((unsigned)ecm->data_id, 0);
...
static int	ecm_func_suspend(struct usb_function *f, u8 suspend_opt)
{
		...
		struct eth_dev *dev)
{
	...
	ecm->port.func.func_rwakeup_cap = 1;
#else
	ecm->port.func.func_rwakeup_cap = 0;
#endif
