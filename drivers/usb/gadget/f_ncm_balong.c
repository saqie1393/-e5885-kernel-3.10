...
static void ncm_do_notify(struct f_ncm *ncm)
{
    ...
    if (unlikely(cdev->gadget->is_suspend)){
        if(unlikely((USB_SPEED_SUPER == cdev->gadget->speed) 
            && !ncm->port.func.func_rwakeup)){
            
            GNET_ERR(("Supperspeed remote wakeup disabled\n"));
            ncm->notify_state = NCM_NOTIFY_NONE;
            ncm->notify_req = req;
            spin_lock(&ncm->lock);
            return;
        ...
        if (usb_gadget_wakeup(cdev->gadget)) {
        ...
        ncm->port.func.func_is_suspend= 0;
    }
...
{
    /*Add this to avoid notify speed cause pc usb netcark connect.*/
    if(bsp_usb_is_ncm_bypass_mode()){
        return;
    }
    /*
...
*/
      ...
      | USB_CDC_NTFY_HUAWEI_GET_NCM_RESERVE_BYTES):
    {
        ...
        DBG(cdev, "Host asked reserved header room len, sending %d\n",
...
#endif
        bsp_usb_set_enum_stat(ncm->data_id, 1, 0);
                    ...
                    struct sk_buff *skb)
{
    ...
    if(skb) /* caller is xmit */
    {
        ...
        else
        {
            ...
            list_del(&req->list);
            spin_unlock_irqrestore(&dev->tx_req_lock, flags);
            GNET_INFO(("%s [xmit]:waterline reach,ctx=%pK\n",__FUNCTION__,ctx));
        }
    }
...
static void ncm_disable(struct usb_function *f)
{
    ...
    bsp_usb_set_enum_stat(ncm->data_id, 0, 0);
...
ncm_bind(struct usb_configuration *c, struct usb_function *f)
{
    ...
    f->func_rwakeup_cap = 1;
#else
    f->func_rwakeup_cap = 0;
#endif
...
ncm_unbind(struct usb_configuration *c, struct usb_function *f)
{
    ...
    bsp_usb_del_setup_dev((unsigned)ncm->data_id, 0);
    ...
    if(ncm->notify_req && ncm->notify_req->buf){
        kfree(ncm->notify_req->buf);
        usb_ep_free_request(ncm->notify, ncm->notify_req);
    }else{
        printk("NCM notify_req empty!\n");
        WARN_ON(1);
    }
...
static int	ncm_get_status(struct usb_function *f)
{
    int status = 0;
...
static int	ncm_func_suspend(struct usb_function *f, u8 suspend_opt)
{
        ...
        struct eth_dev *dev, int ntb16)
{
    ...
    ncm->port.func.get_status = ncm_get_status;
    ncm->port.func.func_suspend = ncm_func_suspend;
