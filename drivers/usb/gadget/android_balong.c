...
/*Warning: Any function support hibernation MUST add 
function: xxx_get_status and xxx func_suspend for usb3.0.
Supposely any function support usb3.0 need this but it did work
without them when disable usb hibernation ability.
*/
#define USB_ANDROID0_HIBER_FUNCTIONS "gnet,acm"
...
#define MAX_ACM_INSTANCES ACM_CDEV_COUNT
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (sscanf(buf, "%s", config->manufacturer) == 1){/* [false alarm]: no other input func in kernel.*/
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (sscanf(buf, "%s", config->common->inquiry_string) != 1){/* [false alarm]: no other input func in kernel.*/
		usb_script_backup_finish(mass_storage_inquiry, false);
		return -EINVAL;
	}
...
void usb_script_init(void)
{
	...
	*/
	...
	if(bsp_usb_is_support_hibernation()){
		usb_script_backup(android0_functions, USB_ANDROID0_HIBER_FUNCTIONS, sizeof(USB_ANDROID0_FUNCTIONS));
	}else{
