...
static netdev_tx_t eth_wrap_and_send(struct eth_dev *dev, struct sk_buff *skb)
{
    ...
    if(0 == hostpkt_exist){
        ...
        if(unlikely(!dev->port_usb)){
            dev->ctx.stats.tx_spe_drop_nofunc++;
            gnet_wakeup_fail_recycle(dev);
            return NETDEV_TX_OK;
        }
        if(unlikely(dev->gadget->is_suspend)) {
            if(unlikely(!dev->port_usb->func.func_rwakeup &&
                    (USB_SPEED_SUPER == dev->gadget->speed))){
                dev->ctx.stats.tx_spe_drop_suspend++;
                gnet_wakeup_fail_recycle(dev);
                return NETDEV_TX_OK;
            }
			
            if (usb_gadget_wakeup(dev->gadget)) {
            ...
            dev->ctx.stats.spe_rwakeup_succ++;
            dev->port_usb->func.func_is_suspend = 0;
        ...
        {/* wrapped OK */
            ...
            if (unlikely(dev->gadget->is_suspend)) {
                if(unlikely(!dev->port_usb->func.func_rwakeup && 
                        (USB_SPEED_SUPER == dev->gadget->speed))){
                    dev->ctx.stats.tx_spe_drop_suspend++;
					dev->ctx.stats.func_rwakeup_disable++;
                    gnet_wakeup_fail_recycle(dev);
                    return NETDEV_TX_OK;
                }
                ...
                if (usb_gadget_wakeup(dev->gadget)) {
                    /* wakeup failed, recycle the buffer resource */
					dev->ctx.stats.rwakeup_fail++;
                    gnet_wakeup_fail_recycle(dev);
                    return NETDEV_TX_OK;
                }
                dev->ctx.stats.ps_rwakeup_succ++;
                dev->port_usb->func.func_is_suspend = 0;
            }
        }
        else
...
void eth_suspend(struct eth_dev *dev)
{
    ...
    if(USB_SPEED_SUPER == dev->gadget->speed){
        if(dev->port_usb){
            dev->port_usb->func.func_is_suspend= 1;
            if(!dev->port_usb->func.func_rwakeup){
                if(!bsp_usb_is_ncm_bypass_mode()){
...
gnet_show(struct device *pdev, struct device_attribute *attr, char *buf)
{
    ...
    if(!dev->ctx.rx_skb_done_q){
        printk("USB gnet enum process unfinished, rx_skb_done_q not alloced.\n");
    }
    ...
    if(dev->port_usb){
        count += snprintf(buf+count,PAGE_SIZE,"| |--func_is_suspend             :%d\n",dev->port_usb->func.func_is_suspend);
        count += snprintf(buf+count,PAGE_SIZE,"| |--func_is_rwakeup_cap         :%d\n",dev->port_usb->func.func_rwakeup_cap);
        count += snprintf(buf+count,PAGE_SIZE,"| |--func_is_rwakeup             :%d\n",dev->port_usb->func.func_rwakeup);
    }
    ...
    count += snprintf(buf+count,PAGE_SIZE,"| |--stat->ps_rwakeup_succ       :%lu\n",dev->ctx.stats.ps_rwakeup_succ);
    count += snprintf(buf+count,PAGE_SIZE,"| |--stat->spe_rwakeup_succ      :%lu\n",dev->ctx.stats.spe_rwakeup_succ);
    count += snprintf(buf+count,PAGE_SIZE,"| |--stat->func_rwakeup_disable  :%lu\n",dev->ctx.stats.func_rwakeup_disable);
    count += snprintf(buf+count,PAGE_SIZE,"| |--stat->rwakeup_fail          :%lu\n",dev->ctx.stats.rwakeup_fail);
    ...
    count += snprintf(buf+count,PAGE_SIZE,"| |--stat->tx_spe_drop_suspend   :%lu\n",dev->ctx.stats.tx_spe_drop_suspend);
    count += snprintf(buf+count,PAGE_SIZE,"| |--stat->tx_spe_drop_nofunc    :%lu\n",dev->ctx.stats.tx_spe_drop_nofunc);
