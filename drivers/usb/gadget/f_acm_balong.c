...
/*ACM_CDEV_COUNT need to synchronize with g_acm_cdev_type_table count*/
struct acm_name_type_tbl g_acm_cdev_type_table[ACM_CDEV_COUNT] = {
    /* name         type(prot id) */
    {"ttyGS0",             USB_IF_PROTOCOL_PCUI, 0},
#ifndef USB_DBG_PORTS_DISABLE
    {"acm_gps",          USB_IF_PROTOCOL_GPS, 1},
    {"acm_4g_diag",     USB_IF_PROTOCOL_DIAG, 1},
    {"acm_3g_diag",     USB_IF_PROTOCOL_3G_DIAG, 1},
    {"acm_c_shell",     USB_IF_PROTOCOL_BLUETOOTH, 1},
    {"acm_ctrl",        USB_IF_PROTOCOL_CTRL, 1},
    {"acm_skytone",         USB_IF_PROTOCOL_3G_PCUI, 1},
    {"acm_voice",       USB_IF_PROTOCOL_PCVOICE, 1},
#endif
    {"cdev_unknown",         USB_IF_PROTOCOL_NOPNP, 1}
};
...
static int acm_set_alt(struct usb_function *f, unsigned intf, unsigned alt)
{
	...
	if (intf == acm->data_id) {
        ...
        if (ACM_IS_TTY(acm))
            ...
            gs_acm_modem_access_available(acm->port_num, acm->data_id, ACM_GET_NAME(acm));
        }else{
            gacm_cdev_connect(&acm->port, acm->port_num);

            //CALL ucdev interface to push the cdev_accdss_evt
            gs_acm_cdev_access_available(acm->port_num, acm->data_id, ACM_GET_NAME(acm));
            }
	}
...
static void acm_disable(struct usb_function *f)
{
    ...
    if (ACM_IS_TTY(acm)){
        ...
        gacm_modem_disconnect(&acm->port);
        bsp_usb_set_enum_stat(acm->data_id, 0, 0);
    }else{
        gacm_cdev_disconnect(&acm->port);
        bsp_usb_set_enum_stat(acm->data_id, 0, 0);
    }
...
static void acm_disconnect(struct gserial *port)
{
	struct f_acm		*acm = port_to_acm(port);
	if(NULL != port){
		port->func.func_is_suspend = 0;
		port->func.func_rwakeup= 0;
	}else{
		printk("acm_disconnect with NULL func \n");
...
static void acm_unbind(struct usb_configuration *c, struct usb_function *f)
{
    ...
    bsp_usb_del_setup_dev((unsigned)acm->data_id, 0);
...
static int	acm_get_status(struct usb_function *f)
{

    int status = 0;

    if(f->func_rwakeup_cap){
...
static int	acm_func_suspend(struct usb_function *f, u8 suspend_opt)
{
    u16 wIndex = (u16)suspend_opt;
    wIndex = wIndex<< 8;

    if(!f->func_rwakeup_cap)
        return -EINVAL;

    if(USB_INTRF_FUNC_SUSPEND_RW & wIndex){
        f->func_rwakeup = 1;
    }else{
        f->func_rwakeup = 0;
    }
...
static struct usb_function *acm_alloc_func(struct usb_function_instance *fi)
{
	...
	acm->port.func.get_status = acm_get_status;
	acm->port.func.func_suspend = acm_func_suspend;
	...
	acm->port.func.func_rwakeup_cap = 1;
#else
	acm->port.func.func_rwakeup_cap = 0;
#endif
