...
static void rndis_response_available(void *_rndis)
{
	...
	if (unlikely(cdev->gadget->is_suspend)) {
		if(unlikely((USB_SPEED_SUPER == cdev->gadget->speed) 
			&& !rndis->port.func.func_rwakeup)){

			DBG(cdev, "Supperspeed remote wakeup disabled \n");
		...
		if (usb_gadget_wakeup(cdev->gadget)) {
			...
			DBG(cdev, "Remote wakeup fail \n");

			atomic_dec(&rndis->notify_count);
			return;
		}
		rndis->port.func.func_is_suspend = 0;
	}
...
static int rndis_set_alt(struct usb_function *f, unsigned intf, unsigned alt)
{
	...
	if (intf == rndis->ctrl_id) {
        ...
        bsp_usb_set_enum_stat(rndis->data_id, 1, 0);
...
static void rndis_disable(struct usb_function *f)
{
    ...
    bsp_usb_set_enum_stat(rndis->data_id, 0, 0);
}
...
rndis_unbind(struct usb_configuration *c, struct usb_function *f)
{
    ...
    bsp_usb_del_setup_dev((unsigned)rndis->data_id, 0);
...
static int	rndis_get_status(struct usb_function *f)
{
    int status = 0;
...
static int	rndis_func_suspend(struct usb_function *f, u8 suspend_opt)
{
		...
		u32 vendorID, const char *manufacturer, struct eth_dev *dev)
{
	...
	rndis->port.func.get_status = rndis_get_status;
	rndis->port.func.func_suspend = rndis_func_suspend;

#ifdef CONFIG_GADGET_SUPPORT_REMOTE_WAKEUP
	rndis->port.func.func_rwakeup_cap = 1;
#else
	rndis->port.func.func_rwakeup_cap = 0;
#endif
