...
static mlog_lv_info_t mlog_lv_str[] =
{
    ...
    /* 烧片级别mlog,主要用于代码产生异常时的log输出，因此为了避免其他级别的log输出，特添加此log级别 */
    {"FACT" , mlog_lv_factory},
    {NULL   , mlog_lv_none  }
};
...
asmlinkage void mlog_print(char *module, mlog_lv_eum loglv,const char *fmt,...)
{
    ...
    /* 烧片版本上，只有mlog_lv_factory级别的log才能输出 */
    if ( mlog_lv_factory != loglv )
    {
        return;
    }
#else
    /* 升级版本上，只有mlog_lv_factory级别的log不输出 */
    if ( mlog_lv_factory == loglv )
    {
        return;
    }
