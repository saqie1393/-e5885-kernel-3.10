...
#define  FEATURE_RESERV_NUM   (46 - sizeof(dload_fota_feature_t) - sizeof(dload_safety1_3_t))     /* 升级预留特性个数 */
typedef struct _DLOAD_FEATURE
{
    ...
    boolean                    en_m3bootbk;                   /* m3boot双倍份启用标志 */
    boolean                    en_partition_mapping;          /* 逻辑分区和物理分区映射特性启用标志 */
    boolean                    en_single_update;              /* 双备份单镜像升级标志 */
    boolean                    en_part_name_mapping;          /* 分区名映射标志 */
    boolean                    reserved[FEATURE_RESERV_NUM];  /* 预留 */
} dload_feature_t;

/* 分区名映射结构体定义 */
#define PART_NAME (20)
typedef struct _PART_NAME_MAPPING
{
    int8 fake_name[PART_NAME];   /* 假分区名，对应于g_dev_file中的part_name */
    int8 real_name[PART_NAME];   /* 真分区名，对应于/proc/mtd中的不带bk后缀的分区名，和real_namebk分区名是同名主分区和备份分区关系 */
    int8 real_namebk[PART_NAME]; /* 真分区名，对应于/proc/mtd中的带bk后缀的分区名 */
} dload_part_name_mapping_t;
...
typedef struct _DLOAD_ADAPT_INFO
{
    uint32 product_id;               /*产品硬件ID*/
    path_info_t  *pPathinfo;         /*指向升级路径的指针*/
    upstate_info_t *pUpstate;        /*指向升级状态的指针*/
    boolean led_feature;             /* LED特性 TRUE:支持，FALSE: 不支持 */
    boolean lcd_feature;             /* LCD特性 TRUE:支持，FALSE: 不支持 */
    uint32 pathinfo_num;             /*path info number*/
    uint32 upstate_num;              /*升级状态个数*/
}product_dload_adapt_t;
