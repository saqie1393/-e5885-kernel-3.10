...
/*MMA msg的消息id和pm msg_id列表结构体*/
typedef struct mma_amsg_node_
{
    TAF_MMA_MSG_TYPE_ENUM_UINT32  taf_id;         
    huawei_pm_rsp_id_type      rsp_id;  
}mma_msg_node;
...
/* 记录是否在pstandby模式 下发pstandby之后进入pstandby模式 */
...
static pm_phone_node pm_phone_table[] = 
{
    ...
    { TAF_PH_EVT_USIM_INFO_IND,       HUAWEI_PM_RSP_ID_UR_SIMSQ },   /* ^SIMSQ*/
...
static cs_call_node pm_cs_call_table[] = 
{
    ...
    { MN_CALL_EVT_ALERTING,             HUAWEI_PM_RSP_ID_UR_CONF},      /* ^CONF [2] */
#else
    ...
    { MN_CALL_EVT_DTMF_OVER_IND,        HUAWEI_PM_RSP_ID_UR_VTSEX},     /* ^VTSEX */
#endif/*FEATURE_ON == FEATURE_DTMF*/
#if (FEATURE_ON == MBB_WPG_ECALL)
    { MN_CALL_EVT_ECALL_REDIAL_CNF,     HUAWEI_PM_RSP_ID_UR_ECLREDIAL},     /* ^ECLREDIAL */
...
/* MMA模块的主动上报 */
static mma_msg_node pm_mma_msg_table[] = 
{
#ifdef BSP_CONFIG_BOARD_TELEMATIC
    { ID_TAF_MMA_TIME_CHANGE_IND,         HUAWEI_PM_RSP_ID_UR_NWTIME},        /* ^NWTIME */
    { ID_TAF_MMA_PLMN_CHANGE_IND,         HUAWEI_PM_RSP_ID_UR_PLMNSELECTION}, /* ^PLMN */
    { ID_TAF_MMA_PLMN_SELECTION_INFO_IND, HUAWEI_PM_RSP_ID_UR_PLMNSELECTION}, /* ^PLMNSELEINFO */
    { ID_TAF_MMA_SRCHED_PLMN_INFO_IND,    HUAWEI_PM_RSP_ID_UR_PLMNSELECTION}, /* ^SRCHEDPLMN */
    { ID_TAF_MMA_REG_REJ_INFO_IND,        HUAWEI_PM_RSP_ID_UR_REJINFO },      /* ^REJINFO  */
    { ID_TAF_MMA_REG_STATUS_IND,          HUAWEI_PM_RSP_ID_UR_REG },          /* +CREG &+CGREG  */
    { ID_TAF_MMA_SRV_STATUS_IND,          HUAWEI_PM_RSP_ID_UR_SRVST },        /* ^SRVST  */
#endif
    { ID_TAF_MMA_RSSI_INFO_IND,           HUAWEI_PM_RSP_ID_UR_RSSI },          /* ^RSSI  */
    { ID_TAF_MMA_MODE_CHANGE_IND,         HUAWEI_PM_RSP_ID_UR_MODE },    /* ^HCSQ 控制方法和RSSI是一样的 */
};
...
#define PM_MMA_TABLE_LEN     (sizeof(pm_mma_msg_table) / sizeof(mma_msg_node))
...
/* 根据车载模块唤醒源主动上报配置修改此表（部分主动上报归到network和voice唤醒源） */
...
{   /*      Msg_id                        RMT_WK_type         CURC_type        cache wether repeat  */
...
#ifdef BSP_CONFIG_BOARD_TELEMATIC
    {HUAWEI_PM_RSP_ID_UR_SRVST,         RMT_WK_SRC_NETWORK,      HUAWEI_PM_RSP_GROUP_NONE,   FALSE},
    {HUAWEI_PM_RSP_ID_UR_REG,           RMT_WK_SRC_NETWORK,      HUAWEI_PM_RSP_GROUP_NONE,   FALSE},
#else
...
#ifdef BSP_CONFIG_BOARD_TELEMATIC
    {HUAWEI_PM_RSP_ID_UR_ORIG,          RMT_WK_SRC_VOICE,      HUAWEI_PM_RSP_GROUP_NONE,   FALSE},
    {HUAWEI_PM_RSP_ID_UR_CONF,          RMT_WK_SRC_VOICE,      HUAWEI_PM_RSP_GROUP_NONE,   FALSE},
    {HUAWEI_PM_RSP_ID_UR_CONN,          RMT_WK_SRC_VOICE,      HUAWEI_PM_RSP_GROUP_NONE,   FALSE},
    {HUAWEI_PM_RSP_ID_UR_CEND,          RMT_WK_SRC_VOICE,      HUAWEI_PM_RSP_GROUP_NONE,   FALSE},
#else
...
#ifdef BSP_CONFIG_BOARD_TELEMATIC
    {HUAWEI_PM_RSP_ID_UR_CLIP,          RMT_WK_SRC_VOICE,      HUAWEI_PM_RSP_GROUP_NONE,   FALSE}, 
    {HUAWEI_PM_RSP_ID_UR_CCWA,          RMT_WK_SRC_VOICE,      HUAWEI_PM_RSP_GROUP_NONE,   FALSE}, 
    {HUAWEI_PM_RSP_ID_UR_CSSI,          RMT_WK_SRC_VOICE,      HUAWEI_PM_RSP_GROUP_NONE,   FALSE}, 
    {HUAWEI_PM_RSP_ID_UR_CSSU,          RMT_WK_SRC_VOICE,      HUAWEI_PM_RSP_GROUP_NONE,   FALSE},  
#else
...
#ifdef BSP_CONFIG_BOARD_TELEMATIC
    {HUAWEI_PM_RSP_ID_UR_ECLSTAT,       RMT_WK_SRC_VOICE,      HUAWEI_PM_RSP_GROUP_NONE,   FALSE},
#else
    ...
    {HUAWEI_PM_RSP_ID_UR_OMASTATE,      RMT_WK_SRC_UR,      HUAWEI_PM_RSP_GROUP_NONE,   FALSE},
    {HUAWEI_PM_RSP_ID_UR_AUDEND,        RMT_WK_SRC_UR,      HUAWEI_PM_RSP_GROUP_NONE,   FALSE},
    {HUAWEI_PM_RSP_ID_UR_NOISERFSSI,    RMT_WK_SRC_UR,      HUAWEI_PM_RSP_GROUP_NONE,    FALSE},
    {HUAWEI_PM_RSP_ID_UR_HFOTA,         RMT_WK_SRC_UR,      HUAWEI_PM_RSP_GROUP_NONE,    FALSE},
    {HUAWEI_PM_RSP_ID_UR_ECDSA,         RMT_WK_SRC_UR,      HUAWEI_PM_RSP_GROUP_NONE,    FALSE},
    {HUAWEI_PM_RSP_ID_UR_VTSEX,         RMT_WK_SRC_UR,      HUAWEI_PM_RSP_GROUP_NONE,    FALSE},
    {HUAWEI_PM_RSP_ID_UR_ECLREDIAL,     RMT_WK_SRC_UR,      HUAWEI_PM_RSP_GROUP_NONE,    FALSE},
    {HUAWEI_PM_RSP_ID_UR_STATUS,        RMT_WK_SRC_UR,      HUAWEI_PM_RSP_GROUP_NONE,    FALSE},
    {HUAWEI_PM_RSP_ID_UR_ALACK,         RMT_WK_SRC_UR,      HUAWEI_PM_RSP_GROUP_NONE,    FALSE},
#ifdef BSP_CONFIG_BOARD_TELEMATIC
    {HUAWEI_PM_RSP_ID_UR_REJINFO,       RMT_WK_SRC_UR,      HUAWEI_PM_RSP_GROUP_NONE,    FALSE},
    {HUAWEI_PM_RSP_ID_UR_PLMNSELECTION, RMT_WK_SRC_UR,      HUAWEI_PM_RSP_GROUP_NONE,    FALSE}, 
#endif 
#if (FEATURE_ON == MBB_WPG_SIMSQ)
    {HUAWEI_PM_RSP_ID_UR_SIMSQ,         RMT_WK_SRC_UR,      HUAWEI_PM_RSP_GROUP_NONE,    FALSE},
#endif/*FEATURE_ON == MBB_WPG_SIMSQ*/
};
/*lint -e63*/
...
#if (FEATURE_ON == MBB_WPG_ECALL)
/******************************************************************************
  函数名称  : huawei_pm_get_id_to_index_voice_control()
  功能描述  : 获取MN_CALLBACK_VOICE_CONTROL类消息对应的curc的位域
  输入参数  : evt_id - 消息的ID
  被调函数  : 无
  输出参数  : 无
  返 回 值  : 对应curc的位域
              VOS_ERROR - 异常
 ********************************************************************************/
static huawei_pm_rsp_id_type huawei_pm_get_id_to_index_voice_control(MN_MSG_EVENT_ENUM_U32 evt_id)
{

    if (APP_VC_EVT_ECALL_TRANS_STATUS == evt_id)
    {
        return HUAWEI_PM_RSP_ID_UR_STATUS;
    }
    else if (APP_VC_EVT_ECALL_TRANS_ALACK == evt_id)
    {
        return HUAWEI_PM_RSP_ID_UR_ALACK;
    }
    else
    {
        return HUAWEI_PM_RSP_ID_UR_NONE;
...
/******************************************************************************
  函数名称  : huawei_pm_mma_get_id_to_index()
  功能描述  : 获取MMA类消息对应的curc的位域
  输入参数  : evt_id - 消息的ID
  被调函数  : 无
  输出参数  : 无
  返 回 值  : 对应curc的位域
              VOS_ERROR - 异常
 ********************************************************************************/
static huawei_pm_rsp_id_type huawei_pm_mma_get_id_to_index(TAF_MMA_MSG_TYPE_ENUM_UINT32 evt_id)
{
    int loop = 0;
    for ( ; loop < PM_MMA_TABLE_LEN ; loop++)
    {
        if (evt_id == pm_mma_msg_table[loop].taf_id)
        {
            pm_ccore_print_log(PM_MSG_PRINT_ON, \
                "[module_pm]:huawei_pm_mma_get_id_to_index: evt_idx 0x%x  succes \r\n", evt_id, 0, 0);
            return pm_mma_msg_table[loop].rsp_id;
        }
    }

    pm_ccore_print_log(PM_MSG_PRINT_ON, \
        "[module_pm]:huawei_pm_mma_get_id_to_index: evt_idx 0x%x  error !!!\r\n", evt_id, 0, 0);
    return HUAWEI_PM_RSP_ID_UR_NONE;    
} 
...
/******************************************************************************
  函数名称  : huawei_pm_pstandby_entry_handle
  功能描述  : 设置C核为产线待机模式状态
  输入参数  : 无
  被调函数  : 无
  输出参数  : 无
...
/******************************************************************************
  函数名称  : huawei_pm_pstandby_state_get
  功能描述  : 获取C核是否处于产线待机模式
  输入参数  : 无
  被调函数  : 无
  输出参数  : 无
  返 回 值  : 1 -处于产线待机模式 0--不处于产线待机模式
 ********************************************************************************/
...
huawei_pm_rsp_id_type huawei_pm_id_converse(VOS_PID Pid, VOS_VOID *pMsg)
{
    ...
    /* mma 模块 主动上报的事件ID */
    VOS_UINT32 msg_at_mma_evt_id = 0;
#if (FEATURE_ON == MBB_WPG_ECALL)
    /* 查看VOICE_CONTROL 主动上报的事件ID */
    MN_MSG_EVENT_ENUM_U32 msg_at_id_voice_control = 0;
#endif/*FEATURE_ON == MBB_WPG_ECALL*/
...
#if (FEATURE_ON == MBB_WPG_ECALL)
            else if( MN_CALLBACK_VOICE_CONTROL == pAtMsgCtrlBlk->usMsgName)
            {
                msg_at_id_voice_control = *(MN_CALL_EVENT_ENUM_U32*)(pAtMsgCtrlBlk->aucContent);

                evt_id = huawei_pm_get_id_to_index_voice_control(msg_at_id_voice_control);

                pm_ccore_print_log(PM_MSG_PRINT_ON, \
                    "[module_pm]:#This is ECALL ur, the send_id is %d\r\n", evt_id, 0, 0);

                return evt_id;
            }
#endif/*FEATURE_ON == MBB_WPG_ECALL*/
#if(FEATURE_ON == MBB_WPG_FOTA)
            else if ( MN_CALLBACK_FOTA == pAtMsgCtrlBlk->usMsgName )
            {
                return HUAWEI_PM_RSP_ID_UR_HFOTA;
            }
            else if ( MN_CALLBACK_ECDSA == pAtMsgCtrlBlk->usMsgName )
            {
                return HUAWEI_PM_RSP_ID_UR_ECDSA;
            }
...
#ifdef BSP_CONFIG_BOARD_TELEMATIC
           || ( SI_STK_CMD_TIMEOUT_IND_EVENT == msg_at_id_stk ) 
#endif
        )
        {
    ...
    else if ( ( MSP_SYS_FTM_PID == Pid ) && ( WUEPS_PID_AT == ulPid ) )
    {
        evt_id = HUAWEI_PM_RSP_ID_UR_NOISERFSSI;
        pm_ccore_print_log(PM_MSG_PRINT_ON, \
            "[module_pm]:This is MSP FTM msg, the send_id is %d\r\n", evt_id, 0, 0);
        return evt_id;
    }
    else if ( ( I0_WUEPS_PID_MMA == Pid ) && ( WUEPS_PID_AT == ulPid ) )
    {
        msg_at_mma_evt_id = ((TAF_MMA_IND_EVT_HEADER *)pMsg)->ulMsgName;
        evt_id = huawei_pm_mma_get_id_to_index(msg_at_mma_evt_id);
        pm_ccore_print_log(PM_MSG_PRINT_ON, \
            "[module_pm]:This is mma msg, the send_id is %d\r\n", evt_id, 0, 0);
        return evt_id;
    }
...
)
{
    ...
    if (TRUE == usb_channel)
    {
        ...
        mlog_print("hw_pmC",mlog_lv_info,"wakeup by USB, source is 0x%02x(GUDSV)", wake_source);
#endif
    }
    ...
    {
#if ( FEATURE_ON == MBB_MLOG )
        mlog_print("hw_pmC",mlog_lv_info,"wakeup by PIN, source is 0x%02x(GUDSV)", wake_source);
#endif
...
#ifdef BSP_CONFIG_BOARD_TELEMATIC
/******************************************************************************
  函数名称  : huawei_pm_check_wakeup_src()
  功能描述  : 判断是否唤醒源C核是否应当上报
  输入参数  : wake_source:唤醒源种类
              status:是否是CURC控制主动上报
  被调函数  : 无
  输出参数  : 无
  返 回 值      : VOS_TRUE VOS_FALSE
  ******************************************************************************/
unsigned int huawei_pm_check_wakeup_src
(
    rmt_wk_src_type wake_source,
    unsigned int status
) 
{
    ...
    /* 如果CURC 为0并且是CURC控制的主动上报就直接丢弃 */
    if ((DSAT_UR_DISABLE == curc_val) && (VOS_TRUE == status))
    {
        return VOS_FALSE;
    }

    /* 如果模块没有休眠则所有上报都需要传输*/
    is_usb_sleep = huawei_pm_usb_is_sleep();
    is_pin_sleep = huawei_pm_pin_is_sleep();
    if ((0 == is_usb_sleep) || (0 == is_pin_sleep))
    {
        return VOS_TRUE;
    }
    else
...
#endif
/******************************************************************************
  函数名称  : huawei_pm_check_data_type_ex()
  功能描述  : 判断数据的处理方式：缓存，传输或者丢弃
  输入参数  : 数据类型
  被调函数  : 无
  输出参数  : 无
...
) 
{
        ...
        ( HUAWEI_PM_RSP_ID_ASYN_CMD == send_id ) )
    {
        ...
        /* 车载上AT回复 在休眠时不需要唤醒上位机并上报 */
        huawei_pm_remote_check_and_handle(TRUE, &usb_store, wake_source);
        if ( TRUE == usb_store)
        {
    ...
    huawei_pm_remote_check_and_handle(wakeup_host, &usb_store, wake_source);
...
boolean huawei_pm_check_data_type(unsigned long Pid, void *pMsg) 
{
    ...
    /*产线待机不允许唤醒*/
    if (1 == huawei_pm_pstandby_state_get())
    {
        return FALSE;
    }
