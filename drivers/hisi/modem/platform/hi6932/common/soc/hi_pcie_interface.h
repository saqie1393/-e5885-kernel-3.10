...
{
    hi_pcie_set(__virt_addr + 0x20, 0, 1, 1);
}
...
static __inline__ void hi_pcie_phy_init(void* __virt_addr, u32 __pcie_id)
{
    ...
    if (0 == __pcie_id)
    {
        hi_pcie_set(__virt_addr + 0xC24, 0, 7, 0x5F);
    }
    else
    {
        hi_pcie_set(__virt_addr + 0xC84, 0, 7, 0x5F);
    }
#endif
#ifdef BSP_CONFIG_BOARD_E5885Ls_93a
    hi_pcie_set(__virt_addr + 0xC24, 0, 7, 0x30);
#endif
}
...
static __inline__ void hi_pcie_output_clk_enable(void* __virt_addr, u32 __pcie_id)
{
    if (0 == __pcie_id)
    {
        hi_pcie_set(__virt_addr + 0xC10, 0, 2, 0x3);
    ...
    {
        hi_pcie_set(__virt_addr + 0xC10, 16, 2, 0x3);
