...
typedef enum
{
    ...
    RMT_WK_SRC_NETWORK  = 0x20,   /*network data*/
    RMT_WK_SRC_MAX   = 0x3F
#else
    RMT_WK_SRC_MAX   = 0x1F
#endif
...
typedef enum
{
    ...
    /*Send to host anyway,if USB suspend,then wake up host*/
...
typedef enum
{
    ...
    PM_MSG_PSTANDBY_IN,         /*A->C产线待机，禁止C核的数据*/ 
}MODULE_PM_MSG_ID;
...
typedef enum
{
    ...
    HUAWEI_PM_RSP_ID_RESULT_CODE,     
    /*Final result code,such as:CONNECT&BUSY&NO CARRIER*/
    ...
    HUAWEI_PM_RSP_ID_UR_SMS,         
    /*CMT,CMTI,CDS,CDSI,CBM,Convert from HUAWEI_PM_RSP_ID_SMS*/
    ...
    HUAWEI_PM_RSP_ID_UR_OMASTATE,
    HUAWEI_PM_RSP_ID_UR_AUDEND,
    HUAWEI_PM_RSP_ID_UR_NOISERFSSI,
    HUAWEI_PM_RSP_ID_UR_HFOTA,
    HUAWEI_PM_RSP_ID_UR_ECDSA,
    HUAWEI_PM_RSP_ID_UR_VTSEX,
    HUAWEI_PM_RSP_ID_UR_ECLREDIAL,
    HUAWEI_PM_RSP_ID_UR_STATUS,
    HUAWEI_PM_RSP_ID_UR_ALACK,
#if (FEATURE_ON == MBB_WPG_SIMSQ)
    HUAWEI_PM_RSP_ID_UR_SIMSQ,
#endif/*FEATURE_ON == MBB_WPG_SIMSQ*/
#ifdef BSP_CONFIG_BOARD_TELEMATIC
    HUAWEI_PM_RSP_ID_UR_REJINFO,          /*^REJINFO */
    HUAWEI_PM_RSP_ID_UR_PLMNSELECTION,          /*^PLMNSELEINFO*/
#endif
...
typedef struct huawei_pm_rsp_info_struct
{
    ...
    boolean duplicate;                  
    /*The flag indicates whether dupliciate store when cache*/
...
typedef struct huawei_pm_rsp_cache_struct
{
    ...
    uint32 tail;                           
    /*Tail index,This position doesn't contain data*/
