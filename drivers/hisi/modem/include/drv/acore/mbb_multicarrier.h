...
typedef enum
{
    MTC_SIM_SWITCH,     /* SIM卡自动切换 */
    MTC_MANUAL_SWITCH,  /* 手动切换 */
    MTC_DEAAULT_SWITCH, /* 设置并运行默认运营商 */
    MTC_OP_VALID,
} mtc_op_type;
...
typedef enum
{
    MTC_SWITCH_CARRIER_FAIL,    /* 切换失败 */
    MTC_SWITCH_CARRIER_SAME,    /* 当前运营商即为目标运营商，不需要切换 */
    MTC_SWITCH_CARRIER_SUCCESS, /* 切换成功 */
...
typedef struct
{
    ...
    unsigned char       aucReserved[3];  /* 与TAF_HPLMN_WITH_MNC_LEN_STRU保持一致 */
