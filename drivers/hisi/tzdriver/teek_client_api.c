	...
	for (param_cnt = 0; param_cnt < 4; param_cnt++) {
		if ((TEEC_MEMREF_TEMP_INPUT == param_type[param_cnt])
			...
			cli_context->params[param_cnt].memref.buffer
				=
				(unsigned int)operation->params[param_cnt].tmpref.buffer;
			cli_context->params[param_cnt].memref.buffer_h_addr
				= 0;
			cli_context->params[param_cnt].memref.size_addr =
				(unsigned int)&operation->params[param_cnt].tmpref.size;
			cli_context->params[param_cnt].memref.size_h_addr = 0;
			...
			if (TEEC_MEMREF_WHOLE == param_type[param_cnt]) {
				...
				cli_context->params[param_cnt].memref.
					size_addr =
					(unsigned int)&operation->params[param_cnt].
					memref.parent->size;
				cli_context->params[param_cnt].memref.
					size_h_addr = 0;
			} else {
			...
			if (operation->params[param_cnt].memref.parent->
			    is_allocated) {
				cli_context->params[param_cnt].memref.buffer =
					(unsigned int)operation->params[param_cnt].memref.
					parent->buffer;
				cli_context->params[param_cnt].memref.buffer_h_addr = 0;
			} else {
				cli_context->params[param_cnt].memref.buffer
					=
					(unsigned int)(operation->params[param_cnt].memref.
			...
			cli_context->params[param_cnt].value.a_addr =
				(unsigned int)&operation->params[param_cnt].value.a;
			cli_context->params[param_cnt].value.a_h_addr = 0;
			cli_context->params[param_cnt].value.b_addr =
				(unsigned int)&operation->params[param_cnt].value.b;
			cli_context->params[param_cnt].value.b_h_addr = 0;
		}
	}
