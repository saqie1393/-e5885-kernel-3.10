...
start:
	...
	if (ring_flag != reader->r_ver /*ring_flag not same */) {
		...
		if (log->w_off < reader->r_off) {
			...
			tloge("ERR tlogcat read=%d write=%d \n", reader->r_off, log->w_off);
			ret = -1;
			goto out;
		}
	...
	if (disjoin_flag && ((reader->r_off + log_len) > m_addr_end)) {
		if (CHECK_BOUNDARY(reader->r_off)) {
			tloge("1 CHECK_BOUNDARY error reader->r_off=0x%d\n", reader->r_off);
		}
...
static int __init tlogger_init(void)
{
	...
	m_log_buffer =(char *)ioremap_wc(m_addr_p, m_rar_mem_len - OFFSET2);
	if(!m_log_buffer){
		return -ENOMEM;
	}
