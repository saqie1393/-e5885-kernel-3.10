	...
	for (i = 0; i < 4; i++) {
		...
		/* value */
			...
			if (copy_from_client(&operation->params[i].value.a,
					     (void __user *)client_param->value.a_addr,
			...
			if (copy_from_client(&operation->params[i].value.b,
					     (void __user *)client_param->value.b_addr,
					     sizeof(operation->params[i].
		...
		if ((TEEC_MEMREF_TEMP_OUTPUT == param_type) ||
			...
			if (copy_to_client
				...
				break;
					  ...
					  (TEEC_VALUE_INOUT == param_type))) {
			/* value */
			if (copy_to_client((void __user *)client_param->value.a_addr,
			...
			if (copy_to_client((void __user *)client_param->value.b_addr,
...
int tc_client_call(TC_NS_ClientContext *client_context,
		   TC_NS_DEV_File *dev_file, uint8_t flags, unsigned int timeout)
{
	int ret = 0;
	...
	tee_ret = TC_NS_SMC(smc_cmd, flags, timeout);
    ...
    if(tee_ret == TEEC_ERROR_CLIENT_TIMEOUT){
        TCERR("teec client timeout 0x%x\n", tee_ret);
        goto error1;
    }
