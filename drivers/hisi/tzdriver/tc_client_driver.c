...
static char ca_hash[SHA256_DIGEST_LENTH] = {
    0x25, 0x8 , 0xae, 0x46,
    0x68, 0x38, 0x7 , 0x37,
    0xb3, 0x9b, 0x6d, 0x82,                                                       
    0x4c, 0x97, 0xa , 0x78,                                                         
...
static char *get_process_path_for_v722(struct task_struct *tsk, char *buf)
{
	struct path path;
	char *path_nm = NULL;
	struct mm_struct *mm;
...
static int check_package_name(TC_NS_DEV_File *dev_file)
{
	...
	if (TEE_REQ_FROM_USER_MODE == dev_file->kernel_api) {
		...
		ca_dentry_path = get_process_path_for_v722(current, dentrypath_buf);
			 ...
			 unsigned int session_id)
{
	...
	ret = tc_client_call(&context, dev, TC_CALL_GLOBAL | TC_CALL_SYNC, TEE_NO_CLIENT_TIMEOUT);
	if (ret)
				 ...
				 TC_NS_ClientContext *context)
{
	...
	* device file pointer */
	...
	context->params[3].memref.size_addr = (unsigned int)&dev_file->pkg_name_len;
	context->params[3].memref.buffer = (unsigned int)dev_file->pkg_name;

	/* Set public key len and public key */
	if (dev_file->pub_key_len != 0) {
		context->params[2].memref.size_addr =
			(unsigned int)&dev_file->pub_key_len;
		context->params[2].memref.buffer = (unsigned int) dev_file->pub_key;
	} else {
...
static int TC_NS_Client_Timeout(TC_NS_DEV_File *dev_file, void __user *buffer)
{
    int ret = -EINVAL;
    TC_NS_Service *service;
    TC_NS_Session *session;

    TC_Client_TimeOut client_timeout_setting;
    if (copy_from_user(&client_timeout_setting, buffer, sizeof(TC_Client_TimeOut))) {
        TCERR("Failed to get client timeout!\n");
        ret = -EINVAL;
        goto error;
    }
    mutex_lock(&g_tc_ns_dev_list.dev_lock);
    service = tc_find_service(&dev_file->services_list, client_timeout_setting.uuid);
    if (service) {
        session =
            tc_find_session(&service->session_list,
                client_timeout_setting.session_id);
        if (session) {
            session->client_timeout =  client_timeout_setting.timeout;
            TCERR("TC_NS_Client_Timeout settimeout %d!\n",session->client_timeout);
        } else {
            ret = -EINVAL;
        }
    } else {
...
find_service:
	...
	/*send smc */
	ret = tc_client_call(context, dev_file, flags, TEE_NO_CLIENT_TIMEOUT);
    ...
    session->client_timeout = TEE_NO_CLIENT_TIMEOUT;
...
int TC_NS_KillSession(TC_NS_DEV_File *dev_file, TC_NS_ClientContext *context)
{
	 int ret = 0;
    TC_NS_Session *session = NULL;
    TC_NS_Service *service = NULL;

    TCERR("TC_NS_KillSession enter %d\n", context->session_id);
	atomic_inc(&error_cmds);
	mutex_lock(&g_tc_ns_dev_list.dev_lock);
    service = tc_find_service(&dev_file->services_list, context->uuid);
    if (service) {
        session =
        ...
        if (session) {
            ret = kill_session(dev_file, service->uuid,
                session->session_id);
			if(!ret){
				list_del(&session->head);
				kfree(session);
    ...
    mutex_unlock(&g_tc_ns_dev_list.dev_lock);
	atomic_dec(&error_cmds);
    TCERR("TC_NS_KillSession %d exit\n", context->session_id);
...
find_session:
	...
	/*send smc */
	mutex_lock(&session->ta_session_lock);
	ret = tc_client_call(context, dev_file, 0, session->client_timeout);
	mutex_unlock(&session->ta_session_lock);
	put_session_struct(session);
				    ...
				    unsigned long arg)
{
	...
	case TC_NS_CLIENT_IOCTL_KILLSESSION:{
			ret = TC_NS_KillSession(dev_file, &context);
...
static long tc_client_ioctl(struct file *file, unsigned cmd, unsigned long arg)
{
	...
	case TC_NS_CLIENT_IOCTL_LOGIN: {
	    ...
	    case TC_NS_CLIENT_IOCTL_TIMEOUT:{
			ret = TC_NS_Client_Timeout(dev_file, argp);
			break;
		}
