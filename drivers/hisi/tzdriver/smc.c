...
static int smc_send_siq(uint32_t cmd, phys_addr_t cmd_addr, uint32_t cmd_type, uint8_t wait)
{
	//TCDEBUG("start to send smc to secure\n");
	register unsigned int r0 asm("r0") = cmd;
	register unsigned int r1 asm("r1") = cmd_addr;
	register unsigned int r2 asm("r2") = cmd_type;
	...
	if (r0 == MAPI_TZ_API_IRQ || r0 == MAPI_TZ_API_SIQ )
        return TSP_REQUEST;
	else if(r0 == MAPI_RET_FROM_SECURE)
		return r1;
...
static int smc_thread_fn(void *arg)
{
	...
	while (!kthread_should_stop()) {
		...
		ret = TSP_REQUEST;
		if(i % 10 == 0 && (atomic_read(&outstading_cmds) > 1 || atomic_read(&error_cmds))) {
			ret = smc_send_siq(MAPI_TZ_API_SIQ, 0, 0, false);
        }
		else {
				  ...
				  uint8_t flags, unsigned int timeout)
{
    ...
    if(timeout == TEE_NO_CLIENT_TIMEOUT){
        wait_for_completion(&we->done);
    } else {
	    if(!wait_for_completion_timeout(&we->done,timeout)){
			mutex_lock(&wait_th_lock);
		    cmd->ret_val = TEEC_ERROR_CLIENT_TIMEOUT; 
            list_del(&we->list);
			mutex_unlock(&wait_th_lock);
            tloge("***client timeout  %d***\n",timeout);
	    }
    }
 ...
 */
unsigned int TC_NS_SMC(TC_NS_SMC_CMD *cmd, uint8_t flags, unsigned int timeout)
{
	unsigned int ret = 0;
	...
	mb();
	ret = smc_send_func(cmd, TC_NS_CMD_TYPE_NS_TO_SECURE, flags, timeout);
...
unsigned int TC_NS_SMC_WITH_NO_NR(TC_NS_SMC_CMD *cmd, uint8_t flags)
{
	 ...
	 */
	mb();
	ret = smc_send_func(cmd, TC_NS_CMD_TYPE_NS_TO_SECURE, flags, TEE_NO_CLIENT_TIMEOUT);
