...
/***********************************************************************************
函 数 名  : save_func_addr
功能描述  : 添加存储指定函数地址到SRAM的功能，用于辅助A核WDT超时问题定位。
输入参数  : i    表示index
            addr 要存储的地址
返 回 值  : 无
***********************************************************************************/
void save_func_addr(int i,int addr)
{
    unsigned int* mbb_debug_data = NULL;
    mbb_debug_data = SRAM_MBB_DEBUG_ADDR;

    if (i >= DEBUG_SAVE_LEN)
    {
        printk("%s: ERROR，position %d out of range.\n", __func__, i);
        return;
    }

    mbb_debug_data[i] = addr;
    printk("%s: i:%d,addr: 0x%x\n", __func__, i, addr);
}
EXPORT_SYMBOL(save_func_addr);

/***********************************************************************************
函 数 名  : show_sram_value
功能描述  : 测试接口，用于ecall调用，打印当前SRAM指定区域存储的数据。
返 回 值  : 返回0
***********************************************************************************/
int show_sram_value(void)
{
    int i = 0;
    unsigned int* mbb_debug_data = NULL;
    mbb_debug_data = SRAM_MBB_DEBUG_ADDR;

    for (i = 0; i < DEBUG_SAVE_LEN; ++i)
    {
        printk("addr: 0x%p, 0x%x\n", &(mbb_debug_data[i]), mbb_debug_data[i]);
    }
    return 0;
}

/***********************************************************************************
函 数 名  : show_sram_addr
功能描述  : 测试接口，用于ecall调用，打印当前SRAM指定区域对应的地址。
返 回 值  : 返回0
***********************************************************************************/
void show_sram_addr(void)
{
    unsigned int* mbb_debug_data = NULL;
    mbb_debug_data = SRAM_MBB_DEBUG_ADDR;

    printk("%-30s%10s%10s%10s\n", "name", "phy addr", "virt addr", "size");
    printk("%-30s%10p%10lx\n", "SRAM_BASE_ADDR", SRAM_V2P((unsigned long)SRAM_BASE_ADDR),
        (unsigned long)SRAM_BASE_ADDR);
    printk("%-30s%10p%10lx\n", "SRAM_MBB_DEBUG_ADDR", SRAM_V2P((unsigned long)SRAM_MBB_DEBUG_ADDR),
        (unsigned long)SRAM_MBB_DEBUG_ADDR);
}
#endif
...
CRADLE_TYPE chg_get_cradletype_from_usbid_voltage(void)
{
    ...
    int ret = 0;
    /*获取USBID电压*/
    ret = bsp_hkadc_convert(USBID_VBUS_HKADC, &usbid_voltage);

    /*海思基线由于提高温度精度，把ADC电压的单位从mv改成了0.1mv，因此ADC的数值变大了10倍*/
    usbid_voltage = usbid_voltage / HKADC_PRECISION;
    if(ret)
    {
        printk(KERN_ERR "get usb voltage fail!!!\n");
        return CRADLE_TYPE_INVALID;
    }

    if(usbid_voltage > MIN_USBID_VOLTAGE && usbid_voltage < MAX_USBID_VOLTAGE)
    {
        return CRADLE_TYPE_V7R22;
    }
    else
    {
        return CRADLE_TYPE_INVALID;
