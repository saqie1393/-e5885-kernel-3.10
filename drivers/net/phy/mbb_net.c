...
#endif/*MBB_BUILD_DEBUG || MBB_FEATURE_ETH_WAN_MIRROR*/
#else
int RNIC_WANMirror(unsigned int enable, unsigned int port)
{
    pr_err("stub %s\n",__FUNCTION__);
...
int RNIC_GetWanMirrorStatus(ETH_MIRROR_STATUS_S *status)
{
    ...
    status->port = wan_mirror_port;
    status->status = wan_mirror_enable;
    return 0;
}
...
int  set_route_state(char* data)
{
    ...
    if ('1' == data[0])
    {
        printk("route_state = 1\n");
        eth_route_state = 1;
    }
    else if ('0' == data[0])
    {
        printk("route_state = 0\n");
        eth_route_state = 0;
    }
    return ret;
...
#if (FEATURE_ON == MBB_ETH_PHY_LOWPOWER)
/*****************************************************************************
函数名：  get_cradle_state
功能描述: 网线插入拔插标识
输入参数:
返回值：
*****************************************************************************/
static ssize_t get_cradle_state(struct device *dev, struct device_attribute *attr, char *buf)
{
    struct phy_device *phydev = mbb_get_phy_device();
    if((NULL == phydev) || (NULL == buf))
    {
        return -EINVAL;
    }
    return snprintf(buf, SYS_NODE_INT_BUF_LEN, "%0x\n", phydev->link);
...
static DEVICE_ATTR(cradle_state, S_IRUGO, get_cradle_state, NULL);

/*****************************************************************************
fuction：  set_timeout_state
discription: by timeout_state request phy 3min wake lock
para:1：request   0：release
return：
*****************************************************************************/

static ssize_t set_timeout_state(struct device *dev,
                struct device_attribute *attr, const char *buf, size_t count)
{
    char value;
    if(NULL == buf)
    {
        printk("error : input timeout state is NULL\n");
        return -EPERM;
    }
    value = *buf;
    if(value == '1')
    ...
    {
        printk("error : input timeout state unknown\n");
    }

    return count;
}
static DEVICE_ATTR(timeout_state, S_IRUGO | S_IWUSR, NULL, set_timeout_state);
#endif
/*****************************************************************************
...
#if (FEATURE_ON == MBB_ETH_PHY_LOWPOWER)
    err = device_create_file(lan_dev, &dev_attr_cradle_state);
    if (err)
    {
        printk("failed to create file cradle_state\n");
    }
    err = device_create_file(lan_dev, &dev_attr_timeout_state);
    if (err)
    {
        printk("failed to create file timeout_state\n");
    }
#endif
...
void hw_net_sysfs_uninit(void)
{
    ...
    device_remove_file(lan_dev, &dev_attr_timeout_state);
#endif
...
int et_reset_linkstate(int mode)
{
    ...
    if (NULL != phydev)
    {
        ...
        regvalue |= BMCR_RESET;
        err = phy_write(phydev, MII_BMCR, regvalue);
        if(err < 0)
        {
            return NET_RET_FAIL;
        }
...
void ET_MMI_test_start(void)
{
    ...
    {       
        SET_RES_BIT(ET_MMITEST_DATA_12_FAIL);
        if (NET_RET_OK == mbb_mmitest_send_packet(pstDev, VLAN_PORT0))
        {
            msleep(ET_MMITEST_SLEEP_TIMEORT);
            mbb_mmitest_check_recv(pstDev, mmitest_check_packet);
        }
