...
static int dwmac1000_core_init(void __iomem *ioaddr)
{
	...
	for (i = 0; i < gmac_priv->tx_queue_cnt; i++) {
				...
				struct stmmac_extra_stats *x)
{
	...
	if (GET_VALUE(mac_isr, MAC_ISR_TSIS_LPOS, MAC_ISR_TSIS_HPOS)) {
		...
		x->timestamp_irq++;
		CHIP_DBG(KERN_DEBUG "timestamp irq:0x%x\n", mac_timstamp_status);
	}
...
static unsigned int dwmac1000_get_ts_rollover(void __iomem *ioaddr)
{
	unsigned int rollover_mode;

	rollover_mode = readl(ioaddr + GMAC_TIMESTAMP_CONTROL);
	rollover_mode &= GMAC_TCR_TSCTRLSSR;

	return rollover_mode;
}

static void dwmac1000_dis_rx(void __iomem *ioaddr)
{
	unsigned int val;

	val = readl(ioaddr + GMAC_CONTROL);
	val &= ~GMAC_CONTROL_RE;
	writel(val, ioaddr + GMAC_CONTROL);	
}

static void dwmac1000_dis_tx(void __iomem *ioaddr)
{
	unsigned int val;
...
static void dwmac1000_enable_tx(void __iomem *ioaddr)
{
	unsigned int val;
...
static void dwmac1000_enable_rx(void __iomem *ioaddr)
{
	unsigned int val;
...
static void dwmac1000_set_mac(void __iomem *ioaddr, unsigned int val)
{
	writel(val, ioaddr + GMAC_CONTROL);
}
...
static const struct stmmac_ops dwmac1000_ops = {
	...
	.get_ts_rollover = dwmac1000_get_ts_rollover,
	.dis_rx = dwmac1000_dis_rx,
	.dis_tx = dwmac1000_dis_tx,
	.enable_tx = dwmac1000_enable_tx,
	.enable_rx = dwmac1000_enable_rx,
	.set_mac = dwmac1000_set_mac,
};
