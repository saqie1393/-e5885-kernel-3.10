...
#else
    /* 车载模块使能以太网的同时也需要支持电源管理，此处需要关闭时钟，否则睡眠时会crash */
    stmmac_clk_disable(dev);
    if (ret)
    {
        return ret;
...
#else
static int stmmac_pltfr_suspend_balong(struct device *dev)
{
	struct net_device *ndev = dev_get_drvdata(dev);

	return stmmac_suspend_balong(ndev);
}

static int stmmac_pltfr_resume_balong(struct device *dev)
{
	struct net_device *ndev = dev_get_drvdata(dev);

	return stmmac_resume_balong(ndev);
}
#endif
...
#ifdef CONFIG_BALONG_GMAC_LPM
	.suspend = stmmac_pltfr_suspend_balong,
	.resume = stmmac_pltfr_resume_balong,
#else
