...
static int ptp_init(void __iomem *ioaddr)
{
	...
	value = readl(ioaddr + GMAC_TIMESTAMP_CONTROL);
	value |= GMAC_TCR_TSCFUPDT | GMAC_TCR_TSINIT | PTP_TCR_TSVER2ENA | 
				PTP_TCR_TSIPENA | PTP_TCR_SNAPTYPSEL_1 | PTP_TCR_TSMSTRENA |
				PTP_TCR_TSEVNTENA | GMAC_TCR_TSCTRLSSR;
	writel(value, ioaddr + GMAC_TIMESTAMP_CONTROL);
	...
	ret = stmmac_config_addend(ioaddr,ADDEND_DEFAULT);
	if (ret < 0) {
		return -EBUSY;
	}
