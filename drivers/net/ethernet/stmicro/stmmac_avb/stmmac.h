...
struct stmmac_priv {
	...
	unsigned int init_suc;
	unsigned int is_open;
	unsigned int need_open;

#ifdef CONFIG_NEW_GMAC_TEST
	struct sk_buff *recv_skb;
	unsigned int recv_pkt;
	unsigned int phy_loopback;
#endif
};
...
int stmmac_resume_balong(struct net_device *ndev);
int stmmac_suspend_balong(struct net_device *ndev);
...
#ifdef BSP_CONFIG_BOARD_TELEMATIC
#if ((FEATURE_ON == MBB_FACTORY) || (FEATURE_ON == MBB_AGING_TEST))
extern void SetPhyReg(unsigned int  regnum, unsigned short val);
extern int GetPhyReg(unsigned int regnum);
void local_receive_skb(int frame_len,struct sk_buff *skb);
extern struct phy_device* mbb_get_phy_device(void);
#endif
#endif
