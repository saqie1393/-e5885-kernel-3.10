...
#define GMAC_REG_OFFSET_END	(0x1280)
...
struct stmmac_extra_stats {
	...
	unsigned long tx_invalid_pkt;
	unsigned long tx_spe_invalid_pkt;

	/*  gmac test    */
	unsigned long tx_desc_full[TX_NUMBER];
	unsigned long enter_xmit[TX_NUMBER];
	unsigned long enter_spe_xmit[TX_NUMBER];
	unsigned long xmit_td_full[TX_NUMBER];
	unsigned long gmac_release;
	unsigned long tx_avail[TX_NUMBER];
	/*LPM status*/
	unsigned long gmac_suspend;
	unsigned long gmac_resume;
	unsigned long gmac_freeze;
	unsigned long gmac_restore;
	unsigned long rx_busy_n;
	unsigned long tx_busy_n;
	unsigned long gmac_suspend_save;
	unsigned long gmac_suspend_save_suc;
	unsigned long gmac_suspend_restore;
	unsigned long gmac_suspend_restore_suc;
	unsigned long gmac_suspend_restore_fail;
	...
	/* packet related */
	unsigned long tx_pkt_cnt[TX_NUMBER];

	/* avb related */
	unsigned long dma_chn_int[TX_NUMBER];
	unsigned long mac_int;

	/* Receive errors */
	unsigned long rx_outer_vlan_tag;
	unsigned long rx_inner_valn_tag;
	...
	unsigned long tx_stopped[TX_NUMBER];
	...
	unsigned long tx_early_irq[TX_NUMBER];
	unsigned long fatal_bus_error_irq[TX_NUMBER];
	unsigned long rx_watchdog_timeout;
	unsigned long tx_jabber_timeout;
	unsigned long tx_ctx_desc_err[TX_NUMBER];
	unsigned long rx_dma_err[RX_CHN_NR];
	unsigned long tx_dma_err[TX_NUMBER];
	unsigned long tx_buf_unavail[TX_NUMBER];
	unsigned long tx_clean_own[TX_NUMBER];
	unsigned long tx_chn_err;
	unsigned long tx_mac_header_err;
	/* Tx/Rx IRQ Events */
	unsigned long rx_early_irq[RX_CHN_NR];
	unsigned long threshold;
	unsigned long tx_pkt_n[TX_NUMBER];
	unsigned long rx_pkt_n;
	unsigned long normal_irq_n[TX_NUMBER];
	unsigned long rx_normal_irq_n[RX_CHN_NR];
	unsigned long napi_poll;
	unsigned long tx_normal_irq_n[TX_NUMBER];
	unsigned long tx_clean[TX_NUMBER];
	unsigned long tx_reset_ic_bit[TX_NUMBER];
	...
	/*MTL interrupt */
	unsigned long mtl_int[TX_NUMBER];

	/* timestamp related */
	unsigned long timestamp_irq;

	/* Link status */
	unsigned long rgmii_link_changed;	//phy interrupts
	unsigned long poll_link_changed;	//phy poll link change succeed
	unsigned long poll_link_cfailed;	//phy poll link change failed

#ifdef CONFIG_AVB_NET
	unsigned long bw_cleared;
#endif
};
...
struct stmmac_desc_ops {
	...
	int (*get_rx_timestamp_status) (union dma_desc *desc);
	/* check if rx tstamp available */
	int (*rx_tstamp_available)(union dma_desc *desc);
	/* config tx slotnum */
...
struct stmmac_dma_ops {
	...
	void (*clear_status) (void __iomem *ioaddr, int chn);
...
struct stmmac_ops {
	...
	unsigned int (*get_ts_rollover)(void __iomem *ioaddr);

	void (*dis_rx)(void __iomem *ioaddr);
	void (*dis_tx)(void __iomem *ioaddr);
	void (*enable_tx)(void __iomem *ioaddr);
	void (*enable_rx)(void __iomem *ioaddr);
	void (*set_mac)(void __iomem *ioaddr, unsigned int val);
};

struct stmmac_mtl_ops {
	/* MTL Initialization */
	void (*mtl_init)(void);
	unsigned int (*tx_status)(void __iomem *ioaddr, int chn);
	unsigned int (*rx_status)(void __iomem *ioaddr, int chn);
	void (*flush_txq)(void __iomem *ioaddr, int chn);
};
...
struct stmmac_cbs_ops {
	...
	int (*get_locredit)(struct net_device *dev,unsigned int class_id, int *locredit);
	int (*get_hicredit)(struct net_device *dev,unsigned int class_id,int *hicredit);
	int (*get_avb_algorithm)(struct net_device *dev,unsigned int class_id,int *sch);
	int (*get_preamble_ipg)(struct net_device *dev,unsigned int *igb_pre);
