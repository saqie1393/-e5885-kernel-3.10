			       ...
			       union dma_desc *p)
{
	...
	if (p->rx_desc.nwb.outer_vlan_tag) {
		x->rx_outer_vlan_tag++;
	}

	if (p->rx_desc.nwb.inner_valn_tag) {
		x->rx_inner_valn_tag++;
	}
#endif
...
static int ndesc_get_rx_timestamp_status(union dma_desc *desc)
{
	if ((desc->rx_desc.ctx.rx_ts_low == 0xffffffff) && (desc->rx_desc.ctx.rx_ts_high == 0xffffffff))
...
static int ndesc_rx_tstamp_available(union dma_desc *desc)
{
	unsigned int rs1v;
	unsigned int ts_avail;

	rs1v = desc->rx_desc.nwb.rdes1_stat_valid;
	if (rs1v == 1) {
		ts_avail = desc->rx_desc.nwb.ts_avail;
		return ts_avail;
	} else {
		return 0;
	}
...
struct stmmac_desc_ops ndesc_ops = {
	...
	.rx_tstamp_available = ndesc_rx_tstamp_available,
