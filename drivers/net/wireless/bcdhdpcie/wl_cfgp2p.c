...
wl_cfgp2p_set_firm_p2p(struct bcm_cfg80211 *cfg)
{
	...
	struct ether_addr_bcm null_eth_addr = { { 0, 0, 0, 0, 0, 0 } };
	s32 ret = BCME_OK;
	s32 val = 0;
 ...
 */
s32
wl_cfgp2p_ifadd(struct bcm_cfg80211 *cfg, struct ether_addr_bcm *mac, u8 if_type,
            chanspec_t chspec)
{
	wl_p2p_if_t ifreq;
	s32 err;
 ...
 */
s32
wl_cfgp2p_ifdisable(struct bcm_cfg80211 *cfg, struct ether_addr_bcm *mac)
{
	s32 ret;
 ...
 */
s32
wl_cfgp2p_ifdel(struct bcm_cfg80211 *cfg, struct ether_addr_bcm *mac)
{
	s32 ret;
	struct net_device *netdev = bcmcfg_to_prmry_ndev(cfg);
 ...
 */
s32
wl_cfgp2p_ifchange(struct bcm_cfg80211 *cfg, struct ether_addr_bcm *mac, u8 if_type,
 ...
 */
s32
wl_cfgp2p_ifidx(struct bcm_cfg80211 *cfg, struct ether_addr_bcm *mac, s32 *index)
{
	s32 ret;
	u8 getbuf[64];
...
wl_cfgp2p_escan(struct bcm_cfg80211 *cfg, struct net_device *dev, u16 active,
	...
	s32 search_state, u16 action, u32 bssidx, struct ether_addr_bcm *tx_dst_addr,
...
wl_cfgp2p_act_frm_search(struct bcm_cfg80211 *cfg, struct net_device *ndev,
	s32 bssidx, s32 channel, struct ether_addr_bcm *tx_dst_addr)
{
	s32 ret = 0;
	u32 chan_cnt = 0;
 ...
 */
void
wl_cfgp2p_generate_bss_mac(struct bcm_cfg80211 *cfg, struct ether_addr_bcm *primary_addr)
{
	struct ether_addr_bcm *mac_addr = wl_to_p2p_bss_macaddr(cfg, P2PAPI_BSSCFG_DEVICE);
	struct ether_addr_bcm *int_addr;

	memcpy(mac_addr, primary_addr, sizeof(struct ether_addr_bcm));
	...
	memcpy(int_addr, mac_addr, sizeof(struct ether_addr_bcm));
...
void
wl_cfg80211_change_ifaddr(u8* buf, struct ether_addr_bcm *p2p_int_addr, u8 element_id)
{
