 ...
 */
static s32 wl_frame_get_mgmt(u16 fc, const struct ether_addr_bcm *da,
	const struct ether_addr_bcm *sa, const struct ether_addr_bcm *bssid,
...
static int wl_cfg80211_delayed_roam(struct bcm_cfg80211 *cfg, struct net_device *ndev,
	const struct ether_addr_bcm *bssid);
 ...
 */
int
wl_cfg80211_ether_atoe(const char *a, struct ether_addr_bcm *n)
{
	char *c = NULL;
	int count = 0;
	...
	struct vif_params *params)
{
	...
	struct ether_addr_bcm primary_mac;
...
#endif
{
	...
	RETURN_EIO_IF_NOT_UP(cfg);
	if (wl_get_mode_by_netdev(cfg, dev) == WL_MODE_AP) {
		err = wldev_iovar_getbuf(dev, "sta_info", (struct ether_addr_bcm *)mac,
		...
		WL_INFORM(("STA %s : idle time : %d sec, connected time :%d ms\n",
			bcm_ether_ntoa((const struct ether_addr_bcm *)mac, eabuf), sinfo->inactive_time,
			sta->idle * 1000));
#endif
	...
	if (ieee80211_is_mgmt(mgmt->frame_control)) {
		if (ieee80211_is_probe_resp(mgmt->frame_control)) {
			...
			char mac_buf[MAX_NUM_OF_ASSOCIATED_DEV *
				sizeof(struct ether_addr_bcm) + sizeof(uint)] = {0};
			int num_associated = 0;
			struct maclist *assoc_maclist = (struct maclist *)mac_buf;
			if (!bcmp((const uint8 *)BSSID_BROADCAST,
				(const struct ether_addr_bcm *)mgmt->da, ETHER_ADDR_LEN)) {
			...
			WL_ERR(("Disconnect STA : %s scb_val.val %d\n",
				bcm_ether_ntoa((const struct ether_addr_bcm *)mgmt->da, eabuf),
				scb_val.val));
...
#endif /* (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 19, 0)) */
{
	...
	WL_ERR(("Disconnect STA : %s scb_val.val %d\n",
		bcm_ether_ntoa((const struct ether_addr_bcm *)mac_addr, eabuf),
		scb_val.val));
	...
	if (wl_get_mode_by_netdev(cfg, ndev) == WL_MODE_AP) {
		...
		if (wl_is_linkup(cfg, e, ndev)) {
			...
			if (wl_get_drv_status(cfg, CONNECTED, ndev)) {
				...
				s32 reason = 0;
				struct ether_addr_bcm bssid_dongle;
				struct ether_addr_bcm bssid_null = {{0, 0, 0, 0, 0, 0}};
...
static s32
wl_frame_get_mgmt(u16 fc, const struct ether_addr_bcm *da,
	const struct ether_addr_bcm *sa, const struct ether_addr_bcm *bssid,
	u8 **pheader, u32 *body_len, u8 *pbody)
{
...
s32 wl_cfg80211_get_p2p_dev_addr(struct net_device *net, struct ether_addr_bcm *p2pdev_addr)
{
	struct bcm_cfg80211 *cfg = g_bcm_cfg;
	struct ether_addr_bcm primary_mac;
	if (!cfg->p2p)
...
void get_primary_mac(struct bcm_cfg80211 *cfg, struct ether_addr_bcm *mac)
{
	...
	const struct cfg80211_acl_data *acl)
{
	...
	list = (struct maclist*)kmalloc(sizeof(int) +
		sizeof(struct ether_addr_bcm) * macnum, GFP_KERNEL);
	if (!list) {
		WL_ERR(("%s : failed to allocate memory\n", __FUNCTION__));
		return -1;
	}
