...
#endif
#ifdef WRITE_MACADDR
extern int dhd_write_macaddr(struct ether_addr_bcm *mac);
#else
static inline int dhd_write_macaddr(struct ether_addr_bcm *mac) { return 0; }
#endif
...
#ifdef WLTDLS
int _dhd_tdls_enable(dhd_pub_t *dhd, bool tdls_on, bool auto_on, struct ether_addr_bcm *mac)
{
	char iovbuf[WLC_IOCTL_SMLEN];
	uint32 tdls = tdls_on;
	int ret = 0;
...
int dhd_tdls_enable(struct net_device *dev, bool tdls_on, bool auto_on, struct ether_addr_bcm *mac)
{
	dhd_info_t *dhd = DHD_DEV_INFO(dev);
	int ret = 0;
	if (dhd)
...
int
dhd_dev_rtt_cancel_cfg(struct net_device *dev, struct ether_addr_bcm *mac_list, int mac_cnt)
{
	dhd_info_t *dhd = *(dhd_info_t **)netdev_priv(dev);
