...
typedef struct wl_bsscfg {
	...
	uint32  BSS;
	uint32  phytest_on;
	struct ether_addr_bcm   prev_BSSID;
	struct ether_addr_bcm   BSSID;
...
typedef struct wl_if_add {
	...
	uint32  if_flags;
	uint32  ap;
	struct ether_addr_bcm   mac_addr;
	uint32  wlc_index;
} wl_if_add_t;
...
typedef struct wl_scan_params {
	...
	struct ether_addr_bcm bssid;	/**< default: bcast */
...
typedef struct wl_assoc_params {
	struct ether_addr_bcm bssid;	/**< 00:00:00:00:00:00: broadcast scan */
...
typedef struct wl_wsec_key {
	...
	uint32		pad_5[2];
	struct ether_addr_bcm ea;		/**< per station */
} wl_wsec_key_t;
...
typedef struct wl_assoc_info {
	...
	struct ether_addr_bcm reassoc_bssid; /* used in reassoc's */
...
typedef struct {
	...
	struct ether_addr_bcm	ea;		/**< Station address */
...
typedef struct maclist {
	...
	struct ether_addr_bcm ea[1];	/**< variable length array of MAC addresses */
} maclist_t;
...
typedef struct {
	struct	ether_addr_bcm ea;	/**< MAC Address */
...
typedef struct wl_ota_test_vector {
	...
	struct ether_addr_bcm sync_mac;				/**< macaddress for sync pkt */
	struct ether_addr_bcm tx_mac;				/**< macaddress for tx */
	struct ether_addr_bcm rx_mac;				/**< macaddress for rx */
...
typedef struct wl_ota_test_status {
	...
	int8 sync_fail_action;		/**< sync fail action */
	struct ether_addr_bcm sync_mac;	/**< macaddress for sync pkt */
	struct ether_addr_bcm tx_mac;	/**< tx mac address */
	struct ether_addr_bcm rx_mac;	/**< rx mac address */
...
typedef struct wl_mtcpkeep_alive_conn_pkt {
	struct ether_addr_bcm saddr;		/* src mac address */
	struct ether_addr_bcm daddr;		/* dst mac address */
...
typedef struct {
	...
	struct ether_addr_bcm addr;		/* addr of the peer */
} wl_anqpo_peer_t;
...
typedef struct {
	...
	struct ether_addr_bcm bssid[1];	/* max ANQPO_MAX_IGNORE_BSSID */
...
typedef struct wl_pkteng {
	...
	struct ether_addr_bcm dest;		/* Destination address */
	struct ether_addr_bcm src;		/* Source address */
} wl_pkteng_t;
...
typedef BWL_PRE_PACKED_STRUCT struct {
	...
	struct ether_addr_bcm ea[WL_IOV_MAC_PARAM_LEN];
...
typedef BWL_PRE_PACKED_STRUCT struct {
	struct ether_addr_bcm	station_address;	/* The station MAC address */
...
typedef BWL_PRE_PACKED_STRUCT	struct {
	uint16			version;
	struct	ether_addr_bcm ea;	/* peer MAC address */
...
typedef struct trf_mgmt_filter {
	struct ether_addr_bcm           dst_ether_addr;         /* His L2 address */
...
typedef struct wl_tfs_fset {
	struct ether_addr_bcm ea;		/* Address of AP/STA involved with this filter set */
...
typedef struct wl_dms_desc {
	...
	uint8 dms_id;
	uint8 tclas_proc;
	uint8 mac_len;		/* length of all ether_addr_bcm in data array, 0 if STA */
	uint8 tclas_len;	/* length of all wl_tclas_t in data array */
	uint8 data[1];		/* VLA of 'ether_addr_bcm' and 'wl_tclas_t' (in this order ) */
} wl_dms_desc_t;
...
typedef struct wl_relmcast_client {
	...
	struct ether_addr_bcm addr;		/* mac address of RMC client */
...
typedef struct wl_rmc_entry {
	 ...
	 */
	int8    flag;
	struct ether_addr_bcm addr;		/* multi-cast group mac address */
} wl_rmc_entry_t;
...
typedef struct wl_rmc_trans_elem {
	struct ether_addr_bcm tr_mac;	/* transmitter mac */
	struct ether_addr_bcm ar_mac;	/* ar mac */
...
typedef BWL_PRE_PACKED_STRUCT struct wl_proxd_params_tof_method {
	...
	struct ether_addr_bcm tgt_mac;	/* target mac addr for TOF method */
...
typedef BWL_PRE_PACKED_STRUCT struct wl_proxd_collect_header {
	...
	struct ether_addr_bcm	loaclMacAddr;		/* local mac address */
	struct ether_addr_bcm	remoteMacAddr;		/* remote mac address */
...
typedef struct nan_ranging_config {
	...
	struct ether_addr_bcm allow_mac;	/* peer initiated ranging: the allowed peer mac
...
typedef struct nan_ranging_result {
	...
	struct ether_addr_bcm ea;		/* initiator MAC address */
	...
	int32 rtt_var;			/* standard deviation in 10th of ns of RTTs measured.
					 ...
					 */
	struct ether_addr_bcm tgtea;	/* target MAC address */
...
typedef struct wl_scanmac_config {
	struct ether_addr_bcm mac;	/* 6 bytes of MAC address or MAC prefix (i.e. OUI) */
	struct ether_addr_bcm random_mask;	/* randomized bits on each scan */
...
typedef struct wl_proxd_status_iovar {
	...
	struct ether_addr_bcm	peer;				/* peer mac address */
...
typedef struct framereq {
	...
	uint16 random_int;
	uint16 dur;
	struct ether_addr_bcm ta;
	uint16 reps;
} framereq_t;

typedef struct statreq {
	struct ether_addr_bcm da;
	struct ether_addr_bcm peer;
	uint16 random_int;
	uint16 dur;
	uint8 group_id;
	uint16 reps;
...
typedef struct {
	uint16 ver;		/* version */
	struct ether_addr_bcm addr;	/* STA MAC addr */
...
typedef struct wl_staprio_cfg {
	struct ether_addr_bcm ea;	/* mac addr */
...
typedef struct wl_taf_define {
	struct ether_addr_bcm ea;	/* STA MAC or 0xFF... */
	uint16 version;         /* version */
...
typedef struct wl_interface_create {
	...
	struct	ether_addr_bcm   mac_addr;	/* Optional Mac address */
} wl_interface_create_t;

typedef struct wl_interface_info {
	uint16	ver;			/* version of this struct */
	struct ether_addr_bcm    mac_addr;	/* MAC address of the interface */
