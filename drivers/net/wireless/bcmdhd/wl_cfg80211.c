 ...
 *
 * $Id: wl_cfg80211.c 631435 2016-04-14 12:14:33Z $
 */
/* */
#include <typedefs.h>
#include <linuxver.h>
 ...
 */
static s32 wl_frame_get_mgmt(u16 fc, const struct ether_addr_wl *da,
	const struct ether_addr_wl *sa, const struct ether_addr_wl *bssid,
...
static int wl_cfg80211_delayed_roam(struct bcm_cfg80211 *cfg, struct net_device *ndev,
	const struct ether_addr_wl *bssid);
 ...
 */
int
wl_cfg80211_ether_atoe(const char *a, struct ether_addr_wl *n)
{
	char *c = NULL;
	int count = 0;
	...
	struct vif_params *params)
{
	...
	struct ether_addr_wl primary_mac;
#ifdef PROP_TXSTATUS_VSDB
#if defined(BCMSDIO)
	s32 up = 1;
	dhd_pub_t *dhd;
...
#endif
{
	...
	RETURN_EIO_IF_NOT_UP(cfg);
	if (wl_get_mode_by_netdev(cfg, dev) == WL_MODE_AP) {
		err = wldev_iovar_getbuf(dev, "sta_info", (struct ether_addr_wl *)mac,
		...
		WL_INFO(("STA %s : idle time : %d sec, connected time :%d ms\n",
			bcm_ether_ntoa((const struct ether_addr_wl *)mac, eabuf), sinfo->inactive_time,
			sta->idle * 1000));
#endif
	...
	if (ieee80211_is_mgmt(mgmt->frame_control)) {
		if (ieee80211_is_probe_resp(mgmt->frame_control)) {
			...
			char mac_buf[MAX_NUM_OF_ASSOCIATED_DEV *
				sizeof(struct ether_addr_wl) + sizeof(uint)] = {0};
			int num_associated = 0;
			struct maclist *assoc_maclist = (struct maclist *)mac_buf;
			if (!bcmp((const uint8 *)BSSID_BROADCAST,
				(const struct ether_addr_wl *)mgmt->da, ETHER_ADDR_LEN)) {
			...
			WL_ERR(("Disconnect STA : %s scb_val.val %d\n",
				bcm_ether_ntoa((const struct ether_addr_wl *)mgmt->da, eabuf),
				scb_val.val));
...
#endif
{
	...
	WL_ERR(("Disconnect STA : %s scb_val.val %d\n",
		bcm_ether_ntoa((const struct ether_addr_wl *)mac_addr, eabuf),
		scb_val.val));
...
static s32
wl_frame_get_mgmt(u16 fc, const struct ether_addr_wl *da,
	const struct ether_addr_wl *sa, const struct ether_addr_wl *bssid,
	u8 **pheader, u32 *body_len, u8 *pbody)
{
...
s32 wl_cfg80211_get_p2p_dev_addr(struct net_device *net, struct ether_addr_wl *p2pdev_addr)
{
	struct bcm_cfg80211 *cfg = g_bcm_cfg;
	struct ether_addr_wl p2pif_addr;
	struct ether_addr_wl primary_mac;
	if (!cfg->p2p)
		return -1;
	if (!p2p_is_on(cfg)) {
...
#endif
{
	...
	case NL80211_TDLS_SETUP:
		/* auto mode on */
		ret = dhd_tdls_enable(dev, true, true, (struct ether_addr_wl *)peer);
		if (ret < 0)
			return ret;
		break;
	case NL80211_TDLS_TEARDOWN:
		...
		/* auto mode off */
		ret = dhd_tdls_enable(dev, true, false, (struct ether_addr_wl *)peer);
		if (ret < 0)
			return ret;
		break;
	default:
...
void get_primary_mac(struct bcm_cfg80211 *cfg, struct ether_addr_wl *mac)
{
	...
	const struct cfg80211_acl_data *acl)
{
	...
	list = (struct maclist*)kmalloc(sizeof(int) +
		sizeof(struct ether_addr_wl) * macnum, GFP_KERNEL);
	if (!list) {
