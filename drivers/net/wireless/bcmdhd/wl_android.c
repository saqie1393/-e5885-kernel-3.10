...
#ifdef WL_CFG80211
int wl_cfg80211_get_p2p_dev_addr(struct net_device *net, struct ether_addr_wl *p2pdev_addr);
...
static int wl_android_get_p2p_dev_addr(struct net_device *ndev, char *command, int total_len)
{
	...
	ret = wl_cfg80211_get_p2p_dev_addr(ndev, (struct ether_addr_wl*)command);
	if (ret)
		return 0;
	bytes_written = sizeof(struct ether_addr_wl);
	return bytes_written;
}
...
wl_android_set_ap_mac_list(struct net_device *dev, int macmode, struct maclist *maclist)
{
	...
	if (macmode != MACLIST_MODE_DISABLED) {
		...
		if ((ret = wldev_ioctl(dev, WLC_SET_MACLIST, maclist,
			sizeof(int) + sizeof(struct ether_addr_wl) * maclist->count, true)) != 0) {
