...
dhd_custom_get_mac_address(void *adapter, unsigned char *buf)
{
	...
	/* EXAMPLE code */
	{
		struct ether_addr_wl ea_example = {{0x00, 0x11, 0x22, 0x33, 0x44, 0xFF}};
		bcopy((char *)&ea_example, buf, sizeof(struct ether_addr_wl));
	}
...
{
#if (defined(CUSTOMER_HW2) && (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 39)) && (FEATURE_ON != MBB_TELE_WIFI))
