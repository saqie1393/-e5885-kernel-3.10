 ...
 *
 * $Id: dhd_linux.c 631416 2016-04-14 11:29:20Z $
 */
...
#endif
#ifdef WRITE_MACADDR
extern int dhd_write_macaddr(struct ether_addr_wl *mac);
#else
static inline int dhd_write_macaddr(struct ether_addr_wl *mac) { return 0; }
#endif
...
#ifdef WLTDLS
int _dhd_tdls_enable(dhd_pub_t *dhd, bool tdls_on, bool auto_on, struct ether_addr_wl *mac)
{
	char iovbuf[WLC_IOCTL_SMLEN];
	uint32 tdls = tdls_on;
	int ret = 0;
...
int dhd_tdls_enable(struct net_device *dev, bool tdls_on, bool auto_on, struct ether_addr_wl *mac)
{
	dhd_info_t *dhd = *(dhd_info_t **)netdev_priv(dev);
	int ret = 0;
	if (dhd)
