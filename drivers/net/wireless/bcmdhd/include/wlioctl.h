...
typedef struct wl_bsscfg {
	...
	uint32  BSS;
	uint32  phytest_on;
	struct ether_addr_wl   prev_BSSID;
	struct ether_addr_wl   BSSID;
...
typedef struct wl_scan_params {
	...
	struct ether_addr_wl bssid;	/* default: bcast */
...
typedef struct wl_assoc_params {
	struct ether_addr_wl bssid;	/* 00:00:00:00:00:00: broadcast scan */
...
typedef struct wl_wsec_key {
	...
	uint32		pad_5[2];
	struct ether_addr_wl ea;		/* per station */
} wl_wsec_key_t;
...
typedef struct wl_assoc_info {
	...
	struct ether_addr_wl reassoc_bssid; /* used in reassoc's */
...
typedef struct wl_plc_nodelist {
	...
	struct _node {
		struct ether_addr_wl ea;	/* Node ether address */
...
typedef struct wl_plc_params {
	...
	struct	ether_addr_wl node_ea;	/* Node ether address */
...
typedef struct {
	...
	struct ether_addr_wl	ea;		/* Station address */
...
struct maclist {
	...
	struct ether_addr_wl ea[1];	/* variable length array of MAC addresses */
};
...
typedef struct {
	struct	ether_addr_wl ea;	/* MAC Address */
...
typedef struct wl_ota_test_vector {
	...
	struct ether_addr_wl sync_mac;				/* macaddress for sync pkt */
	struct ether_addr_wl tx_mac;				/* macaddress for tx */
	struct ether_addr_wl rx_mac;				/* macaddress for rx */
...
typedef struct wl_ota_test_status {
	...
	int8 sync_fail_action;		/* sync fail action */
	struct ether_addr_wl sync_mac;	/* macaddress for sync pkt */
	struct ether_addr_wl tx_mac;	/* tx mac address */
	struct ether_addr_wl rx_mac;	/* rx mac address */
...
typedef struct wl_mtcpkeep_alive_conn_pkt {
	struct ether_addr_wl saddr;		/* src mac address */
	struct ether_addr_wl daddr;		/* dst mac address */
...
typedef struct {
	...
	struct ether_addr_wl addr;		/* addr of the peer */
} wl_anqpo_peer_t;
...
typedef struct {
	...
	struct ether_addr_wl bssid[1];	/* max ANQPO_MAX_IGNORE_BSSID */
...
typedef struct wl_pkteng {
	...
	struct ether_addr_wl dest;		/* Destination address */
	struct ether_addr_wl src;		/* Source address */
} wl_pkteng_t;
...
typedef struct {
	...
	struct ether_addr_wl ea[WL_IOV_MAC_PARAM_LEN];
} wl_iov_mac_params_t;
...
typedef BWL_PRE_PACKED_STRUCT struct {
	struct ether_addr_wl	station_address;	/* The station MAC address */
...
typedef BWL_PRE_PACKED_STRUCT	struct {
	uint16			version;
	struct	ether_addr_wl ea;	/* peer MAC address */
...
typedef struct trf_mgmt_filter {
	struct ether_addr_wl           dst_ether_addr;         /* His L2 address */
...
typedef struct wl_tfs_fset {
	struct ether_addr_wl ea;		/* Address of AP/STA involved with this filter set */
...
typedef struct wl_dms_desc {
	...
	uint8 dms_id;
	uint8 tclas_proc;
	uint8 mac_len;		/* length of all ether_addr_wl in data array, 0 if STA */
	uint8 tclas_len;	/* length of all wl_tclas_t in data array */
	uint8 data[1];		/* VLA of 'ether_addr_wl' and 'wl_tclas_t' (in this order ) */
} wl_dms_desc_t;
...
typedef struct wl_relmcast_client {
	...
	struct ether_addr_wl addr;		/* mac address of RMC client */
...
typedef struct wl_rmc_entry {
	 ...
	 */
	int8    flag;
	struct ether_addr_wl addr;		/* multi-cast group mac address */
} wl_rmc_entry_t;
...
typedef struct wl_rmc_trans_info {
	struct ether_addr_wl addr;		/* transmitter mac */
...
typedef struct wl_rmc_grp_entry {
	struct ether_addr_wl   mcaddr;	/* multi-cast group mac */
	struct ether_addr_wl   ar;		/* active receiver for the group */
	wl_rmc_trans_info_t tr_info[WL_RMC_MAX_TRS_PER_GROUP];
} wl_rmc_grp_entry_t;

/* RMC ACKALL Table */
typedef struct wl_rmc_ackall_entry {
	struct ether_addr_wl   ar;		/* active receiver for the entry */
...
typedef BWL_PRE_PACKED_STRUCT struct wl_proxd_params_tof_method {
	...
	struct ether_addr_wl tgt_mac;	/* target mac addr for TOF method */
...
typedef BWL_PRE_PACKED_STRUCT struct wl_proxd_collect_header {
	...
	struct ether_addr_wl	loaclMacAddr;		/* local mac address */
	struct ether_addr_wl	remoteMacAddr;		/* remote mac address */
...
typedef struct nan_ranging_config {
	...
	struct ether_addr_wl allow_mac;	/* peer initiated ranging: the allowed peer mac
...
typedef struct nan_ranging_result {
	...
	struct ether_addr_wl ea;		/* initiator MAC address */
	...
	int32 rtt_var;			/* standard deviation in 10th of ns of RTTs measured.
					 ...
					 */
	struct ether_addr_wl tgtea;	/* target MAC address */
...
typedef struct wl_proxd_status_iovar {
	...
	struct ether_addr_wl	peer;				/* peer mac address */
...
typedef struct framereq {
	...
	uint16 random_int;
	uint16 dur;
	struct ether_addr_wl ta;
	uint16 reps;
} framereq_t;

typedef struct statreq {
	struct ether_addr_wl da;
	struct ether_addr_wl peer;
	uint16 random_int;
	uint16 dur;
	uint8 group_id;
	uint16 reps;
...
typedef struct wl_staprio_cfg {
	struct ether_addr_wl ea;	/* mac addr */
