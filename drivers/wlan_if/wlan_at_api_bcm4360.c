...
#define       WIFI_RF_PARA_LEN                   (2)   /* wifi射频每组中参数占用两个字节 */
#define       WIFI_2G_RF_NAME_LEN                (8)   /* 2Gwifi射频每组值对应的名称 */
#define       WIFI_2G_RF_GROUPS_DATA_NUMBER      (6)   /* 2Gwifi射频全部组值的参数个数 */
#define       WIFI_2G_RF_GROUPS_DATA_NAME_LEN    (9)   /* 2Gwifi射频组值的名称长度 */
#define       WIFI_2G_RF_GROUPS_DATA_VALUE_LEN   (6)   /* 2Gwifi射频组值的参数长度 */
#define       WIFI_RF_GROUP_DATA_VALUE_TOTAL_LEN (30)  /* 定义wifi射频组值的缓冲区长度 */
...
{        
    ...
    { 
        ...
        OSA_SNPRINTF(acCmdValueStr, sizeof(acCmdValueStr), 
                 "/system/bin/insmod /system/bin/bcm4352/driver/wl.ko"\
                 " nvram_path=/system/bin/bcm4352/nv/B610_AT.nvm");
...
{         
    static char sChannels24G[] = "7,9,11";
...
/*****************************************************************************
 ...
 功能描述  : 2.4G射频参数查询封装函数
 输入参数  : NA
 输出参数  : strBuf               用于存放获取到的射频参数
 返 回 值  : WLAN_SUCCESS         获取射频参数成功
             AT_RETURN_FAILURE    获取射频参数失败
 其他说明  : 
*****************************************************************************/
STATIC int32 ATGetWifi2GPavars(WLAN_AT_BUFFER_STRU *strBuf)
{
    ...
    int8 foramtdata2g[ANT_2G_NUM][WIFI_2G_RF_NAME_LEN] = {"pa2ga0=", "pa2ga1="};
    int8 realdata2g[WIFI_2G_RF_GROUPS_DATA_NUMBER][WIFI_2G_RF_GROUPS_DATA_NAME_LEN]   = {"pa2gw0a0", "pa2gw1a0", "pa2gw2a0", "pa2gw0a1", "pa2gw1a1", "pa2gw2a1"};
    int32 i = 0 , j = 0;
    char *temp = NULL;
    VOS_INT32  ulRet = 0 ;
    ...
    /* pavars命令执行结果 */
    ulRet = wlan_read_file("/var/tmpfile", filebuff, sizeof(filebuff));
    ...
    /* 组装2.4g参数值，与5g显示格式一致 */
    for ( i = 0, j = 0 ; i < ANT_2G_NUM ; i++ )
    {
        strncat(strBuf->buf, foramtdata2g[i], (sizeof(foramtdata2g[i]) - 1));
        ...
        temp = temp + WIFI_2G_RF_GROUPS_DATA_NAME_LEN;
        strncat(strBuf->buf, temp, WIFI_2G_RF_GROUPS_DATA_VALUE_LEN);
...
STATIC int32 ATSetWifi2GPavars(WLAN_AT_PAVARS2G_STRU *strBuf)
{
    ...
    for (i = 0 ; i < WIFI_2G_RF_GROUPS_NUMBER; i++)
    {
        OSA_SNPRINTF(wl_temp, WIFI_CMD_MAX_SIZE, "pa2gw%da%d=%s ", i, strBuf->ANT_Index, strBuf->data[i]);
        strncat(wl_cmd, wl_temp, (sizeof(wl_temp) - 1));
        memset(wl_temp, 0, WIFI_CMD_MAX_SIZE);
    }
...
STATIC int32 ATGetWifi5GPavars(WLAN_AT_BUFFER_STRU *strBuf)
{
    ...
    strncpy(strBuf->buf, temp, sizeof(strBuf->buf));
...
STATIC int32 ATSetWifi5GPavars(WLAN_AT_PAVARS5G_STRU *strBuf)
{
    ...
    for (i = 0 ; i < WIFI_5G_RF_GROUPS_NUMBER; i++)
    {
...
/*****************************************************************************
 ...
 输入参数  : type                  nv的id
             len                   nv的16进制形式的数据结构大小         
 输出参数  : buf                   传入空字符串指针用于存放获取的nv值
 返 回 值  : WLAN_SUCCESS          获取nv值成功
             AT_RETURN_FAILURE     获取nv值失败
             WLAN_PARA_ERR         传入参数有错误
 其他说明  : 
*****************************************************************************/
...
{
    int8 wl_cmd[WIFI_CMD_MAX_SIZE] = {0};
    int8 temp[WIFI_RF_GROUP_DATA_VALUE_TOTAL_LEN] = {0};
    unsigned short nv_id_2g = NV_ID_WIFI_2G_RFCAL;
    unsigned short nv_id_5g = NV_ID_WIFI_5G_RFCAL;
    unsigned int nv_wifi_2g_len = 0;
    unsigned int nv_wifi_5g_len = 0;
    ...
    for (i = 0; i < ANT_2G_NUM; i++)
    {
        if (0 != *(buf_2g + i * ANT_2G_CAL_NV_VALUE_BYTE_NUM))
        {
            ...
            for (j = 0; j < ANT_2G_CAL_NV_VALUE_BYTE_NUM / WIFI_RF_PARA_LEN; j++)
            {
                memset(temp, 0, sizeof(temp));
                OSA_SNPRINTF(temp, WIFI_CMD_MAX_SIZE, "pa2gw%da%d=0x%02x%02x ", j, i, \
                             * (buf_2g + i * ANT_2G_CAL_NV_VALUE_BYTE_NUM + j * WIFI_RF_PARA_LEN), \
                             * (buf_2g + i * ANT_2G_CAL_NV_VALUE_BYTE_NUM + j * WIFI_RF_PARA_LEN + 1));
                strncat(wl_cmd, temp, (sizeof(temp) - 1));
            }
    ...
    for (i = 0; i < ANT_5G_NUM; i++)
    {
        if (0 != *(buf_5g + i * ANT_5G_CAL_NV_VALUE_BYTE_NUM ))
        {
            ...
            for (j = 0; j < ANT_5G_CAL_NV_VALUE_BYTE_NUM / WIFI_RF_PARA_LEN; j++)
            {
                memset(temp, 0, sizeof(temp));
                OSA_SNPRINTF(temp, WIFI_CMD_MAX_SIZE, "0x%02x%02x,", \
                             * (buf_5g + i * ANT_5G_CAL_NV_VALUE_BYTE_NUM + j * WIFI_RF_PARA_LEN), \
                             * (buf_5g + i * ANT_5G_CAL_NV_VALUE_BYTE_NUM + j * WIFI_RF_PARA_LEN + 1));
                strncat(wl_cmd, temp, (sizeof(temp) - 1));
            }
