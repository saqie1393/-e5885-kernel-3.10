...
extern WLAN_CHIP_OPS hi1151_ops;
#endif
...
===========================================================================*/
/*****************************************************************************
 函数名称  : int32 WlanATSetWifiDebug(WLAN_AT_DEBUG_TYPE debug_type, uint32 vaule1, uint32 *valule2)
 功能描述  : 读写WIFI寄存器的相关信息。也可以支持WIFI芯片维测配置。
 输入参数  : debug操作类型，value1,*valu2
 输出参数  : NA
 返 回 值  : int32
*****************************************************************************/
int32 WlanATSetWifiDebug(WLAN_AT_DEBUG_TYPE debug_type, uint32 vaule1, uint32 *valule2)
{
    /* 判断g_cur_chip和g_wlan_ops[g_cur_chip]有效性 */
    ASSERT_CHIP_IDX_VALID(g_cur_chip, AT_RETURN_FAILURE);
    ASSERT_NULL_POINTER(g_wlan_ops[g_cur_chip]->WlanATSetWifiDebug, AT_RETURN_FAILURE);
    return g_wlan_ops[g_cur_chip]->WlanATSetWifiDebug(debug_type, vaule1, valule2);
}
...
#ifdef FEATURE_HUAWEI_MBB_HI1151
    wlan_at_reg_chip(hi1151, &hi1151_ops);
    g_cur_chip = hi1151;
#endif
...
EXPORT_SYMBOL(WlanATSetWifiDebug);
