...
unsigned int wifi_gpio_addr = GPIO_2_2;
int g_wifi_power_state = 1;
...
#if defined(MBB_WIFI_RF_PATH_SWITCH)
#if defined(BSP_CONFIG_BOARD_E5885Ls_93a)

/* 串改并相关GPIO */
unsigned int wlan_saw_sel0_gpio = GPIO_SLEEP_1_5;
unsigned int wlan_saw_sel1_gpio = GPIO_SLEEP_1_6;
#else

/* 串改并相关GPIO, 0xFFFF表示无效，不采用串改并方案 */
unsigned int wlan_saw_sel0_gpio = 0xFFFF;
unsigned int wlan_saw_sel1_gpio = 0xFFFF;
#endif
#else
unsigned int wlan_saw_sel0_gpio = 0xFFFF;
unsigned int wlan_saw_sel1_gpio = 0xFFFF;
...
/* Hi1151 wifi芯片供电相关GPIO */
#if defined(BSP_CONFIG_BOARD_E5)
#ifdef FEATURE_HUAWEI_MBB_HI1151
#if defined(BSP_CONFIG_BOARD_E5885Ls_93a)

/* WiFi芯片上电相关GPIO */
#define HISI_DCDC_3V3_EN    GPIO_5_5
#define HISI_DCDC_1V1_EN    GPIO_5_6
#define HISI_PMU_PWRON      GPIO_5_3

/* 低功耗相关GPIO */
unsigned int wlan_wakehost_gpio    = GPIO_0_7;
unsigned int wlan_sleep_gpio       = GPIO_6_5;
unsigned int wlan_pciereset_gpio   = GPIO_6_4;
#else

/* WiFi芯片上电相关GPIO */
#define HISI_DCDC_3V3_EN    GPIO_1_1
#define HISI_DCDC_1V1_EN    GPIO_2_3
#define HISI_PMU_PWRON      GPIO_2_2

/* 低功耗相关GPIO */
unsigned int wlan_wakehost_gpio    = GPIO_0_7;
unsigned int wlan_sleep_gpio       = GPIO_1_2;
unsigned int wlan_pciereset_gpio   = GPIO_1_3;

#endif

#define WiFi_POWER_3V3      "wifi_power_3v3"
#define WiFi_POWER_1V1      "wifi_power_1v1"
#define WiFi_POWER_PWRON    "wifi_power_pmu_on"
#define HISI_PWR_DELAY      20   /* 延时时间20ms */

#define PCIE_BALONG_PCIE0_CLASS  (0x0604)   /* 厂商ID */
#define PCIE_1151_CLASS          (0x0280)   /* 设备ID */
#if ((defined(CONFIG_BALONG_SPE) || defined(CONFIG_BALONG_SPE_MODULE)))
#define MAX_WLAN_PORT_NUM   (2)       /* wlan接口最大个数 */
#define MAX_SPE_PORT_NUM    (8)       /* SPE最大端口号为8 */

typedef struct _wifi_spe_port_info
{
    unsigned int portNo;
    unsigned int portStatus;
} WIFI_SPE_PORT_INFO;

/* 保存wifi电源状态的全局变量,0:关闭,1:打开 */
int g_wifi_power_state = 1;

/* 保存SPE端口的全局变量 */
WIFI_SPE_PORT_INFO g_st_hisi_spe_alloc_port[MAX_WLAN_PORT_NUM] = {{0,0}, {0,0}};
...
typedef enum
{
    ...
    SPE_PORT_STATUS_BUTT
}SPE_PORT_STATUS_ENUM;
#endif
#endif
#endif
...
/* 保存滤波器当前状态 */
static int g_wifi_rf_path_status = RF_PATH_NOB40;
#endif
...
#if (MBB_WIFI_RF_PATH_SWITCH == FEATURE_ON)
static int wifi_get_rf_path_status();
#endif

#if defined(BSP_CONFIG_BOARD_E5)
#if (defined(FEATURE_HUAWEI_MBB_BCM4356) || defined(FEATURE_HUAWEI_MBB_HI1151))
...
#define EXTC_INSERT (0x0005)
#define EXTC_REMOVE (0x0004)
uint8_t pcie_is_suspend_due_to_extchg = 0;
extern uint8_t chg_stm_get_extchg_online_st(void);
extern void extchg_notify_pcie_powersave(struct notifier_block *nb, unsigned long event, void *v);
extern struct blocking_notifier_head g_extchg_pciesave_notifier_list;
struct notifier_block extchg_notify_pcie_powersave_block = {
    .notifier_call = extchg_notify_pcie_powersave,
};
#endif
...
/***********************************************************************************
 ...
 Return:            NA                  
 ************************************************************************************/
static irqreturn_t wifi_timer_callback(void *data)
{
    WIFI_USER_EVENT event;
    unsigned int indata = (unsigned int)data;
    WIFI_USER_TMR_ID timer_id = (WIFI_USER_TMR_ID)indata;
...
#if (MBB_WIFI_RF_PATH_SWITCH == FEATURE_ON)
        case WIFI_SWITCH_SAW_SET:
        {
            (void)sscanf(buf, WIFI_SWITCH_SAW_SET_STR, &tmp_val);
            if (RF_PATH_NOB40 == tmp_val)
            {
                (void)wifi_switch_saw_nob40();
            }
            else
            {
                (void)wifi_switch_saw_b40();
            }
            break;
        }
#endif
...
unsigned int wlan_request_wakeup_irq(irq_handler_t handler, const char *devname, void *dev) /*lint !e10 !e578 !e101 */
{
    ...
    const unsigned long irq_flags = (IRQF_NO_SUSPEND | IRQF_SHARED | IRQF_TRIGGER_RISING);//(IRQF_TRIGGER_RISING);liuzixi
...
/*****************************************************************************
函数名称  : int wifi_switch_saw_nob40()
功能描述  : 切换到非Band40状态对应滤波器
输入参数  : NA
输出参数  : NA
返 回 值  : 返回切换结果,0:成功,-1:失败
*****************************************************************************/
int wifi_switch_saw_nob40(void)
{
    int32 ret = 0;

    PLAT_WLAN_INFO("Enter, wifi_switch_saw_nob40\n");
    gpio_set_value(wlan_saw_sel0_gpio, 1);
    gpio_set_value(wlan_saw_sel1_gpio, 0);

    ret = wifi_switch_saw_notify_hisi(RF_PATH_NOB40);
    if (0 != ret)
    {
       PLAT_WLAN_ERR("wifi_switch_saw_notify_hisi error\n");
       return (-1);
    }
    g_wifi_rf_path_status = RF_PATH_NOB40;
    PLAT_WLAN_ERR("change RF path to nob40  sucess\n");
    return 0;
}
/*****************************************************************************
函数名称  : int wifi_switch_saw_b40()
功能描述  : 切换到Band40状态对应滤波器
输入参数  : NA
输出参数  : NA
返 回 值  : 返回切换结果,0:成功,-1:失败
*****************************************************************************/
int wifi_switch_saw_b40(void)
{
    int32 ret = 0;
    int value = 0;
    PLAT_WLAN_INFO("Enter, wifi_switch_saw_b40");
    gpio_set_value(wlan_saw_sel0_gpio, 0);
    gpio_set_value(wlan_saw_sel1_gpio, 1);

    ret = wifi_switch_saw_notify_hisi(RF_PATH_B40);
    if(0 != ret)
    {
       PLAT_WLAN_ERR("wifi_switch_saw_notify_hisi\n");
       return (-1);
    }

    g_wifi_rf_path_status = RF_PATH_B40;
    PLAT_WLAN_ERR("change RF path to b40  sucess\n");
    return 0;
}
/*****************************************************************************
函数名称  : static int wifi_get_rf_path_status()
功能描述  : 获取射频滤波器状态
输入参数  : NA
输出参数  : NA
返 回 值  : 当前滤波器状态
*****************************************************************************/
static int wifi_get_rf_path_status(void)
{
    PLAT_WLAN_INFO(" curent wifi rf path =%d\n", g_wifi_rf_path_status);
    return g_wifi_rf_path_status;
}
/*****************************************************************************
函数名称  : static int wifi_coexist_gpio_init()
功能描述  : 初始化射频滤波器切换GPIO
输入参数  : NA
输出参数  : NA
返 回 值  : 返回初始化结果,0:成功,-1:失败
*****************************************************************************/
static int wifi_coexist_gpio_init(void)
{
    int ret = 0;

    ret = gpio_request(wlan_saw_sel0_gpio, "wifi_saw_sel01");
    if (ret < 0)
    {
        PLAT_WLAN_ERR("[WiFi]wifi_coexist_gpio_init request 1_01 fail,ret = %d!!!\n", ret);
        return ret;
    }
    ret = gpio_request(wlan_saw_sel1_gpio, "wifi_saw_sel02");
    if (ret < 0)
    {
        PLAT_WLAN_ERR("[WiFi]wifi_coexist_gpio_init request 1_02 fail,ret = %d!!!\n", ret);
        return ret;
    }

    /* 默认非B40 */
    ret = gpio_direction_output(wlan_saw_sel0_gpio, 1);
    if (0 != ret)
    {
        PLAT_WLAN_ERR("switch gpio %d fail,result= d%\n",wlan_saw_sel0_gpio, ret);
        return (-1);
    }
    ret = gpio_direction_output(wlan_saw_sel1_gpio, 0);
    if (0 != ret)
    {
        PLAT_WLAN_ERR("switch gpio %d fail,result= d%\n",wlan_saw_sel1_gpio, ret);
        return (-1);
    }
...
static int __init wlan_if_init(void)
{
    ...
    #if (MBB_CHG_EXTCHG == FEATURE_ON)
    ret = blocking_notifier_chain_register(&g_extchg_pciesave_notifier_list,
        &extchg_notify_pcie_powersave_block);
    if( ret < 0 )
    {
        printk("wlan_if_init:g_extchg_pciesave_notifier_list register fail \n");
    }

    ret = register_reboot_notifier(&extchg_notify_pcie_powersave_block);
    if( ret < 0 )
    {
        printk("wlan_if_init:register_reboot_notifier fail \n");
    }
    #endif
    #if (MBB_CHARGE == FEATURE_ON)
    if( 1 == chg_is_powdown_charging())
    {
    ...
    /*如果已经下过电，则直接返回*/
        ...
        {
            printk("WL_REG_ON has been pulled down in wlan_if_init!!!\n");
            return 0;
        }
...
#endif
    spin_lock_init(&power_onoff_lock);
...
#if (defined(CONFIG_BALONG_SPE) || defined(CONFIG_BALONG_SPE_MODULE))
    int idx = 0;
    for (idx = 0; idx < MAX_WLAN_PORT_NUM; idx++)
    {
        if (0 != g_st_hisi_spe_alloc_port[idx].portNo)
        {
            (void)spe_hook.port_free(g_st_hisi_spe_alloc_port[idx].portNo);
            g_st_hisi_spe_alloc_port[idx].portNo = 0;
            g_st_hisi_spe_alloc_port[idx].portStatus = SPE_PORT_DISABLE;
        }
    }
#endif
#endif
...
#if (MBB_CHG_EXTCHG == FEATURE_ON)
    if (0 == pcie_is_suspend_due_to_extchg)
    {
...
void wifi_power_off_4356(void)
{
    ...
    if(first_flag != 0)
    {
        ...
        if ((1 == chg_stm_get_extchg_online_st()) && (0 == pcie_is_suspend_due_to_extchg))
        {
            balong_pcie_pm_sys_suspend();
...
#if (FEATURE_ON == MBB_CHG_EXTCHG)
#ifdef FEATURE_HUAWEI_MBB_BCM4356
void extchg_notify_pcie_powersave(struct notifier_block *nb, unsigned long event, void *v)
{
    struct pci_dev *h = NULL;
    struct pci_dev *d = NULL;
    unsigned long flags = 0;
    ...
    switch(event)
    {
        case EXTC_INSERT:  /* extchg insert*/
            if ((0 == pcie_is_suspend_due_to_extchg) && (0 == g_wifi_power_state))
            {
                /*get host pcie配置寄存器*/
                h = pci_get_class(PCIE_BALONG_PCIE0_CLASS << 8,h);
                /*get device配置寄存器*/
        ...
        case SYS_HALT:
        case SYS_POWER_OFF:
        case EXTC_REMOVE:  /*syscall restart halt shutdown & extchg remove*/
...
#ifdef FEATURE_HUAWEI_MBB_HI1151
/*****************************************************************************
函数名称  : int wifi_pwr_gpio_request()
功能描述  : 申请wifi电源控制GPIO
输入参数  : NA
输出参数  : NA
返 回 值  : 返回申请结果,0:成功,-1:失败
*****************************************************************************/
int wifi_pwr_gpio_request(void)
{
    int ret = 0;
    ret = gpio_request(HISI_DCDC_3V3_EN, WiFi_POWER_3V3);
    if(0 != ret)
    {
        PLAT_WLAN_ERR("request gpio %s FAIL!\n", WiFi_POWER_3V3);
        return ret;
    }
    ret = gpio_request(HISI_DCDC_1V1_EN, WiFi_POWER_1V1);
    if(0 != ret)
    {
        PLAT_WLAN_ERR("request gpio %s FAIL!\n", WiFi_POWER_1V1);
        return ret;
    }
    ret = gpio_request(HISI_PMU_PWRON, WiFi_POWER_PWRON);
    if(0 != ret)
    {
        PLAT_WLAN_ERR("request gpio %s FAIL!\n", WiFi_POWER_PWRON);
        return ret;
...
void wifi_pwr_gpio_pull_up(void)
{
    int ret = 0;
    /* Hi1151 wifi芯片时序:先拉高DCDC_3V3_EN和PMU_PWRON，然后延时13.5~18.5ms，再拉高DCDC_1V1_EN */
    if (gpio_is_valid(HISI_DCDC_3V3_EN) && gpio_is_valid(HISI_DCDC_1V1_EN) && gpio_is_valid(HISI_PMU_PWRON))
    {
        /*如果已经上过电，则直接返回*/
        ret = gpio_get_value(HISI_PMU_PWRON);
        if (1 == ret)
        {
            PLAT_WLAN_ERR("has been pulled up!!!\n");
            return;
        }
        gpio_direction_output(HISI_DCDC_3V3_EN, 1);
        gpio_direction_output(HISI_PMU_PWRON, 1);
        mdelay(HISI_PWR_DELAY);
        gpio_direction_output(HISI_DCDC_1V1_EN, 1);
        mdelay(100);
    }
    else
    {
...
void wifi_pwr_gpio_pull_down(void)
{
    if (gpio_is_valid(HISI_DCDC_3V3_EN) && gpio_is_valid(HISI_DCDC_1V1_EN) && gpio_is_valid(HISI_PMU_PWRON))
    {
        /* 如果已经下电则不进行操作 */
        if (1 == gpio_get_value(HISI_DCDC_1V1_EN))
        {
            gpio_direction_output(HISI_DCDC_1V1_EN, 0);
        }
        if (1 == gpio_get_value(HISI_PMU_PWRON))
        {
            gpio_direction_output(HISI_PMU_PWRON, 0);
        }
        if (1 == gpio_get_value(HISI_DCDC_3V3_EN))
        {
            gpio_direction_output(HISI_DCDC_3V3_EN, 0);
        }
        mdelay(50);
    }
    else
...
/*****************************************************************************
函数名称  : void wifi_power_on_1151()
功能描述  : 打开1151 wifi电源
输入参数  : NA
输出参数  : NA
返 回 值  : NA
*****************************************************************************/
void wifi_power_on_1151(void)
{
    struct pci_dev *p_hostdev = NULL;
    struct pci_dev *p_devicedev = NULL;
    unsigned long flags = 0;
    int ret = 0;

    /* 保存wifi电源状态,1代表打开 */
    g_wifi_power_state = 1;
    ret = gpio_is_valid(HISI_PMU_PWRON);
    if (1 != ret)
    {
        return;
    }
    ...
    /* PCIE resume*/
#ifdef CONFIG_BALONG_PCIE_PM
    (void)balong_pcie_pm_sys_suspend();
    mdelay(100);    /* 延时100毫秒 */
    (void)balong_pcie_pm_sys_resume();
#endif

    spin_lock_irqsave(&power_onoff_lock, flags);

    /* get host pcie配置寄存器,需要左移8位得到设备号 */
    p_hostdev = pci_get_class(PCIE_BALONG_PCIE0_CLASS << 8, p_hostdev);

    /* get device配置寄存器,需要左移8位得到设备号 */
    p_devicedev = pci_get_class(PCIE_1151_CLASS << 8, p_devicedev);
    if ((NULL == p_hostdev) || (NULL == p_devicedev))
    {
        PLAT_WLAN_ERR("please check pcie enumeration!!!\n");
        spin_unlock_irqrestore(&power_onoff_lock, flags);
        return;
    }

    /* restore host pcie配置寄存器 */
    pci_restore_state(p_hostdev);

    /* restore device pcie配置寄存器 */
    pci_restore_state(p_devicedev);
    spin_unlock_irqrestore(&power_onoff_lock, flags);
}

/*****************************************************************************
函数名称  : void wifi_power_off_1151()
功能描述  : 关闭1151 wifi电源
输入参数  : NA
输出参数  : NA
返 回 值  : NA
*****************************************************************************/
void wifi_power_off_1151(void)
{
    struct pci_dev *p_devicedev = NULL;
    struct pci_dev *p_hostdev = NULL;
    unsigned long flags = 0;
    ...
    if (1 != ret)
    {
        PLAT_WLAN_ERR("fail to get wifi gpio!\n");
        return;
    }
    ...
    /* save host pcie配置寄存器 */
    (void)pci_save_state(p_hostdev);

    /* save device pcie配置寄存器 */
    (void)pci_save_state(p_devicedev);
...
/*****************************************************************************
函数名称  : int wifi_spe_port_alloc()
功能描述  : wifi SPE端口申请
输入参数  : ndev: 网络设备接口,attr: 申请的SPE端口属性
输出参数  : NA
返 回 值  : 申请的SPE端口号
*****************************************************************************/
int wifi_spe_port_alloc(struct net_device *ndev, struct spe_port_attr *attr)
{
    int port_num = 0;
    int idx = 0;
    int ret = 0;
    struct spe_port_attr_alt st_port_property;

    if ((NULL == ndev) || (NULL == attr))
    {
        PLAT_WLAN_ERR("[wifi_spe_port_alloc]:invalid para!\n");
        return ERROR;
    }

    (void)memset(&st_port_property, 0x0, sizeof(struct spe_port_attr_alt));

    /* 获取可用的SPE端口index */
    for (idx = 0; idx < MAX_WLAN_PORT_NUM; idx++)
    {
        if ((0 == g_st_hisi_spe_alloc_port[idx].portNo) || (SPE_PORT_DISABLE == g_st_hisi_spe_alloc_port[idx].portStatus))
        {
            break;
        }
    }
    if (idx >= MAX_WLAN_PORT_NUM)
    {
        PLAT_WLAN_ERR("idx error, idx = %d\n", idx);
        return ERROR;
    }

    /* 第一次申请则从SPE模块进行申请，第二次直接从全局变量中获取 */
    if (0 == g_st_hisi_spe_alloc_port[idx].portNo)
    {
        /* 申请SPE端口 */
        port_num = spe_hook.port_alloc(attr);
        PLAT_WLAN_INFO("spe port %d first alloc;\n", port_num);

        if ((port_num > MAX_SPE_PORT_NUM) || (port_num < 0))
        {
            PLAT_WLAN_ERR("spe port alloc failed, port num = %d;\n", port_num);
            return ERROR;
        }

        /* 端口号保存在全局变量中 */
        g_st_hisi_spe_alloc_port[idx].portNo = port_num;
    }
    else
    {
        /* 非第一次申请，则从全局变量中获取port num */
        port_num = g_st_hisi_spe_alloc_port[idx].portNo;
        PLAT_WLAN_INFO("spe port %d has been alloc, and get port num from global var;\n", g_st_hisi_spe_alloc_port[idx].portNo);

        /* 设置钩子函数 */
        st_port_property.enc_type = spe_enc_none;
        st_port_property.desc_ops.finish_td = attr->desc_ops.finish_td;
        st_port_property.desc_ops.finish_rd = attr->desc_ops.finish_rd;
        spe_hook.set_port_property(ndev, &st_port_property, g_st_hisi_spe_alloc_port[idx].portNo);
    }

    /* 使能SPE端口 */
    ret = spe_hook.port_enable(port_num);
    if (0 != ret)
    {
        PLAT_WLAN_ERR("spe port enable fail.\n");
        return ERROR;
    }
    g_st_hisi_spe_alloc_port[idx].portStatus = SPE_PORT_ENABLE;

    return port_num;
}

/*****************************************************************************
函数名称  : int wifi_spe_port_free()
功能描述  : wifi SPE端口释放
输入参数  : port_num: 要释放SPE的端口号
输出参数  : NA
返 回 值  : 无
*****************************************************************************/
void wifi_spe_port_free(unsigned int port_num)
{
    int idx = 0;
    struct spe_port_attr_alt st_port_property;
    ...
    for (idx = 0; idx < MAX_WLAN_PORT_NUM; idx++)
    {
        if (port_num == g_st_hisi_spe_alloc_port[idx].portNo)
        {
            break;
        }
    }
    if (MAX_WLAN_PORT_NUM <= idx)
    {
        PLAT_WLAN_ERR("spe port(%d) error.\n", port_num);
        return;
    }

    /* disable SPE端口 */
    (void)spe_hook.port_disable(port_num);
    g_st_hisi_spe_alloc_port[idx].portStatus = SPE_PORT_DISABLE;
    PLAT_WLAN_INFO("spe port(%d) disabled.\n", port_num);

    /* 不释放SPE端口，只将钩子函数设置为NULL */
    st_port_property.enc_type = spe_enc_none;
    st_port_property.desc_ops.finish_td = NULL;
    st_port_property.desc_ops.finish_rd = NULL;
    spe_hook.set_port_property(NULL, &st_port_property, port_num);
}
#endif
#endif
#endif
...
===========================================================================*/
EXPORT_SYMBOL(wlan_saw_sel0_gpio);
EXPORT_SYMBOL(wlan_saw_sel1_gpio);
...
#ifdef FEATURE_HUAWEI_MBB_HI1151
EXPORT_SYMBOL(wifi_pwr_gpio_request);
EXPORT_SYMBOL(wifi_pwr_gpio_pull_up);
EXPORT_SYMBOL(wifi_pwr_gpio_pull_down);
EXPORT_SYMBOL(wifi_power_on_1151);
EXPORT_SYMBOL(wifi_power_off_1151);
EXPORT_SYMBOL(wlan_wakehost_gpio);
EXPORT_SYMBOL(wlan_sleep_gpio);
EXPORT_SYMBOL(wlan_pciereset_gpio);

#if (defined(CONFIG_BALONG_SPE) || defined(CONFIG_BALONG_SPE_MODULE))
EXPORT_SYMBOL(wifi_spe_port_alloc);
EXPORT_SYMBOL(wifi_spe_port_free);
#endif
#endif
