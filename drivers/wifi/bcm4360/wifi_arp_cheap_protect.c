...
#define WIFI_THREE    3
#define WIFI_FOUR     4
#define WIFI_ELEVEN   11
#define WIFI_TWELVE   12
#define WIFI_SIXTEEN  16

/* 针对不同版本的内核获得桥接口信息 */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(WIFI_THREE, WIFI_FOUR, WIFI_ELEVEN)
#define GET_BR_PORT(netdev) ((struct net_bridge_port *)\
        br_port_get_rcu((struct net_device *)netdev))
#else
#define GET_BR_PORT(netdev) ((struct net_bridge_port *)\
        rcu_dereference(((struct net_device *)netdev)->br_port))
#endif
...
static inline int check_arp_with_br(struct net_device * from_device, 
unsigned char* arp_src_ptr)
{
