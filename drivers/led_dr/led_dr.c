...
static int __init dr_led_init(void)
{
    int ret = 0;
    /*获取共享内存的地址*/
    ...
    /*从共享内存里读出信息，判断是否支持dr灯，如果不支持直接return*/
    if (NOT_SUPPORT_DR_LED == smem_data->dr_led)
    {
        printk(KERN_ERR "[%s]:Not support dr led!\n", __func__);
        return ret;
    }
    /*注册platform驱动*/
...
module_init(dr_led_init); 
static void __exit dr_led_exit(void)
{
    platform_driver_unregister(&led_dr_drv);
}
module_exit(dr_led_exit);
#else
