...
/* CPE B529产品电池适配，没有库仑计，开电池一级宏MBB_CHARGE,编译版本调用MBB_CHG_COULOMETER宏括的函数发生错误 */
/* 用MBB_CHG_COULOMETER宏括起来，解决MBB_CHG_COULOMETER宏遗留问题 */
extern int smartstar_battery_current(void);
#endif /* (FEATURE_ON == MBB_CHG_COULOMETER) */
...
/* 新增全局变量，用于接收设备树中电池参数和充电参数   */
chg_hardware_data_dtsi g_chg_hardpara_dtsi = {0};
chg_batt_data_dtsi *g_battery_para_dtsi = NULL;
#endif
/*硬件产品版本号*/
...
/*the comand of limit current to 1A*/
#define EXTCHG_APORT_1A_CMD             _IOWR(CHG_CMD_BASE, 11, unsigned long)
/*the comand of limit current to 2A*/
...
#if (FEATURE_ON == MBB_CHG_BATT_UNIFORM)
    /* 若产品是不支持外充的，则USB端发送的外充插入拔出消息不接收 */
    if(EXTCHG_NOSUPPORT == g_chg_hardpara_dtsi.chg_extchg_support && CHG_EXGCHG_CHGR == v)
    {   
        pr_err("battery_monitor_blocking_notifier_call_chain: the product not support extchg!\n");
        return 0;
    }
#endif
...
#if (FEATURE_ON == MBB_CHG_BATT_UNIFORM)
/**********************************************************************  
  Function:       chg_para_dtsi_print
  Description:    充电初始化读取设备树中充电参数值
  Input:          无
  Output:         无
  Return:         无
  Others:         设备树获取失败使用默认值
**********************************************************************/
void chg_para_dtsi_print(void)
{
    int i = 0;
   /* 充电参数打印 */
    printk(KERN_ERR "g_chg_hardpara_dtsi:batt_bad_volt  %d \n",g_chg_hardpara_dtsi.batt_bad_volt);
    printk(KERN_ERR "g_chg_hardpara_dtsi:chg_ic_en_gpio %d \n",g_chg_hardpara_dtsi.chg_ic_en_gpio);
    printk(KERN_ERR "g_chg_hardpara_dtsi:g_default_dpm_volt %d \n",g_chg_hardpara_dtsi.g_default_dpm_volt);
    printk(KERN_ERR "g_chg_hardpara_dtsi:poweron_batt_volt %d \n",g_chg_hardpara_dtsi.poweron_batt_volt);
    printk(KERN_ERR "g_chg_hardpara_dtsi:trickle_chg_volt %d \n",g_chg_hardpara_dtsi.trickle_chg_volt);
    printk(KERN_ERR "g_chg_hardpara_dtsi:batt_cali_volt_max %d \n",g_chg_hardpara_dtsi.batt_cali_volt_max);
    printk(KERN_ERR "g_chg_hardpara_dtsi:batt_cali_volt_min %d \n",g_chg_hardpara_dtsi.batt_cali_volt_min);
    printk(KERN_ERR "g_chg_hardpara_dtsi:batt_id_hkadc_chan %d \n",g_chg_hardpara_dtsi.batt_id_hkadc_chan);
    printk(KERN_ERR "g_chg_hardpara_dtsi:batt_temp_hkadc_chan %d \n",g_chg_hardpara_dtsi.batt_temp_hkadc_chan);
    printk(KERN_ERR "g_chg_hardpara_dtsi:batt_temp_hkadc_chan %d \n",g_chg_hardpara_dtsi.batt_temp_hkadc_chan);
    printk(KERN_ERR "g_chg_hardpara_dtsi:usb_temp_hkadc_chan %d \n",g_chg_hardpara_dtsi.usb_temp_hkadc_chan);
    printk(KERN_ERR "g_chg_hardpara_dtsi:vbus_volt_hkadc_chan %d \n",g_chg_hardpara_dtsi.vbus_volt_hkadc_chan);
    printk(KERN_ERR "g_chg_hardpara_dtsi:fastchg_timeout %d  %d  %d  %d \r\n",g_chg_hardpara_dtsi.chg_fastchg_timeout_value_in_sec[0][0],
        g_chg_hardpara_dtsi.chg_fastchg_timeout_value_in_sec[0][1],g_chg_hardpara_dtsi.chg_fastchg_timeout_value_in_sec[1][0],
        g_chg_hardpara_dtsi.chg_fastchg_timeout_value_in_sec[1][1]);
   
    printk(KERN_ERR "g_chg_hardpara_dtsi:wall chgr  %d  %d  %d \n",g_chg_hardpara_dtsi.charger_current_limit_paras[0].chgr_type,
        g_chg_hardpara_dtsi.charger_current_limit_paras[0].current_limit,g_chg_hardpara_dtsi.charger_current_limit_paras[0].current_limit_usb);   	
    printk(KERN_ERR "g_chg_hardpara_dtsi:usb chgr  %d  %d  %d \n",g_chg_hardpara_dtsi.charger_current_limit_paras[1].chgr_type,
        g_chg_hardpara_dtsi.charger_current_limit_paras[1].current_limit,g_chg_hardpara_dtsi.charger_current_limit_paras[1].current_limit_usb);
    printk(KERN_ERR "g_chg_hardpara_dtsi:wireless  %d  %d  %d \n",g_chg_hardpara_dtsi.charger_current_limit_paras[2].chgr_type,
        g_chg_hardpara_dtsi.charger_current_limit_paras[2].current_limit,g_chg_hardpara_dtsi.charger_current_limit_paras[2].current_limit_usb);
    printk(KERN_ERR "g_chg_hardpara_dtsi:hvdcp  %d  %d  %d \r\n",g_chg_hardpara_dtsi.charger_current_limit_paras[3].chgr_type,
        g_chg_hardpara_dtsi.charger_current_limit_paras[3].current_limit,g_chg_hardpara_dtsi.charger_current_limit_paras[3].current_limit_usb);

    for (i = 0;i < CHG_STM_MAX_ST;i++)
    {
        printk(KERN_ERR "g_chg_hardpara_dtsi:wall chgr  %d  %d %d  %d  %d\n",g_chg_hardpara_dtsi.chg_std_chgr_hw_paras[i].pwr_supply_current_limit_in_mA,
            g_chg_hardpara_dtsi.chg_std_chgr_hw_paras[i].chg_current_limit_in_mA,g_chg_hardpara_dtsi.chg_std_chgr_hw_paras[i].chg_CV_volt_setting_in_mV,
            g_chg_hardpara_dtsi.chg_std_chgr_hw_paras[i].chg_taper_current_in_mA,g_chg_hardpara_dtsi.chg_std_chgr_hw_paras[i].chg_is_enabled);
    }
    for (i = 0;i < CHG_STM_MAX_ST;i++)
    {
        printk(KERN_ERR "g_chg_hardpara_dtsi:usb2  %d  %d  %d  %d  %d\n",g_chg_hardpara_dtsi.chg_usb_chgr_hw_paras[i].pwr_supply_current_limit_in_mA,
            g_chg_hardpara_dtsi.chg_usb_chgr_hw_paras[i].chg_current_limit_in_mA,g_chg_hardpara_dtsi.chg_usb_chgr_hw_paras[i].chg_CV_volt_setting_in_mV,
            g_chg_hardpara_dtsi.chg_usb_chgr_hw_paras[i].chg_taper_current_in_mA,g_chg_hardpara_dtsi.chg_usb_chgr_hw_paras[i].chg_is_enabled);
    }
    for (i = 0;i < CHG_STM_MAX_ST;i++)
    {
        printk(KERN_ERR "g_chg_hardpara_dtsi:usb3  %d  %d  %d  %d  %d\n",g_chg_hardpara_dtsi.chg_usb3_chgr_hw_paras[i].pwr_supply_current_limit_in_mA,
            g_chg_hardpara_dtsi.chg_usb3_chgr_hw_paras[i].chg_current_limit_in_mA,g_chg_hardpara_dtsi.chg_usb3_chgr_hw_paras[i].chg_CV_volt_setting_in_mV,
            g_chg_hardpara_dtsi.chg_usb3_chgr_hw_paras[i].chg_taper_current_in_mA,g_chg_hardpara_dtsi.chg_usb3_chgr_hw_paras[i].chg_is_enabled);
    }
    for (i = 0;i < CHG_STM_MAX_ST;i++)
    {
        printk(KERN_ERR "g_chg_hardpara_dtsi:weak  %d  %d  %d  %d  %d \n",g_chg_hardpara_dtsi.chg_weak_chgr_hw_paras[i].pwr_supply_current_limit_in_mA,
            g_chg_hardpara_dtsi.chg_std_chgr_hw_paras[i].chg_current_limit_in_mA,g_chg_hardpara_dtsi.chg_weak_chgr_hw_paras[i].chg_CV_volt_setting_in_mV,
            g_chg_hardpara_dtsi.chg_std_chgr_hw_paras[i].chg_taper_current_in_mA,g_chg_hardpara_dtsi.chg_std_chgr_hw_paras[i].chg_is_enabled);
    }
   
    for (i = 0;i < g_battery_para_dtsi->count; i++)
    {
        printk(KERN_ERR "min=%d max=%d calib=%d id=%d volt= %d %d %d %d %d %d %d %d %d %d %d %d %d\n",
            g_battery_para_dtsi->chg_batt_data_paras[i]->id_voltage_min,
            g_battery_para_dtsi->chg_batt_data_paras[i]->id_voltage_max,
            g_battery_para_dtsi->chg_batt_data_paras[i]->is_20pct_calib,
            g_battery_para_dtsi->chg_batt_data_paras[i]->batt_id,
            g_battery_para_dtsi->chg_batt_data_paras[i]->chg_batt_volt_paras.battVoltPowerOnThreshold,
            g_battery_para_dtsi->chg_batt_data_paras[i]->chg_batt_volt_paras.battVoltPowerOffThreshold,
            g_battery_para_dtsi->chg_batt_data_paras[i]->chg_batt_volt_paras.battOverVoltProtectThreshold,
            g_battery_para_dtsi->chg_batt_data_paras[i]->chg_batt_volt_paras.battOverVoltProtectOneThreshold,
            g_battery_para_dtsi->chg_batt_data_paras[i]->chg_batt_volt_paras.battChgTempMaintThreshold,
            g_battery_para_dtsi->chg_batt_data_paras[i]->chg_batt_volt_paras.battChgRechargeThreshold,
            g_battery_para_dtsi->chg_batt_data_paras[i]->chg_batt_volt_paras.VbatLevelLow_MAX,
            g_battery_para_dtsi->chg_batt_data_paras[i]->chg_batt_volt_paras.VbatLevel0_MAX,
            g_battery_para_dtsi->chg_batt_data_paras[i]->chg_batt_volt_paras.VbatLevel1_MAX,
            g_battery_para_dtsi->chg_batt_data_paras[i]->chg_batt_volt_paras.VbatLevel2_MAX,
            g_battery_para_dtsi->chg_batt_data_paras[i]->chg_batt_volt_paras.VbatLevel3_MAX,
            g_battery_para_dtsi->chg_batt_data_paras[i]->chg_batt_volt_paras.battInsertChargeThreshold,
            g_battery_para_dtsi->chg_batt_data_paras[i]->chg_batt_volt_paras.battNormalTempRechergeThreshold);
    }

}

/**********************************************************************  
  Function:       chg_get_charge_dts_init
  Description:    充电初始化读取设备树中充电参数值
  Input:          无
  Output:         无
  Return:         无
  Others:         设备树获取失败使用默认值
**********************************************************************/
static void chg_get_charge_dts_init()
{
    int ret = 0;
    struct device_node *dev_node   = NULL;

    chg_hw_param_t *std_chgr_hw_paras = NULL;
    chg_hw_param_t *usb_chgr_hw_paras = NULL;
    chg_hw_param_t *usb3_chgr_hw_paras = NULL;
    chg_hw_param_t *weak_chgr_hw_paras = NULL;
    charger_current_limit_st *current_limit_paras = NULL;

    /* 查找充电模块设备树节点  */
    dev_node = of_find_compatible_node(NULL, NULL, "mbb_charge_paras");
    if(NULL == dev_node)
    {
        printk(KERN_ERR "mbb_charge_dts_init:mbb_charge_paras device node not found!\n");
        goto error_read1;
    }
    /* 读取设备树中各门限和通道值  */
    ret = of_property_read_u32_index(dev_node, "batt_cali_volt_min", 0, &g_chg_hardpara_dtsi.batt_cali_volt_min);
    ret |= of_property_read_u32_index(dev_node, "batt_cali_volt_max", 0, &g_chg_hardpara_dtsi.batt_cali_volt_max);
    ret |= of_property_read_u32_index(dev_node, "default_dpm_volt", 0, &g_chg_hardpara_dtsi.g_default_dpm_volt);
    ret |= of_property_read_u32_index(dev_node, "batt_id_hkadc_chan", 0, &g_chg_hardpara_dtsi.batt_id_hkadc_chan);
    ret |= of_property_read_u32_index(dev_node, "batt_volt_hkadc_chan", 0, &g_chg_hardpara_dtsi.batt_volt_hkadc_chan);
    ret |= of_property_read_u32_index(dev_node, "vbus_volt_hkadc_chan", 0, &g_chg_hardpara_dtsi.vbus_volt_hkadc_chan);
    ret |= of_property_read_u32_index(dev_node, "batt_temp_hkadc_chan", 0, &g_chg_hardpara_dtsi.batt_temp_hkadc_chan);
    ret |= of_property_read_u32_index(dev_node, "usb_temp_hkadc_chan", 0, &g_chg_hardpara_dtsi.usb_temp_hkadc_chan);
    ret |= of_property_read_u32_index(dev_node, "batt_bad_volt", 0, &g_chg_hardpara_dtsi.batt_bad_volt);
    ret |= of_property_read_u32_index(dev_node, "poweron_batt_volt", 0, &g_chg_hardpara_dtsi.poweron_batt_volt);
    ret |= of_property_read_u32_index(dev_node, "trickle_chg_volt", 0, &g_chg_hardpara_dtsi.trickle_chg_volt);
    ret |= of_property_read_u32_index(dev_node, "chg_ic_en_gpio", 0, &g_chg_hardpara_dtsi.chg_ic_en_gpio);
    ret |= of_property_read_u32_index(dev_node, "chgr_size", 0, &g_chg_hardpara_dtsi.chgr_size);
    ret |= of_property_read_u32_index(dev_node, "curlimit_size", 0, &g_chg_hardpara_dtsi.curlimit_size);
    if(0 != ret)
    {
        goto error_read1;
    }
    ...
    /*  读取标充各充电电流   */
    std_chgr_hw_paras = kzalloc(sizeof(chg_hw_param_t) * CHG_STM_MAX_ST, GFP_KERNEL);
    if(NULL == std_chgr_hw_paras)
    {
        printk(KERN_ERR "Failed to allocate struct chg_hw_param_t\n");
        goto error_alloc1;
    }
    /*  读取USB2.0 各充电电流   */
    usb_chgr_hw_paras = kzalloc(sizeof(chg_hw_param_t) * CHG_STM_MAX_ST, GFP_KERNEL);
    if(NULL == usb_chgr_hw_paras)
    {
        printk(KERN_ERR "Failed to allocate struct chg_hw_param_t\n");
        goto error_alloc1;
    }
    /*  读取USB3.0 各充电电流   */
    usb3_chgr_hw_paras = kzalloc(sizeof(chg_hw_param_t) * CHG_STM_MAX_ST, GFP_KERNEL);
    if(NULL == usb3_chgr_hw_paras)
    {
        printk(KERN_ERR "Failed to allocate struct chg_hw_param_t\n");
        goto error_alloc1;
    }
    ...
    /*  读取弱充 各充电电流   */
    weak_chgr_hw_paras = kzalloc(sizeof(chg_hw_param_t) * CHG_STM_MAX_ST, GFP_KERNEL);
    if(NULL == weak_chgr_hw_paras)
    {
        printk(KERN_ERR "Failed to allocate struct chg_hw_param_t\n");
        goto error_alloc1;
    }

    ret |= of_property_read_u32_array(dev_node, "chg_std_chgr_hw_paras", std_chgr_hw_paras, CHG_STM_MAX_ST * g_chg_hardpara_dtsi.chgr_size);
    ret |= of_property_read_u32_array(dev_node, "chg_usb_chgr_hw_paras", usb_chgr_hw_paras, CHG_STM_MAX_ST * g_chg_hardpara_dtsi.chgr_size);
    ret |= of_property_read_u32_array(dev_node, "chg_usb3_chgr_hw_paras", usb3_chgr_hw_paras, CHG_STM_MAX_ST * g_chg_hardpara_dtsi.chgr_size);
    ret |= of_property_read_u32_array(dev_node, "chg_weak_chgr_hw_paras", weak_chgr_hw_paras, CHG_STM_MAX_ST * g_chg_hardpara_dtsi.chgr_size);
    if(0 != ret)
    {
        goto error_alloc1;
    }
    ...
    {
        /* 设备树数据读取成功，将数据赋值为全局信息 */
        g_chg_hardpara_dtsi.chg_std_chgr_hw_paras = std_chgr_hw_paras; 
        g_chg_hardpara_dtsi.chg_usb_chgr_hw_paras = usb_chgr_hw_paras;
        g_chg_hardpara_dtsi.chg_usb3_chgr_hw_paras = usb3_chgr_hw_paras;
        g_chg_hardpara_dtsi.chg_weak_chgr_hw_paras = weak_chgr_hw_paras;
    }

    /* 低温限流参数  */
    current_limit_paras = kzalloc(sizeof(charger_current_limit_st) * CHG_CHGR_INVALID, GFP_KERNEL);
    if(NULL == current_limit_paras)
    {
        printk(KERN_ERR "Failed to allocate struct charger_current_limit_paras\n");
        goto error_alloc2;
    }
    ret = of_property_read_u32_array(dev_node, "charge_current_limit", current_limit_paras, CHG_CHGR_INVALID * g_chg_hardpara_dtsi.curlimit_size);
    if(0 != ret)
    {
        goto error_alloc2;
    }
    ...
    {
        /* 设备树数据读取成功，将数据赋值为全局信息 */
        g_chg_hardpara_dtsi.charger_current_limit_paras = current_limit_paras;
    }

    /* 充电超时时间 */
    ret |= of_property_read_u32_index(dev_node, "fastchg_timeout", 0, &g_chg_hardpara_dtsi.chg_fastchg_timeout_value_in_sec[0][0]);
    ret |= of_property_read_u32_index(dev_node, "fastchg_timeout", 1, &g_chg_hardpara_dtsi.chg_fastchg_timeout_value_in_sec[0][1]);
    ret |= of_property_read_u32_index(dev_node, "fastchg_timeout", 2, &g_chg_hardpara_dtsi.chg_fastchg_timeout_value_in_sec[1][0]);
    ret |= of_property_read_u32_index(dev_node, "fastchg_timeout", 3, &g_chg_hardpara_dtsi.chg_fastchg_timeout_value_in_sec[1][1]);
    if(0 != ret)
    {
        goto error_read2;
    }
    ...
    return;
    /* 设备树数据获取失败，则使用chg_charge_cust.h中的默认值 */
    error_read1:
        g_chg_hardpara_dtsi.batt_bad_volt = CHG_SHORT_CIRC_BATTERY_THRES;
        g_chg_hardpara_dtsi.batt_cali_volt_max = CHG_BATT_CALI_MAX_VOLT;
        g_chg_hardpara_dtsi.batt_cali_volt_min = CHG_BATT_CALI_MIN_VOLT;
        g_chg_hardpara_dtsi.batt_id_hkadc_chan = CHG_BATT_ID_CHAN;
        g_chg_hardpara_dtsi.batt_temp_hkadc_chan = CHG_BATT_TEMP_CHAN;
        g_chg_hardpara_dtsi.batt_volt_hkadc_chan = CHG_BATT_VOLT_CHAN;
        g_chg_hardpara_dtsi.chg_ic_en_gpio = CHG_ENABLE_GPIO;
        g_chg_hardpara_dtsi.g_default_dpm_volt = CHG_DEFAULT_DPM_VOLT;
        g_chg_hardpara_dtsi.poweron_batt_volt = BAT_CHECK_POWERON_VOLT;
        g_chg_hardpara_dtsi.trickle_chg_volt = BAT_CHECK_JUDGMENT_VOLT;
        g_chg_hardpara_dtsi.usb_temp_hkadc_chan = CHG_USB_TEMP_CHAN;
        g_chg_hardpara_dtsi.vbus_volt_hkadc_chan = CHG_VBUS_VOLT_CHAN;

    error_alloc1:
        /* 母产品的充电电流参数通过硬件版本号赋值了，这里在设备树获取失败后使用了1A的默认值 */
        g_chg_hardpara_dtsi.chg_std_chgr_hw_paras = chg_std_1A_chgr_hw_paras_def;
        g_chg_hardpara_dtsi.chg_usb_chgr_hw_paras = chg_usb_chgr_hw_paras_def;
        g_chg_hardpara_dtsi.chg_usb3_chgr_hw_paras = chg_usb3_chgr_hw_paras_def;
        g_chg_hardpara_dtsi.chg_weak_chgr_hw_paras = chg_weak_chgr_hw_paras_def;

        if(NULL != std_chgr_hw_paras)
        {
            kfree(std_chgr_hw_paras);
        }
        if(NULL != usb_chgr_hw_paras)
        {
        kfree(usb_chgr_hw_paras);
        }
        if(NULL != usb3_chgr_hw_paras)
        {
            kfree(usb3_chgr_hw_paras);
        }
        if(NULL != weak_chgr_hw_paras)
        {
            kfree(weak_chgr_hw_paras);
        }

    error_alloc2:
        g_chg_hardpara_dtsi.charger_current_limit_paras = charger_current_limit_paras;
        if(NULL != current_limit_paras)
        {
            kfree(current_limit_paras);
        }
    error_read2:
        g_chg_hardpara_dtsi.chg_fastchg_timeout_value_in_sec[0][0] = CHG_POWEROFF_FAST_USB_TIMER_VALUE;
        g_chg_hardpara_dtsi.chg_fastchg_timeout_value_in_sec[0][1] = CHG_POWEROFF_FAST_CHG_TIMER_VALUE;
        g_chg_hardpara_dtsi.chg_fastchg_timeout_value_in_sec[1][0] = CHG_FAST_USB_TIMER_VALUE;
        g_chg_hardpara_dtsi.chg_fastchg_timeout_value_in_sec[1][1] = CHG_FAST_CHG_TIMER_VALUE;
...
/**********************************************************************  
  Function:       chg_get_battery_dts_init
  Description:    充电初始化读取设备树中不同电池厂家参数
  Input:          无
  Output:         无
  Return:         无
  Others:         设备树获取失败使用默认值
**********************************************************************/
static void chg_get_battery_dts_init(void)
{
    struct device_node *dev_node = NULL,*child = NULL;
    int count = 0, i = 0;
    int ret = 0;
    struct chg_batt_data *chg_batt_data = NULL;

    /* 读取设备树节点 */
    dev_node = of_find_compatible_node(NULL, NULL, "mbb_battery_paras");
    if(NULL == dev_node)
    {
        printk(KERN_ERR "mbb_battery_dts_init:mbb_battery_paras device node not found!\n");
        goto error_read;
    }

    count = of_get_child_count(dev_node);
    if (!count)
    {
        printk(KERN_ERR "mbb_battery_dts_init: no child node\n");
        goto error_read;
    }

    /* 为电池全局变量申请空间 ,若申请失败，则使用报错，电池参数使用default值 */
    g_battery_para_dtsi = kzalloc(sizeof(chg_batt_data_dtsi) + count * sizeof(struct chg_batt_data *), GFP_KERNEL);
    if(NULL == g_battery_para_dtsi)
    {
        printk(KERN_ERR "mbb_battery_dts_init:g_battery_para_dtsi malloc fail!\n");
        return;
    }
    /* 子节点个数赋值 */
    g_battery_para_dtsi->count = count;
    chg_batt_data = kzalloc(sizeof(struct chg_batt_data) * count , GFP_KERNEL);
    if(NULL == chg_batt_data)
    {
        printk(KERN_ERR "mbb_battery_dts_init: alloc chg_batt_data fail\n");
        goto error_read;
    }
    ...
    for_each_child_of_node(dev_node, child) 
    {
        if(i < count)
        {
            ret |= of_property_read_u32_index(child, "id_voltage_min", 0,&(chg_batt_data[i].id_voltage_min));
            ret |= of_property_read_u32_index(child, "id_voltage_max", 0, &(chg_batt_data[i].id_voltage_max));
            ret |= of_property_read_u32_index(child, "batt_id", 0, &(chg_batt_data[i].batt_id));
            ret |= of_property_read_u32_index(child, "is_20pct_calib", 0, &(chg_batt_data[i].is_20pct_calib));
            ret |= of_property_read_u32_array(child, "batt_volt_paras", &(chg_batt_data[i].chg_batt_volt_paras), sizeof(CHG_SHUTOFF_VOLT_PROTECT_NV_TYPE)/sizeof(int));
            i++;
            if(0 != ret)
            {
                goto error_read;
    ...
    for (i = 0;i < count;i++)
    {
        g_battery_para_dtsi->chg_batt_data_paras[i] = &chg_batt_data[i];
    }
    return;
...
error_read:
    ...
    if(NULL != g_battery_para_dtsi)
    {
        kfree(g_battery_para_dtsi);
        g_battery_para_dtsi = NULL;
    }
    count = sizeof(chg_batt_data_array) / sizeof(chg_batt_data_array[0]); 
    /* 设备树获取失败，使用头文件中的默认值,根据新的count数量申请空间并赋值 */
    g_battery_para_dtsi = kzalloc(sizeof(chg_batt_data_dtsi) + count * sizeof(struct chg_batt_data *), GFP_KERNEL);
    if(NULL == g_battery_para_dtsi)
    {
        printk(KERN_ERR "mbb_battery_dts_init:g_battery_para_dtsi malloc fail again!\n");
        return;
    }
    g_battery_para_dtsi->count = count;
    for (i = 0;i < g_battery_para_dtsi->count;i++)
    {
        g_battery_para_dtsi->chg_batt_data_paras[i] = chg_batt_data_array[i];
    }
    return;
...
/**********************************************************************  
  Function:       chg_get_exchg_support_init
  Description:    根据硬件版本号初始化外充特性是否支持
  Input:          无
  Output:         无
  Return:         无
  Others:         电池特性二进制归一的产品需要在初始化设置是否支持外充
**********************************************************************/
void chg_get_exchg_support_init(void)
{
    switch(g_hardware_version_id)
    {
        /* 支持外充特性的产品，将support置1  */
        case HW_VER_PRODUCT_E5785Lh_92a:
            g_chg_hardpara_dtsi.chg_extchg_support = EXTCHG_SUPPORT;
            break;
        case HW_VER_PRODUCT_E5783h_92a:
            g_chg_hardpara_dtsi.chg_extchg_support = EXTCHG_NOSUPPORT;
            break;
        default:
            g_chg_hardpara_dtsi.chg_extchg_support = EXTCHG_NOSUPPORT;
            break;
...
#if (FEATURE_ON == MBB_CHG_BATT_UNIFORM)
    /* 电池特性二进制归一的产品需要初始化化相关参数 */
    chg_get_charge_dts_init();
    chg_get_battery_dts_init();
    chg_get_exchg_support_init();
#endif
...
#if (MBB_CHG_EXTCHG == FEATURE_ON)
#if (FEATURE_ON == MBB_CHG_BATT_UNIFORM)
    if( EXTCHG_SUPPORT == g_chg_hardpara_dtsi.chg_extchg_support )
    {
