...
/*关机外充时单板能否关机标志位*/
extern boolean g_extchg_shutoff_flag;
#endif
/*----------------------------------------------*
 * 全局变量                                     *
 *----------------------------------------------*/
...
#if defined(BSP_CONFIG_BOARD_E5885Ls_93a)
/*关机外充拔出外充后起定时器检测是否外充真正拔出*/
struct softtimer_list g_extchg_shutoff_timer ;
#endif
...
#if defined(BSP_CONFIG_BOARD_E5885Ls_93a)
struct workqueue_struct *extchg_shutoff_check_workqueue = NULL;
struct work_struct extchg_shutoff_check_work;
#endif
...
#if defined(BSP_CONFIG_BOARD_E5885Ls_93a)
/**********************************************************************
函 数 名: extchg_plug_out_detect_timer_call
功能描述: 定时器回调函数，实现类似中断上半部分功能，防止定时器被阻塞到回调函数
输入参数: None
输出参数: None
返 回 值: None
***********************************************************************/
void extchg_plug_out_detect_timer_call(void)
{
    /*调度工作队列*/
    queue_work(extchg_shutoff_check_workqueue, &extchg_shutoff_check_work); 

    return;
}

void extchg_shutoff_work(void)
{
    int count = 0;
...
#if defined(BSP_CONFIG_BOARD_E5885Ls_93a)
    extchg_shutoff_check_workqueue = create_workqueue("extchg_shutoff");
    INIT_WORK(&extchg_shutoff_check_work, extchg_shutoff_work);   
    g_extchg_shutoff_timer.func = extchg_plug_out_detect_timer_call;
    g_extchg_shutoff_timer.para = 0;
    g_extchg_shutoff_timer.timeout = EXTCHG_PLUG_OUT_POLL_CYCLE;
    g_extchg_shutoff_timer.wake_type = SOFTTIMER_WAKE;
#endif
