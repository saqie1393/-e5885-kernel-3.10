...
#define MAX_SLEEP_TIME (80)
#define COUL_FIRMWARE_UPDATE_LEN (191278)
/*the flag of start of coul firmware update*/
bool coul_update_start_flag  = false;
/*the status of coul firmware update*/
bool coul_update_status_flag = false;
#endif/*FEATURE_ON == MBB_CHG_BQ27510*/
...
static void bq27510_get_firmware_name(char *config_name)
{
    int index = 0;
...
#if (FEATURE_ON == MBB_CHG_BQ27510)
    CHG_BATT_ID_TYPE battery_id = CHG_BATT_ID_MAX;
    struct chg_batt_data *batt_data = NULL;
    struct battery_info *batt_info = NULL;
#endif
    /* open file */
...
#endif
#if (FEATURE_ON == MBB_CHG_BQ27510)
    /*the firmware of coul saved as a head file,
    when updating the firmware we must detect the battery ID to distinguish
    the two kind of battery*/
    length = COUL_FIRMWARE_UPDATE_LEN;

    batt_info = (huawei_smem_info *)SRAM_BATTERY_INFO_ADDR;
    if (NULL == batt_info)
    {
        printk(KERN_ERR "SRAM_BATTERY_INFO_ADDR error!\n");
        return ERR_READ_CONFIG_NAME_ERROR;
    }

    batt_data = chg_get_batt_data(batt_info->batt_id_voltage);
    if (NULL == batt_data)
    {
        printk(KERN_ERR "get battery id failed!\n");
        return ERR_READ_CONFIG_NAME_ERROR;
    }

    battery_id = batt_data->batt_id;
    printk(KERN_ERR "[%s,%d]:battery_id = %d \n",__FUNCTION__,__LINE__,battery_id);
    switch(battery_id)
    {
        case CHG_BATT_ID_XINGWANGDA_6400MAH:
            buf = bq27510_cmd_buffer_xinwangda_ver1_0;
        break;
        case CHG_BATT_ID_FEIMAOTUI_6400MAH:
            buf = bq27510_cmd_buffer_feimaotui_ver1_0;
        break;
        default:
            return ERR_READ_CONFIG_NAME_ERROR;
    }
...
static ssize_t bq27510_attr_store(struct device_driver *driver,const char *buf, size_t count)
{
    ...
    if(0 == iRet)
    {
        /*when complete the update turn flag to true*/
...
int bq27510_coul_firmware_update_init(void)
{
    int pid;
    ...
    version = bq27510_get_firmware_version_by_i2c(g_battery_measure_by_bq27510_i2c_client);
    printk(KERN_DEBUG "kernel_thread begin to iRet =%d\r\n",iRet);
    printk(KERN_DEBUG "bq27510_coul_firmware_update_init version =%d\r\n",version);

    iRet = bsp_nvm_read(NV_ID_DRV_VER_FLAG,\
                           &real_factory_mode,\
                           sizeof(uint32_t));
    if (0 != iRet)
    {
        printk(KERN_ERR "NV_FACTORY_MODE_I read failed.\r\n");
        return iRet;
    }
    printk(KERN_ERR "NV_FACTORY_MODE_I real_factory_mode =%d.\r\n",\
        real_factory_mode);

    if(real_factory_mode)
    {
        iRet = kthread_run(bq27510_update_test, NULL, "coul_fw_up");
        coul_update_start_flag = true;
    }

    printk(KERN_DEBUG "kernel_thread end to  iRet =%d\r\n",iRet);
    return iRet;
}
...
void bq27510_sync_coulupdate_port_control(void)
{
    int count = 0;
    if(!bq27510_get_coul_update_start_flag())
    {
        printk(KERN_DEBUG "[%s]: LINE [%d] not need to delay notify port enum done \r\n",\
                    __FUNCTION__,__LINE__);
        return;
    }
    else
    {
        while(!bq27510_get_coul_update_status_flag())
        {
            /*the updating is not completed so wait until complete*/
            msleep(SLEEP_500MS);
            count++;
            if(MAX_SLEEP_TIME == count)
            {
                break;
            }
        }
    ...
    printk(KERN_ERR "[%s]: LINE [%d] coul update success count = %d \r\n",\
                    __FUNCTION__,__LINE__,count);
    bq27510_set_coul_update_start_flag(false);
    bq27510_set_coul_update_status_flag(false);
    return;
}
...
EXPORT_SYMBOL_GPL(bq27510_sync_coulupdate_port_control);
#endif/*FEATURE_ON == MBB_CHG_BQ27510*/
