...
#define BATT_TEMP_CHANNEL    0
#endif/*defined(BSP_CONFIG_BOARD_E5885Ls_93a)*/
...
#if defined(BSP_CONFIG_BOARD_E5885Ls_93a)
    /*add the work queue of update the battery temperature*/
    struct delayed_work bq_update_temp_work;
#endif
};
...
static int capacity_changed(struct bq_bci_device_info *di)
{
    ...
    int batt_rechg_threshold = BATT_CAPACITY_RECHG;
    ...
    */
    ...
    if (di->capacity == -1)
    {
        if (curr_capacity <= BATT_CAPACITY_LOW_BATT)
        {
            di->capacity = BATT_CAPACITY_LOW_BATT;
            di->prev_capacity = BATT_CAPACITY_LOW_BATT;
        }
        else
        {
    ...
    if (chg_is_batt_in_state_of_emergency() && (curr_capacity > BATT_CAPACITY_SHUTOFF))
    {
        curr_capacity = di->capacity - BATT_CAPACITY_INTERVAL;
        dev_err(di->dev,"emergency!!adjust Battery Capacity to %d Percents\n", curr_capacity);
    }
    /*电压未达到关机门限时，soc低于关机门限2%,则显示3%*/
    if ((curr_capacity <= BATT_CAPACITY_SHUTOFF) && (!chg_is_batt_in_state_of_emergency()))
    {
        curr_capacity = BATT_CAPACITY_SHUTOFF + BATT_CAPACITY_INTERVAL;
    ...
    case POWER_SUPPLY_STATUS_CHARGING:
        ...
        if (curr_capacity > BATT_CAPACITY_NINTY_NINE)
        {
            curr_capacity = BATT_CAPACITY_NINTY_NINE;
        }
        break;

    case POWER_SUPPLY_STATUS_FULL:
        /*开始充电到满电停充，但是soc未达到100%，则迅速平滑至100%*/
        if (curr_capacity <= BATT_CAPACITY_FULL)
        {
            curr_capacity = di->prev_capacity + BATT_CAPACITY_INTERVAL;
            if (curr_capacity >= BATT_CAPACITY_FULL)
            {
                curr_capacity = BATT_CAPACITY_FULL;
                dev_info(di->dev,"smooth soc=100\n");
            }
        }

        /*设置复充门限*/
        batt_rechg_threshold = chg_get_recharge_threshold();

        /*电量到达复充门限时，库仑计退出满电状态*/
        if (batt_rechg_threshold >= hisi_battery_capacity())
        {
            di->charge_status = POWER_SUPPLY_STATUS_NOT_CHARGING;
            dev_err(di->dev, "when recharging that the batt recharge is %d!\n", batt_rechg_threshold);
        }
        break;
    ...
    if (hisi_battery_current_avg() > 0)
    {
        ...
        if ((curr_capacity - di->prev_capacity) > BATT_CAPACITY_INTERVAL)
        {
            curr_capacity = di->prev_capacity + BATT_CAPACITY_INTERVAL;
        }
    }
...
static void bq_write_temp_to_share_work(struct work_struct *work)
{
    ...
    struct bq_bci_device_info *di = container_of(work,
        struct bq_bci_device_info, bq_update_temp_work.work);

    if(TEMPERATURE_MAGIC_DATA != p_area->magic_start \
        || (TEMPERATURE_MAGIC_DATA != p_area->magic_end))
    {
        dev_err(di->dev, "%s mem is writed by others.\n", __func__);
        return;
    }

    /*get the temperature after revise*/
    temp = chg_batt_temp_revise(hisi_battery_temperature());

    p_area->chan_out[BATT_TEMP_CHANNEL].temp_l = temp;
    p_area->chan_out[BATT_TEMP_CHANNEL].temp_r = ~temp;

    schedule_delayed_work(&di->bq_update_temp_work,
        msecs_to_jiffies(UPDATE_TEMP_TIME));
...
#if defined(BSP_CONFIG_BOARD_E5885Ls_93a)
            if (hisi_battery_capacity() <= BATT_CAPACITY_LOW_BATT)
            {
                capacity_filter[i] = BATT_CAPACITY_LOW_BATT;
            }
            else
            {
...
#if defined(BSP_CONFIG_BOARD_E5885Ls_93a)
    /*check the NV firstly, the ADC channel's out_config must set to 0*/
    if((0 == bsp_nvm_read(NV_ID_DRV_TEMP_HKADC_CONFIG, (u8*)&chan_cfg, \
        (u32)(HKADC_CHAN_MAX * sizeof(TEMP_HKADC_CHAN_CONFIG)))) \
            && (0 == chan_cfg[BATT_TEMP_CHANNEL].out_config))
    {
        INIT_DELAYED_WORK(&di->bq_update_temp_work,
            bq_write_temp_to_share_work);
        /*if it is update mode then do not start the work queue*/
        if(FALSE == is_in_update_mode())
        {
            schedule_delayed_work(&di->bq_update_temp_work, 0);
        }
    }
#endif
