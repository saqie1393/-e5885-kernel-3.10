...
int c_offset_b = 0;         /* ua */
    ...
    || defined(BSP_CONFIG_BOARD_R227h) || defined(BSP_CONFIG_BOARD_E5785Lh_67a))
#define E5785_UUC_DIV    (100)    /*UUC div value*/
#endif
/* 电池校准的最小及最大电压**/
...
#define COUL_CALI_ENABLE     (1<<7)

/*Hi6559 Reg CLJ_CTRL_REG, 0x253: ECO filter timer如下设定分别对应50-400ms.*/
#define COUL_ECO_FLT_50MS    (0<<4)
#define COUL_ECO_FLT_100MS    (0x1<<4)
#define COUL_ECO_FLT_150MS    (0x2<<4)
#define COUL_ECO_FLT_200MS    (0x3<<4)
#define COUL_ECO_FLT_250MS    (0x4<<4)
#define COUL_ECO_FLT_300MS    (0x5<<4)
#define COUL_ECO_FLT_350MS    (0x6<<4)
#define COUL_ECO_FLT_400MS    (0x7<<4)

#define COUL_ALL_REFLASH    (0x0<<3)
...
#define DEFAULT_COUL_CTRL_VAL   (COUL_ECO_FLT_200MS  \
                                | COUL_ALL_REFLASH |COUL_ECO_DISABLE)

/*Hi6559 Reg CLJ_CTRL_REG2, 0x2BB: */
#define COUL_CTRL_ENABLE     (1<<0)
#define COUL_CTRL_REG_CLR            (1<<1)
#define COUL_CTRL_REG_NOT_CLR    (0<<1)
#define COUL_CTRL_REG_CLR_MASK    (1<<1)

#define COUL_CTRL_CALI_AUTO_TIME_15S    (0<<2)
#define COUL_CTRL_CALI_AUTO_TIME_30S    (1<<2)
#define COUL_CTRL_CALI_AUTO_TIME_60S    (2<<2)
#define COUL_CTRL_CALI_AUTO_TIME_2MIN    (3<<2)
#define COUL_CTRL_CALI_AUTO_TIME_4MIN    (4<<2)
#define COUL_CTRL_CALI_AUTO_TIME_8MIN    (5<<2)
#define COUL_CTRL_CALI_AUTO_TIME_16MIN    (6<<2)
#define COUL_CTRL_CALI_AUTO_TIME_32MIN    (7<<2)

#define COUL_CTRL_CALI_AUTO_ENABLE    (1<<5)
#define COUL_CTRL_CALI_AUTO_DISABLE    (0<<5)
#define COUL_CTRL_REG_RESERVE    (0<<6)
...
/*库仑计相关寄存器宏定义，具体参数见芯片寄存器手册*/
#define SMARTSTAR_COUL_IRQ_REG      (0x24D)
#define SMARTSTAR_COUL_IRQ_MASK_REG (0x24E)

#define COUL_CALI_ING     (0x03)
/*库仑计内部校准参数寄存器*/
#define SMARTSTAR_COUL_CAIL_REG1    (0X12F)
#define SMARTSTAR_COUL_CAIL_REG2    (0X130)
#define SMARTSTAR_COUL_CTRL_REG    (0x253)
#define SMARTSTAR_CL_OUT_BASE       (0x255)
#define SMARTSTAR_CL_IN_BASE        (0x259)
#define SMARTSTAR_CHG_TIMER_BASE    (0x25D)
#define SMARTSTAR_LOAD_TIMER_BASE   (0x261)
#define SMARTSTAR_CL_INT_BASE    (0x265)
#define SMARTSTAR_VOL_INT_BASE    (0x269)
#define SMARTSTAR_OFFSET_CURRENT    (0x26B)
#define SMARTSTAR_OFFSET_VOLTAGE    (0x26D)
#define SMARTSTAR_OCV_REG_BASE      (0x26F)
#define SMARTSTAR_VOL_FIFO_BASE     (0x27B)
#define SMARTSTAR_CUR_FIFO_BASE     (0x28F)
#define SMARTSTAR_COUL_STATE_REG    (0x2B1)
/*ocv and ocv flag take 2bytes, so 0x1a6:0x1a5 used.*/
#define SMARTSTAR_SAVE_OCV_ADDR    (0x2A7)
#define SMARTSTAR_FIFO_CLEAR    (0x2B0)        /*Reg 0x1AE Bit3 on Hi6421*/
#define SMARTSTAR_NV_SAVE_SUCCESS    (0x2A9)   //use bit 1
#define SMARTSTAR_NV_READ_SUCCESS    (0x2A9) //use bit 2
#define SMARTSTAR_BATTERY_MOVE_ADDR    (0x2AA)
#define SMARTSTAR_DEBUG_REG    (0x2AF)
#define SMARTSTAR_COUL_CTRL_REG2    (0x2BB)
...
#if defined(BSP_CONFIG_BOARD_E5787Ph_92a)
...
int smartstar_battery_voltage_uv_nocail(void);
...
#define COUL_V_OFFSET_B         (0X7E)
#define COUL_V_OFFSET_A_HI    (0X80)
#define COUL_V_OFFSET_A_LO    (0XFF)

int g_coul_v_offset_a = 10000; /*库仑计内部校准参数a*/
int g_coul_v_offset_b = 0;     /*库仑计内部校准参数b*/


void read_coul_cail_para(void)
{
    ...
    int v_offset_a = 0;
    int v_offset_b = 0;
    /*read coul v_offset_a and coul v_offset_b*/
    SMARTSTAR_REGS_READ(SMARTSTAR_COUL_CAIL_REG1, &cail_temp1, 1);
    SMARTSTAR_REGS_READ(SMARTSTAR_COUL_CAIL_REG2, &cail_temp2, 1);

    SMARTSTAR_COUL_INF("read_coul_cail_para SMARTSTAR_OFFSET_VOLTAGE = %04x!\R\n", volreg_offset);
    SMARTSTAR_COUL_INF("read_coul_cail_para SMARTSTAR_COUL_CAIL_REG1 = %02x!\R\n", cail_temp1);
    SMARTSTAR_COUL_INF("read_coul_cail_para SMARTSTAR_COUL_CAIL_REG2 = %02x!\R\n", cail_temp2);

    v_offset_b = cail_temp2 & COUL_V_OFFSET_B;
    v_offset_b = v_offset_b >> 1;
    v_offset_b = v_offset_b * 10000 - 315000;/*10000--v_offset_b放大10000倍保存，315000--b基准值为31.5*/
    v_offset_a = cail_temp2 & COUL_V_OFFSET_A_HI;
    v_offset_a = v_offset_a << 1;
    v_offset_a += cail_temp1 & COUL_V_OFFSET_A_LO;
    v_offset_a += 9744;/*a基准值为0.9744*/

    g_coul_v_offset_a = v_offset_a;
    g_coul_v_offset_b = v_offset_b * 1000;/*转换成uV*/
    SMARTSTAR_COUL_INF("read_coul_cail_para v_offset_a = %d!\r\n", g_coul_v_offset_a);
    SMARTSTAR_COUL_INF("read_coul_cail_para v_offset_b = %d!\r\n", g_coul_v_offset_b);
}
...
/**
 * convert_regval2uv
 * 1 bit = 225.1196 uv = 4.8/21322 V = 4.8/21322 * 1000 * 1000 uV = 36 * 1000 * 100/ 15991 uV
...
static  int convert_regval2uv(short reg_val)
{
    ...
    temp = (s64)((s64)(reg_val)  * (s64)(36 * 1000 * 100));

    temp = div_s64(temp, 15991);
    /*库仑计内部校准*/
    temp = (temp * g_coul_v_offset_a + g_coul_v_offset_b);
    temp = div_s64 (temp, 10000);
...
static  short convert_uv2regval(int uv_val)
{
    ...
    temp = (s64)uv_val * (s64)(10000);
    temp -= g_coul_v_offset_b;
    temp = div_s64(temp,g_coul_v_offset_a);

    temp = (s64)((s64)temp * (s64)15991);
    temp = div_s64(temp,3600000);
    ret = (short)temp;
...
int smartstar_battery_voltage_uv_nocali(void)
{
    ...
    SMARTSTAR_REGS_READ(SMARTSTAR_VOL_FIFO_BASE, &regval, 2);
    return (convert_regval2uv_nocail(regval));
}
                ...
                int iavg_ua)
{
    ...
    || defined(BSP_CONFIG_BOARD_E5787Ph_92a) || defined(BSP_CONFIG_BOARD_E5787Ph_67a) \
    || defined(BSP_CONFIG_BOARD_R227h) || defined(BSP_CONFIG_BOARD_E5785Lh_67a))
    uuc_uah_iavg += fcc_uah / E5785_UUC_DIV;
#else
...
void update_ioffset_value(struct ss_coul_nv_info *pinfo)
{
    ...
    /*根据硬件提供的等效电阻计算得到*/
    pinfo->c_offset_a = TEN_MOHM_RESISTANCE_CORRECT;
}
...
static int save_nv_info(struct smartstar_coul_device *di)
{
    ...
    if (!di->is_nv_read){
        ...
        val = SMARTSTAR_REG_READ(SMARTSTAR_COUL_CTRL_REG2);
        SMARTSTAR_REG_WRITE(SMARTSTAR_COUL_CTRL_REG2, val & (~COUL_CTRL_REG_CLR_MASK));
...
#ifdef  BSP_CONFIG_BOARD_E5787Ph_92a
...
static void smartstar_coul_config_power_on_reg(struct smartstar_coul_device *di)
{
    ...
    reg_val = SMARTSTAR_REG_READ(SMARTSTAR_COUL_CTRL_REG2);
    /* Mode */
    /*使能库仑计*/
    reg_val = reg_val | COUL_CTRL_ENABLE;
    SMARTSTAR_REG_WRITE(SMARTSTAR_COUL_CTRL_REG2, reg_val);
}
...
{
    /* Mode */
    SMARTSTAR_REG_WRITE(SMARTSTAR_COUL_CTRL_REG2, (~COUL_CTRL_ENABLE) );
}
