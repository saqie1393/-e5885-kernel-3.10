...
boolean chg_set_cur_level(uint32_t chg_cur)
{
    ...
    {
        chg_cur = CHG_DEF_BATT_CUR_LEVEL;
    }
    /*MAX = BQ24192_ICHARGE_MAX,MIN = 20% of BQ24192_ICHARGE_OFFSET*/
    if ((BQ24192_ICHARGE_MAX < chg_cur)
        || (BQ24192_ICHARGE_OFFSET / 5 > chg_cur))
    {
        chg_print_level_message(CHG_MSG_ERR, "CHG_DRV: chg_set_cur_level Para invalid!\n");
        return FALSE;
    }
    /*IC 规定小于512mA,通过设置20% BIT位实现小电流设置*/
...
boolean chg_set_supply_limit(uint32_t lmt_val)
{
    ...
    /*工厂模式和非工厂模式下的电池不在位开机使能模式电池不在位，不做处理*/
    if( FALSE == chg_is_ftm_mode() && FALSE == chg_is_no_battery_powerup_enable())
    {
        if (CHG_BATT_ID_DEF == chg_get_batt_id())
        {
            lmt_val = CHG_DEF_BATT_PWR_SUPPLY_LEVEL;
        }
    }
...
boolean chg_set_vreg_level(uint32_t vreg_val)
{
    ...
    if (CHG_BATT_ID_DEF == chg_get_batt_id())
    {
        vreg_val = CHG_DEF_BATT_VREG_LEVEL;
    }
...
boolean chg_set_tmr_rst(void)
{
    ...
    /*充电IC看门狗未开，踢狗操作直接返回TRUE*/
    chg_print_level_message(CHG_MSG_INFO, "Watchdog disable chg_set_tmr_rst return TRUE!\n");
    return TRUE;
...
void chg_set_chip_cd(boolean enable)
{
    ...
    if (!gpio_is_valid(g_chg_hardpara_dtsi.chg_ic_en_gpio))
    {
    ...
    if (gpio_request(g_chg_hardpara_dtsi.chg_ic_en_gpio, "chip_cd"))
    {
    ...
    if(TRUE == enable)
    {
        gpio_direction_output(g_chg_hardpara_dtsi.chg_ic_en_gpio,TRUE);
        chg_print_level_message( CHG_MSG_DEBUG,"CHG_PLT:set charger enable GIIO success!\r\n ");
    }
    else
    {
        gpio_direction_output(g_chg_hardpara_dtsi.chg_ic_en_gpio,FALSE);
        chg_print_level_message( CHG_MSG_DEBUG,"CHG_PLT:set charger enable GIIO success!\r\n ");
    }
}
#else
...
boolean chg_chip_init(void)
{
...
#if (FEATURE_ON == MBB_CHG_BATT_UNIFORM)
    else if (FALSE == chg_set_dpm_val(g_chg_hardpara_dtsi.g_default_dpm_volt))
#else
    ...
    /*暂时先关闭watchdog,chg_set_tmr_rst踢狗操作直接返回TRUE.*/
