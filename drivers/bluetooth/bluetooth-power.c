...
#ifdef BSP_CONFIG_BOARD_TELEMATIC
#if (FEATURE_OFF == MBB_FACTORY)
/*rfkil0默认状态是0 BT_REGON默认电平也需配合默认输出低电平*/
    rc = gpio_request(g_bt_regon_gpio, "bt_sys_rst_n");
    if (rc) {
        BT_PWR_ERR("unable to request gpio %d (%d)\n",
                g_bt_regon_gpio, rc);
        return rc;
    }
    rc = gpio_direction_output(g_bt_regon_gpio,0);
    if (rc) {
        BT_PWR_ERR("Unable to set direction\n");
        return rc;
    }
    gpio_free(g_bt_regon_gpio);
#endif
#endif
...
static int  bt_power_probe(struct platform_device *pdev)
{
    ...
    //v722车载模块在GPS特性中使能32k CLKB，此处不再处理
#else
    /*使能bt clk*/
