...
#define UART3_IRQ        (223)     /*UART3中断号*/
#if ((FEATURE_ON == MBB_DEVBOOTSTATE) && (FEATURE_ON == MBB_FACTORY))
#define UART_UNREADY 0 /*0表示UART测试未准备OK*/
#define UART_RAEDY 1   /*1表示UART测试已经准备OK*/
static int g_hs_uart_flag = UART_UNREADY;
#endif
#if (FEATURE_ON == MBB_FACTORY) || (FEATURE_ON == MBB_AGING_TEST)
#define UART_PROC_FIlE  "uart_spi_type"
static int uart_mode_flag = MULTIPINS_UART;
#endif
#endif /* BSP_CONFIG_BOARD_TELEMATIC */
#define UART_NR			14
...
#ifdef BSP_CONFIG_BOARD_TELEMATIC
extern unsigned int get_ports_sec_lock(void);
#endif
...
static unsigned int get_fifosize_arm(unsigned int periphid)
{
	...
	return rev < 3 ? 16 : 32;/* [false alarm]:native code */
}
...
/*************************************************************************
* 函数名     :get_uart_flag
* 功能描述   :读取当前g_hs_uart_flag状态,g_hs_uart_flag标识uart3是否启动ok
* 输入参数   :
* 输出参数   :
* 返回值     :
**************************************************************************/
...
static int pl011_fifo_to_tty(struct uart_amba_port *uap)
{
	...
	while (max_count--) {
        ...
        /* 烧片版本中开启物理串口回环测试时，不处理串口输入 */
#else
#ifdef BSP_CONFIG_BOARD_TELEMATIC
#define UART0_IRQ_NUMBER 102
        if ((0 == lock) || (uap->port.irq != UART0_IRQ_NUMBER))
        {
...
static unsigned int pl011_tx_empty(struct uart_port *port)
{
	...
	return status & (UART01x_FR_BUSY|UART01x_FR_TXFF) ? 0 : TIOCSER_TEMT;/* [false alarm]:native code */
}
...
#ifdef BSP_CONFIG_BOARD_TELEMATIC
    /* 当uart端口为以下特定配置时直接取custom_divisor作为设置的波特率 */
    if ((BAUD_38400 == baud) && ((port->flags & UPF_SPD_MASK) == UPF_SPD_CUST))
    {
        baud = port->custom_divisor;
    }
#endif
...
static int pl011_probe(struct amba_device *dev, const struct amba_id *id)
{
    ...
    {
        MultiPinsPara.UART_SPI = multipins_data->UART_SPI;
    }
    else
    {
        printk("factory mode read the wrong num,but set UART\n");
        MultiPinsPara.UART_SPI = MULTIPINS_UART;
    }
    uart_mode_flag = MultiPinsPara.UART_SPI;
#else
    ...
    {
        MultiPinsPara.UART_SPI = MULTIPINS_UART;
        printk("read wrong nv num, but set to UART\n");
    }
#endif

    /*如果是SPI 模式则不初始化,223为UART3的中断号*/
    if ((MULTIPINS_SPI == MultiPinsPara.UART_SPI) && (UART3_IRQ == dev->irq[0]))
    {
        printk("the mode is SPI,UART3_IRQ == dev->irq[0], not init UART3\n");
        ret = -EBUSY;
        goto out;
    }
#endif /* BSP_CONFIG_BOARD_TELEMATIC */
...
#ifdef CONFIG_SERIAL_AMBA_PL011_BALONG
#include <linux/amba/amba_pl011_balong.h>
#endif
...
#if ((defined BSP_CONFIG_BOARD_TELEMATIC) && ((FEATURE_ON == MBB_FACTORY) || (FEATURE_ON == MBB_AGING_TEST)))
static ssize_t uart_spi_proc_read(struct file *filp, char *buffer, size_t length, loff_t *offset)
{
    int ret  = -1;
    int mode = -1;

    mode = uart_mode_flag;
    printk(KERN_ERR "%s UART_SPI mode=%d\r\n", __FUNCTION__, mode);
    ret = copy_to_user(buffer, (void *)&mode, sizeof(int));
    if (0 != ret)
    {
        printk(KERN_ERR "UART_SPI copy_to_user fail!\r\n");
    }
#if ((FEATURE_ON == MBB_DEVBOOTSTATE) && (FEATURE_ON == MBB_FACTORY))
    g_hs_uart_flag = UART_RAEDY;
#endif
    return ret;
}

static ssize_t uart_spi_proc_write(struct file *filp, char *buffer, size_t length, loff_t *offset)
{
    return 0;
}

static const struct file_operations uart_spi_ops = {
    .owner    = THIS_MODULE,
    .read     = uart_spi_proc_read,
    .write    = uart_spi_proc_write,
};
#endif
...
#if ((defined BSP_CONFIG_BOARD_TELEMATIC) && ((FEATURE_ON == MBB_FACTORY) || (FEATURE_ON == MBB_AGING_TEST)))
        /*创建proc文件节点uart_spi_type*/
        if (NULL == proc_create(UART_PROC_FIlE, S_IFREG | S_IRUGO | S_IWUSR, NULL, &uart_spi_ops))
        {
            printk(KERN_ERR"%s: create proc file for uart3 failed\n", __FUNCTION__);
        }
#endif
...
static void __exit pl011_exit(void)
{
	...
	uart_unregister_driver(&amba_reg);
#if ((defined BSP_CONFIG_BOARD_TELEMATIC) && ((FEATURE_ON == MBB_FACTORY) || (FEATURE_ON == MBB_AGING_TEST)))
    /*移除proc文件uart_spi_type*/
    remove_proc_entry(UART_PROC_FIlE, NULL);
#endif
}
