...
static ssize_t
remove_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    return 0;
}
#endif
...
#if (FEATURE_ON == MBB_TELE_WIFI)
static ssize_t
dev_bus_rescan_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    return 0;
}
#endif
...
#if (FEATURE_ON == MBB_TELE_WIFI)
static ssize_t
dev_bus_suspend_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    return 0;
}

static ssize_t
dev_bus_suspend_store(struct device *dev, struct device_attribute *attr,
        const char *buf, size_t count)
{
    unsigned long val = 0;
    struct pci_bus *bus = to_pci_bus(dev);
...
extern int set_power_bcm89335(int val);
static ssize_t
dev_bus_set_power_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    return 0;
}

static ssize_t
dev_bus_set_power_store(struct device *dev, struct device_attribute *attr,
        const char *buf, size_t count)
{
    unsigned long val = 0;
    struct pci_bus *bus = to_pci_bus(dev);
    ...
    if (val) {
        set_power_bcm89335(1);
    } else {
        set_power_bcm89335(0);
    }
    return count;
}
#endif
...
#if (FEATURE_ON == MBB_TELE_WIFI)
    __ATTR(remove, (S_IRUSR|S_IRGRP|S_IWUSR|S_IWGRP), remove_show, remove_store),
#else
...
#if (FEATURE_ON == MBB_TELE_WIFI)
    __ATTR(rescan, (S_IRUSR|S_IRGRP|S_IWUSR|S_IWGRP),
        dev_bus_rescan_show, dev_bus_rescan_store),
#else
...
#if (FEATURE_ON == MBB_TELE_WIFI)
    __ATTR(suspend, (S_IRUSR|S_IRGRP|S_IWUSR|S_IWGRP),
        dev_bus_suspend_show, dev_bus_suspend_store),
    __ATTR(set_power, (S_IRUSR|S_IRGRP|S_IWUSR|S_IWGRP), 
        dev_bus_set_power_show, dev_bus_set_power_store),
#endif
	__ATTR_NULL,
};
